...
async function resolveChatGroupForRequest(groupIdParam: number, user: { id: number; role: string }) {
  // 1) If this is a package-subject-item group id, map it to a chat_groups row (create if missing)
  const pkgGroupRes = await pool.query(
    `SELECT
  ...
  if (pkgGroupRes.rowCount) {
    const row = pkgGroupRes.rows[0] as any;
    ...
    if (user.role === 'student') {
      const ok = await pool.query(
        `SELECT 1
         ...
         LIMIT 1`,
        [packageSubjectGroupId, packageId, user.id],
      );
      if (!ok.rowCount) return { allowed: false as const, reason: 'Not in this package group' };
    } else if (user.role === 'teacher') {
      if (!teacherId || teacherId !== user.id) return { allowed: false as const, reason: 'Not allowed' };
    } // admin allowed

    const chatGroup = await ChatService.getOrCreatePackageSubjectGroupChat(packageSubjectGroupId);
    // ensure membership rows exist (keeps same structure as existing chat)
    if (user.role === 'student') await ChatService.addMember(chatGroup.id, user.id, 'student');
    if (user.role === 'teacher') await ChatService.addMember(chatGroup.id, user.id, 'teacher');
  ...
  const legacy = await pool.query(`SELECT id, owner_teacher_id, package_subject_group_id FROM chat_groups WHERE id = $1`, [
    groupIdParam,
  ]);
  if (!legacy.rowCount) return { allowed: false as const, reason: 'Group not found' };
  return { allowed: true as const, kind: 'legacy' as const, chatGroupId: groupIdParam };
}

async function getRoomForChatGroup(chatGroupId: number): Promise<string> {
  const r = await pool.query(
    `SELECT id, package_subject_group_id
     FROM chat_groups
     WHERE id = $1`,
    [chatGroupId],
  );
  if (!r.rowCount) return `group:${chatGroupId}`;
  const row = r.rows[0] as any;
  return row.package_subject_group_id ? `group_${Number(row.package_subject_group_id)}` : `group:${chatGroupId}`;
}

async function userCanAccessChatGroup(chatGroupId: number, user: { id: number; role: string }): Promise<boolean> {
  if (user.role === 'admin') return true;

  const cgRes = await pool.query(
    `SELECT id, owner_teacher_id, package_subject_group_id, direct_student_id, direct_teacher_id
     FROM chat_groups
     WHERE id = $1`,
    [chatGroupId],
  );
  if (!cgRes.rowCount) return false;
  const cg = cgRes.rows[0] as any;
  ...
  if (cg.package_subject_group_id) {
    if (user.role === 'teacher') {
      const ok = await pool.query(
        `SELECT 1 FROM package_subject_item_groups g WHERE g.id = $1 AND g.teacher_id = $2`,
        [cg.package_subject_group_id, user.id],
      );
      if (ok.rowCount) return true;
    }
  ...
  if (user.role === 'teacher' && cg.owner_teacher_id && Number(cg.owner_teacher_id) === user.id) return true;

  const mem = await pool.query(`SELECT 1 FROM chat_group_members WHERE group_id = $1 AND user_id = $2`, [
    chatGroupId,
    user.id,
  ]);
  return !!mem.rowCount;
...
router.get(
  ...
  asyncWrapper(async (req, res) => {
    ...
    if (user.role === 'student') {
      ...
      const groups = await ChatService.getGroupsForStudent(user.id);
      // Append teacher name to group label
      const withNames = await pool.query(
        `SELECT cg.id, u.name AS teacher_name FROM chat_groups cg JOIN users u ON u.id = cg.owner_teacher_id WHERE cg.id = ANY($1::int[])`,
        [groups.map((g) => g.id)],
      );
      const idToTeacher: Record<number, string> = {};
      for (const r of withNames.rows) idToTeacher[r.id] = r.teacher_name;

      // Package-subject groups the student belongs to (with teacher + students list)
      const pkgGroupsRes = await pool.query(
        `SELECT
      ...
      const packageGroupIds = pkgGroupsRes.rows.map((r: any) => Number(r.package_subject_group_id));
      const studentsByGroup: Record<number, any[]> = {};
      if (packageGroupIds.length) {
        const st = await pool.query(
          `SELECT gs.group_id, u.id, u.name, u.avatar
           FROM package_subject_item_group_students gs
        ...
        for (const row of st.rows as any[]) {
      ...
      return res.json({
        ...
        package_subject_groups: pkgGroupsRes.rows.map((r: any) => ({
    ...
    if (user.role === 'teacher') {
      ...
      return res.json({
        ...
        all_groups: [
          ...result.rows.map((g: any) => ({ type: 'grade_chat', chat_group_id: g.id, ...g })),
          ...pkg.rows.map((r: any) => ({
            type: 'package_subject_group',
            chat_group_id: Number(r.chat_group_id),
            allow_student_send: Boolean(r.allow_student_send),
    ...
    // Admin: all groups
    const result = await pool.query(`SELECT * FROM chat_groups ORDER BY grade_id`);
    const pkg = await pool.query(
      `SELECT
         cg.id AS chat_group_id,
         cg.allow_student_send,
...
router.get(
  ...
  asyncWrapper(async (req, res) => {
    const user = req.user!;
    if (user.role === 'student') {
      const teachers = await ChatService.listDirectTeachersForStudent(user.id);
      return res.json({
        contacts: teachers.map((t: any) => ({
          type: 'teacher',
    ...
    if (user.role === 'teacher') {
      const r = await ChatService.listDirectStudentsForTeacher(user.id);
      return res.json({
        contacts: r.map((s: any) => ({
          type: 'student',
...
router.get(
  '/direct/:otherId/messages',
  authMiddleware(['student', 'teacher', 'admin']),
  asyncWrapper(async (req, res) => {
    const otherId = Number(req.params.otherId);
    if (isNaN(otherId)) return res.status(400).json({ message: 'Invalid otherId' });
    const user = req.user!;
    ...
    if (user.role === 'student') {
      ...
      const messages = await ChatService.getHistory(cg.id, limit ? Number(limit) : 50, before);
    ...
    const cgRes = await pool.query(
      `SELECT id FROM chat_groups WHERE (direct_student_id = $1 AND direct_teacher_id IS NOT NULL) OR (direct_teacher_id = $1 AND direct_student_id IS NOT NULL) LIMIT 1`,
      [otherId],
    );
    if (!cgRes.rowCount) return res.status(404).json({ message: 'Not found' });
    const messages = await ChatService.getHistory(Number(cgRes.rows[0].id), limit ? Number(limit) : 50, before);
    const otherUser = await pool.query(`SELECT id, name, avatar, role FROM users WHERE id = $1`, [otherId]);
    return res.json({
...
router.post(
  ...
  asyncWrapper(async (req, res) => {
    ...
    let chatGroupId: number | null = null;
    ...
    const replyTo: any = parsed.data.reply_to_message_id ?? null;
    if (replyTo !== null) {
      const replyCheck = await pool.query(
        `SELECT id, group_id FROM chat_messages WHERE id = $1 AND group_id = $2`,
        [replyTo, chatGroupId],
      );
    ...
    const saved = await ChatService.saveMessage(chatGroupId, user.id, text, replyTo);
    const appAny = req.app as any;
    const io = appAny.io as any;
    ...
    if (typeof appAny.emitChatMessage === 'function') appAny.emitChatMessage(chatGroupId, realtimePayload);
    ...
    if (io) {
      ...
      if (recipientId) io.to(`user:${recipientId}`).emit('chat:new-message', realtimePayload);
    }
...
router.put(
  ...
  asyncWrapper(async (req, res) => {
    ...
    const schema = z.object({ message: z.string().min(1).optional(), text: z.string().min(1).optional() });
    const parsed = schema.safeParse(req.body);
    ...
    const existing = await pool.query(
      `SELECT id, group_id, sender_id, attachment_url, attachment_type
       FROM chat_messages
       WHERE id = $1`,
      [messageId],
    );
    if (!existing.rowCount) return res.status(404).json({ message: 'Message not found' });
    const msg = existing.rows[0] as any;

    const canAccess = await userCanAccessChatGroup(Number(msg.group_id), user);
    if (!canAccess) return res.status(403).json({ message: 'Not allowed' });
    ...
    const appAny = req.app as any;
    const io = appAny.io as any;
    if (io) io.to(room).emit('chat:message-updated', { message: updated.rows[0] });
...
router.delete(
  ...
  asyncWrapper(async (req, res) => {
    ...
    const user = req.user!;
    const existing = await pool.query(`SELECT id, group_id, sender_id FROM chat_messages WHERE id = $1`, [messageId]);
    if (!existing.rowCount) return res.status(404).json({ message: 'Message not found' });
    const msg = existing.rows[0] as any;
    ...
    const appAny = req.app as any;
    const io = appAny.io as any;
    if (io) io.to(room).emit('chat:message-deleted', { message_id: messageId, group_id: Number(msg.group_id) });
...
router.get(
  ...
  asyncWrapper(async (req, res) => {
    ...
    if (isNaN(groupIdParam)) return res.status(400).json({ message: 'Invalid group id' });

    const user = req.user!;
    const resolved = await resolveChatGroupForRequest(groupIdParam, user);
    if (!resolved.allowed) return res.status(403).json({ message: resolved.reason });

    const { limit, before } = req.query as any;
    const messages = await ChatService.getHistory(resolved.chatGroupId, limit ? Number(limit) : 50, before);
    return res.json({
...
router.get(
  ...
  asyncWrapper(async (req, res) => {
    ...
    if (user.role !== 'admin') {
      if (user.role === 'teacher') {
        const cgRes = await pool.query(`SELECT id, owner_teacher_id, package_subject_group_id FROM chat_groups WHERE id = $1`, [groupId]);
        if (!cgRes.rowCount) return res.status(404).json({ message: 'Group not found' });
        const cg = cgRes.rows[0] as any;
        if (cg.package_subject_group_id) {
          const ok = await pool.query(
            `SELECT 1 FROM package_subject_item_groups g WHERE g.id = $1 AND g.teacher_id = $2`,
          ...
          if (!ok.rowCount) return res.status(403).json({ message: 'Not in this group' });
        } else {
          if (Number(cg.owner_teacher_id) !== user.id) return res.status(403).json({ message: 'Not in this group' });
        }
      } else {
        ...
        if (!mem.rowCount) return res.status(403).json({ message: 'Not in this group' });
      }
    }
    const { limit, before } = req.query as any;
    const history = await ChatService.getHistory(resolved.chatGroupId, limit ? Number(limit) : 50, before);
...
router.get(
  ...
  asyncWrapper(async (req, res) => {
    ...
    if (req.user!.role === 'teacher') {
      ...
      const cg = cgRes.rows[0] as any;
      if (cg.package_subject_group_id) {
        const ok = await pool.query(`SELECT 1 FROM package_subject_item_groups g WHERE g.id = $1 AND g.teacher_id = $2`, [cg.package_subject_group_id, req.user!.id]);
        if (!ok.rowCount) return res.status(403).json({ message: 'Not allowed' });
      } else {
        if (Number(cg.owner_teacher_id) !== req.user!.id) return res.status(403).json({ message: 'Not allowed' });
      }
    }
    const members = await ChatService.listMembers(groupId);
    res.json({ members });
  }),
);
...
router.patch(
  ...
  asyncWrapper(async (req, res) => {
    const groupId = Number(req.params.groupId);
    const { allow_student_send } = req.body as any;
    ...
    if (req.user!.role === 'teacher') {
      ...
      if (cg.package_subject_group_id) {
        const ok = await pool.query(
          `SELECT 1 FROM package_subject_item_groups g WHERE g.id = $1 AND g.teacher_id = $2`,
          [cg.package_subject_group_id, req.user!.id],
        );
        if (!ok.rowCount) return res.status(403).json({ message: 'Not allowed' });
      } else {
        if (Number(cg.owner_teacher_id) !== req.user!.id) return res.status(403).json({ message: 'Not allowed' });
    ...
    await ChatService.setStudentPermission(groupId, allow_student_send);
...
router.post(
  ...
  asyncWrapper(async (req, res) => {
    ...
    if (user.role === 'admin') {
      ...
      const cg = await pool.query(`SELECT id, owner_teacher_id, package_subject_group_id FROM chat_groups WHERE id = $1`, [chatGroupId]);
      if (!cg.rowCount) return res.status(404).json({ message: 'Group not found' });
      const row = cg.rows[0] as any;
      if (row.package_subject_group_id) {
        const ok = await pool.query(
          `SELECT 1 FROM package_subject_item_groups g WHERE g.id = $1 AND g.teacher_id = $2`,
          [row.package_subject_group_id, user.id],
        );
        if (!ok.rowCount) return res.status(403).json({ message: 'Not allowed' });
      } else {
        // grade chat: teacher must teach the grade
        const teachRes = await pool.query(
          `SELECT 1 FROM chat_groups cg JOIN teacher_grades tg ON tg.grade_id = cg.grade_id
         WHERE cg.id = $1 AND tg.teacher_id = $2`,
          [chatGroupId, user.id],
        );
        if (!teachRes.rowCount) return res.status(403).json({ message: 'Not allowed' });
      }
    } else {
      const mem = await pool.query(
      ...
      if (role === 'student') {
        const can = await ChatService.canStudentSend(chatGroupId);
        if (!can) return res.status(403).json({ message: 'Sending disabled by teacher' });
      }
    }
    ...
    try {
      const groupResult = await pool.query('SELECT name FROM chat_groups WHERE id = $1', [chatGroupId]);
  ...
  asyncWrapper(async (req, res) => {
    ...
    if (user.role === 'admin') {
      ...
      const cg = await pool.query(
        `SELECT id, owner_teacher_id, package_subject_group_id FROM chat_groups WHERE id = $1`,
        [groupId],
      );
      if (!cg.rowCount) return res.status(404).json({ message: 'Group not found' });
      const row = cg.rows[0] as any;
    ...
    const saved = await ChatService.saveAttachmentMessage(groupId, user.id, {
    ...
    // إرسال إشعارات لأعضاء المجموعة
    try {
      const groupResult = await pool.query('SELECT name FROM chat_groups WHERE id = $1', [groupId]);
      const groupName = groupResult.rows[0]?.name || 'المجموعة';
    ...
    const appAny = req.app as any;
    if (typeof appAny.emitChatMessage === 'function') appAny.emitChatMessage(groupId, saved);
    res.status(201).json({ message: saved });
  }),
);
