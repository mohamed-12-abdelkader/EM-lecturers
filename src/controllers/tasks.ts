...
import { TaskService, TaskData, TaskUpdateData, TaskListFilters } from '../services/tasks';
...
async function resolveTaskAssigneeEmployeeId(body: {
  ...
  if (rawUser !== undefined && rawUser !== null && String(rawUser).trim() !== '') {
    ...
    const e = await EmployeeService.getEmployeeByUserId(uid);
    return e ? e.id : null;
  }
  ...
  let e = await EmployeeService.getEmployeeById(n);
  if (e) return e.id;
  e = await EmployeeService.getEmployeeByUserId(n);
  return e ? e.id : null;
}
...
router.post('/', authMiddleware(['admin']), async (req: Request, res: Response) => {
  try {
    const { title, description, priority, due_date, deadline, start_date, assigned_to, assigned_user_id } =
      req.body;
    ...
    if (assigneeEmployeeId == null) {
      return res.status(400).json({
        ...
        hint:
          'أرسل assigned_to = معرف سجل الموظف (employees.id) أو assigned_user_id = معرف المستخدم (users.id). التطبيقات التي ترسل users.id في assigned_to تُدعم تلقائياً إذا لم يُوجد employees.id بهذا الرقم.',
      });
    }
    ...
    const task = await TaskService.createTask(taskData, assignedBy);
...
router.get('/', authMiddleware(['admin']), async (req: Request, res: Response) => {
  try {
    ...
    const filters: TaskListFilters = {
      ...
      deadline_from: deadline_from as string | undefined,
      deadline_to: deadline_to as string | undefined,
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const tasks = await TaskService.getEmployeeTasks(employee.id, filters);
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const dashboard = await TaskService.getEmployeeDashboard(employee.id, userId);
      res.json(dashboard);
    } catch (error) {
      logger.error('Error fetching employee task dashboard:', error);
      res.status(500).json({
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const history = await TaskService.getEmployeeTaskHistory(employee.id);
      res.json({ history, total: history.length });
    } catch (error) {
      logger.error('Error fetching employee task history:', error);
      res.status(500).json({
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const stats = await TaskService.getTaskStats(employee.id);
...
router.get('/stats/overview', authMiddleware(['admin']), async (req: Request, res: Response) => {
  try {
...
// أداء كل موظف (للادمن)
router.get('/stats/by-employee', authMiddleware(['admin']), async (_req: Request, res: Response) => {
  try {
    const rows = await TaskService.getStatsByEmployee();
    res.json({ employees: rows, message: 'أداء الموظفين حسب المهام' });
  } catch (error) {
    logger.error('Error fetching task stats by employee:', error);
    res.status(500).json({
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      await pool.query('UPDATE users SET role = $1 WHERE id = $2', ['employee', userId]);

      // إنشاء employee record
      const employeeResult = await pool.query(
        `INSERT INTO employees (user_id, name, email, permissions, created_by)
         VALUES ($1, $2, $3, $4, $5)
         RETURNING *`,
        [
          userId,
    ...
    try {
      ...
      const existingUser = await pool.query(
        `SELECT id FROM users WHERE tenant_id = $3
         AND (email = $1 OR ($2::text IS NOT NULL AND length(trim($2::text)) > 0 AND phone = $2))`,
        [email, phone ?? null, tenantId],
      );
      ...
      const userResult = await pool.query(
        ...
        [name, email, phone, hashedPassword, tenantId],
      );
...
router.get('/:id', authMiddleware(['admin', 'employee']), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    try {
      await TaskService.assertUserCanViewTask(taskId, user.id, user.role);
    } catch (e) {
...
router.put('/:id', authMiddleware(['admin']), async (req: Request, res: Response) => {
  try {
    ...
    delete (updateData as any).due_date;

    const task = await TaskService.updateTask(parseInt(id, 10), updateData);
...
router.patch('/:id/start', authMiddleware(['employee']), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const userId = (req as any).user.id;
    const task = await TaskService.startTask(parseInt(id, 10), userId);
...
router.patch('/:id/complete', authMiddleware(['employee']), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const userId = (req as any).user.id;
    const message = typeof req.body?.message === 'string' ? req.body.message : null;
    const task = await TaskService.completeTask(parseInt(id, 10), userId, message);
...
router.patch('/:id/approve', authMiddleware(['admin']), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { admin_notes } = req.body;
    const userId = (req as any).user.id;
    
    const task = await TaskService.approveTask(parseInt(id, 10), userId, admin_notes);
...
// رفض المهمة (للأدمن)
router.patch('/:id/reject', authMiddleware(['admin']), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { admin_notes } = req.body;
    const userId = (req as any).user.id;
    
    if (!admin_notes) return res.status(400).json({ error: 'برجاء كتابة سبب الرفض' });
    
    const task = await TaskService.rejectTask(parseInt(id, 10), userId, admin_notes);
...
// جلب Logs المهمة
router.get('/:id/logs', authMiddleware(['admin', 'employee']), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const taskId = parseInt(id, 10);
    const user = (req as any).user;
...
router.post('/:id/comments', authMiddleware(['admin', 'employee']), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    const taskComment = await TaskService.addTaskComment(taskId, user.id, employeeId, comment.trim());
...
// جلب تعليقات مهمة
router.get('/:id/comments', authMiddleware(['admin', 'employee']), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const taskId = parseInt(id, 10);
    const user = (req as any).user;
...
// جلب ملفات مهمة
router.get('/:id/attachments', authMiddleware(['admin', 'employee']), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const taskId = parseInt(id, 10);
    const user = (req as any).user;
