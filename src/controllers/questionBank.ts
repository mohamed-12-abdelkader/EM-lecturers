    ...
    try {
      ...
      if (gradeResult.rowCount && gradeResult.rowCount > 0) {
        const qbResult = await pool.query(
          `SELECT id, name FROM question_banks WHERE grade_id = $1`,
          [gradeResult.rows[0].grade_id],
        );
      ...
      if (questionBank) {
        const subjectsResult = await pool.query(
          `SELECT id, name FROM subjects WHERE question_bank_id = $1 AND is_active = true`,
          [questionBank.id],
        );
  ...
  asyncWrapper(async (req, res) => {
    try {
      ...
      const studentGradeResult = await pool.query(
        `SELECT ug.grade_id FROM user_grades ug WHERE ug.user_id = $1 LIMIT 1`,
        [studentId],
      );
...
router.get(
  ...
  asyncWrapper(async (req, res) => {
    try {
      ...
      const chapterCheckResult = await pool.query(
        `SELECT c.id, c.name, s.name as subject_name
         FROM chapters c
         JOIN subjects s ON s.id = c.subject_id
         JOIN question_banks qb ON qb.id = s.question_bank_id
         JOIN user_grades ug ON ug.grade_id = qb.grade_id
         WHERE c.id = $1 AND ug.user_id = $2`,
        [chapterId, studentId],
      );
      ...
      if (!chapterCheckResult.rowCount) {
        console.log(
          `Debug - Chapter ${chapterId} not found or not accessible for student ${studentId}`,
        );
...
router.get(
  '/student/lessons/:lessonId/questions',
  authMiddleware(['student']),
  asyncWrapper(async (req, res) => {
    try {
      const studentId = req.user!.id;
      ...
      try {
        lessonCheckResult = await pool.query(
          `SELECT l.id 
         FROM lessons l
         ...
         JOIN question_banks qb ON qb.id = s.question_bank_id
         JOIN user_grades ug ON ug.grade_id = qb.grade_id
         WHERE l.id = $1 AND ug.user_id = $2 AND l.is_active = true`,
          [lessonId, studentId],
        );
      } catch {
