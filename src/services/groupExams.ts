...
export class GroupExamService {
  // إنشاء امتحان جديد للمجموعة
  static async createGroupExam(examData: GroupExamData) {
    const result = await pool.query(
      `INSERT INTO group_exams 
       (group_id, name, total_grade, exam_date) 
       VALUES ($1, $2, $3, $4) 
       RETURNING *`,
      [examData.group_id, examData.name, examData.total_grade || 100, examData.exam_date || null],
    );
    return result.rows[0];
  }

  // تحديث امتحان
  static async updateGroupExam(
    examId: number,
    teacherId: number,
    updateData: Partial<GroupExamData>,
  ) {
    ...
    const groupResult = await pool.query('SELECT teacher_id FROM study_groups WHERE id = $1', [
      exam.group_id,
    ]);
  ...
  // حذف امتحان
  static async deleteGroupExam(examId: number, teacherId: number) {
    // التحقق من ملكية المجموعة
    const exam = await this.getGroupExamById(examId);
    if (!exam) {
      throw new Error('الامتحان غير موجود');
    ...
    const result = await pool.query('DELETE FROM group_exams WHERE id = $1 RETURNING *', [examId]);
    return result.rows[0];
  }

  // جلب امتحان بواسطة ID
  static async getGroupExamById(examId: number) {
    const result = await pool.query(
      `SELECT ge.*, sg.name as group_name, sg.teacher_id
       FROM group_exams ge
  ...
  static async getGroupExams(groupId: number) {
    const result = await pool.query(
      `SELECT ge.*, 
  ...
  static async addStudentGrade(gradeData: ExamGradeData, teacherId: number) {
    ...
    const studentExists = await pool.query('SELECT id, name, role FROM users WHERE id = $1', [
    ...
    if (studentExists.rows[0].role !== 'student') {
      throw new Error(
        `المستخدم برقم ${gradeData.student_id} ليس طالب (الدور: ${studentExists.rows[0].role})`,
      );
    }
    ...
    const studentInGroup = await pool.query(
      ...
      [exam.group_id, gradeData.student_id],
    );

    if (studentInGroup.rows.length === 0) {
      // إضافة معلومات debugging
      const groupStudents = await pool.query(
        'SELECT gs.student_id, u.name FROM group_students gs JOIN users u ON gs.student_id = u.id WHERE gs.group_id = $1',
        [exam.group_id],
      );
      ...
      const studentList = groupStudents.rows
        ...
        .join(', ');
      throw new Error(
        `الطالب ${studentExists.rows[0].name} (ID: ${gradeData.student_id}) غير موجود في المجموعة. الطلاب الموجودون في المجموعة: ${studentList || 'لا يوجد طلاب'}`,
      );
    }
    ...
    const result = await pool.query(
      `INSERT INTO group_exam_grades 
       ...
       DO UPDATE SET 
         ...
         updated_at = NOW()
       RETURNING *`,
      [gradeData.exam_id, gradeData.student_id, gradeData.grade, gradeData.notes || null],
    );
    return result.rows[0];
  }
  ...
  // حذف درجة طالب
  static async deleteStudentGrade(examId: number, studentId: number, teacherId: number) {
    // التحقق من ملكية الامتحان
    const exam = await this.getGroupExamById(examId);
    if (!exam) {
      throw new Error('الامتحان غير موجود');
    ...
    const result = await pool.query(
      'DELETE FROM group_exam_grades WHERE exam_id = $1 AND student_id = $2 RETURNING *',
      [examId, studentId],
    );
    return result.rows[0];
  }
  ...
  static async getExamGrades(examId: number) {
    const result = await pool.query(
      `SELECT geg.*, 
  ...
  static async getStudentGrades(groupId: number, studentId: number) {
    const result = await pool.query(
      `SELECT geg.*, 
              ge.name as exam_name,
              ge.total_grade,
  ...
  // جلب إحصائيات امتحان
  static async getExamStats(examId: number) {
    const result = await pool.query(
      `SELECT 
       ...
       WHERE ge.id = $1
       GROUP BY ge.id, ge.total_grade, ge.name`,
      [examId],
    );
    return result.rows[0];
  }
  ...
  static async getGroupStudents(groupId: number) {
    const result = await pool.query(
      `SELECT gs.student_id, u.name as student_name, u.email
  ...
  static async diagnoseStudentIds(examId: number) {
    const exam = await this.getGroupExamById(examId);
    if (!exam) {
      throw new Error('الامتحان غير موجود');
    }
    ...
    const orphanedRecords = await pool.query(
      `SELECT gs.student_id, gs.group_id, gs.joined_at
       FROM group_students gs
       LEFT JOIN users u ON gs.student_id = u.id
       WHERE u.id IS NULL AND gs.group_id = $1`,
      [exam.group_id],
    );
  ...
  static async fixStudentIds(
    examId: number,
    studentMappings: Array<{ old_id: number; new_id: number }>,
  ) {
    const exam = await this.getGroupExamById(examId);
    if (!exam) {
      throw new Error('الامتحان غير موجود');
    ...
    for (const mapping of studentMappings) {
      try {
        ...
        const newStudent = await pool.query('SELECT id, name, role FROM users WHERE id = $1', [
          mapping.new_id,
        ]);
        ...
        // تحديث ID الطالب
        const result = await pool.query(
          'UPDATE group_students SET student_id = $1 WHERE group_id = $2 AND student_id = $3 RETURNING *',
          [mapping.new_id, exam.group_id, mapping.old_id],
        );
  ...
  static async fixWrongRoleStudents(
    examId: number,
    replacements: Array<{
      wrong_id: number;
    ...
    for (const replacement of replacements) {
      try {
        ...
        const wrongUser = await pool.query(
          'SELECT gs.*, u.name, u.role FROM group_students gs JOIN users u ON gs.student_id = u.id WHERE gs.group_id = $1 AND gs.student_id = $2',
          [exam.group_id, replacement.wrong_id],
        );
        ...
        const newStudent = await pool.query(
          `INSERT INTO users (name, phone, parent_phone, password, role, payment_status, payment_amount, payment_date)
        ...
        const updateResult = await pool.query(
          'UPDATE group_students SET student_id = $1 WHERE group_id = $2 AND student_id = $3 RETURNING *',
          [newStudentId, exam.group_id, replacement.wrong_id],
        );
