...
export type ExamReportKind = 'lecture' | 'course';
...
export function isExamAnalysisRequest(text: string): boolean {
...
export function classifyExamReportKind(text: string): ExamReportKind | null {
  ...
  if (isLecture && !isCourse) return 'lecture';
  if (isCourse && !isLecture) return 'course';

  if (isLecture && isCourse) {
    const n = normalizeText(text);
    const lectureIndex = Math.min(
      ...lectureHints
...
export function extractExamSearchQuery(text: string): string {
  let query = text.trim();
  const patterns = [
...
export class DataAnalystExamReportsService {
  static async findLectureExams(
    teacherId: number,
    query: string,
  ): Promise<ExamMatch[]> {
    const pattern = buildSearchPattern(query);
    const result = await pool.query(
      `SELECT e.id, e.title, e.type, l.title AS lecture_title, c.title AS course_title
       FROM exams e
    ...
    return result.rows.map((row) => ({
      ...
      examKind: 'lecture' as const,
      type: row.type,
    }));
  }

  static async findCourseExams(teacherId: number, query: string): Promise<ExamMatch[]> {
    const pattern = buildSearchPattern(query);
    const result = await pool.query(
    ...
    return result.rows.map((row) => ({
      ...
      title: row.title,
      courseTitle: row.course_title,
      examKind: 'course' as const,
    }));
  }

  static async getLectureExamAnalysis(teacherId: number, examId: number) {
    const exam = await ExamFlowService.getExamWithCourse(examId);
    if (!exam) throw new HttpError(404, 'الامتحان غير موجود');
    if (exam.teacher_id !== teacherId) {
      throw new HttpError(403, 'ليس لديك صلاحية على هذا الامتحان');
    }

    const questionReport = await ExamFlowService.getExamQuestionReport(examId, {
      id: teacherId,
      role: 'teacher',
    });
    ...
    const submissionRate =
      totalEnrolled > 0 ? Math.round((submitted.length / totalEnrolled) * 1000) / 10 : 0;
    ...
    const averageScore =
      submitted.length > 0
        ? Math.round(
            (submitted.reduce((sum, s) => sum + Number(s.totalGrade ?? 0), 0) / submitted.length) *
              10,
          ) / 10
        : 0;
    ...
    const metaRes = await pool.query(
      `SELECT c.title AS course_title, l.title AS lecture_title
       FROM lectures l
  ...
  static async getCourseExamAnalysis(teacherId: number, examId: number) {
    const requester = { id: teacherId, role: 'teacher' };
    const [detailReport, gradesReport] = await Promise.all([
      CourseLevelExamsService.getExamReport(examId, requester),
      CourseLevelExamsService.getExamGrades(examId, requester),
    ]);
