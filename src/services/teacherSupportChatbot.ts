...
import type {
  TeacherDailyCourseReport,
  WeakQuestion,
} from './teacherDailyCourseReport';
...
export const TEACHER_DAILY_GREETING = `صباح الخير 👋
...
export const TEACHER_QUICK_BUTTONS = [
...
export type TeacherChatIntent = 'problem' | 'report' | 'marketing' | 'greeting' | 'other';

/** رسالة من سياق الشات (للفهم أن المدرس يتابع طلب سابق) */
export type TeacherChatContextMessage = {
  role: 'teacher' | 'admin';
  text: string | null;
};
 ...
 */
export async function detectTeacherIntent(message: string): Promise<TeacherChatIntent> {
...
أو {"intent":"other"}`;
  ...
  try {
    ...
    if (['problem', 'report', 'marketing', 'other'].includes(intent)) {
      return intent as TeacherChatIntent;
    }
  } catch (_e) {
...
export interface TeacherChatbotResult {
  intent: TeacherChatIntent;
  reply: string;
 ...
 */
export function formatTeacherDailyReportForChat(
  report: TeacherDailyCourseReport,
  opts: { skipClosing?: boolean } = DEFAULT_FORMAT_OPTS,
): string {
  ...
  if (report.weak_questions && report.weak_questions.length > 0) {
    ...
    report.weak_questions.slice(0, 10).forEach((q: WeakQuestion, i: number) => {
 ...
 */
export function formatTeacherDailyReportsForChat(reports: TeacherDailyCourseReport[]): string {
  if (!reports.length) {
    return 'لا يوجد كورس لديك حالياً. عند إنشاء كورس والعمل عليه ستظهر هنا تقارير مفصلة.';
  }
 ...
 */
function parseStudentReportRequest(
  text: string,
  ...
  for (const prefix of STUDENT_REPORT_PREFIXES) {
    ...
    if (!Number.isNaN(num)) return { type: 'id', value: num };
...
function hasStudentReportIntentWithoutName(text: string): boolean {
  if (parseStudentReportRequest(text)) return false;
  const t = text.trim().replace(/\s+/g, ' ');
  for (const prefix of STUDENT_REPORT_PREFIXES) {
    const p = prefix.replace(/\s+/g, ' ').trim();
    const regex = new RegExp(p.replace(/\s+/g, '\\s+'), 'i');
...
function isGeneralReportRequest(text: string): boolean {
  if (parseStudentReportRequest(text)) return false;
  if (hasStudentReportIntentWithoutName(text)) return false;
  return containsAny(text, REPORT_KEYWORDS);
}

/** هل آخر رسالة من البوت تطلب كود الطالب (بعد وجود أكثر من طالب بنفس الاسم)؟ */
function isLastBotMessageAskingForStudentCode(messages: TeacherChatContextMessage[]): boolean {
  if (!messages.length) return false;
  const last = messages[messages.length - 1];
  if (last.role !== 'admin' || !last.text) return false;
...
function extractStudentIdFromFollowUp(text: string): number | null {
  const t = text.trim().replace(/\s+/g, ' ');
  if (/^\d+$/.test(t)) return parseInt(t, 10);
  const studentReportReq = parseStudentReportRequest(t);
  if (studentReportReq && studentReportReq.type === 'id') return studentReportReq.value;
  return null;
}
...
function formatStudentReportForChat(report: {
  ...
  courses: Array<{
    ...
    statistics: {
      ...
      averageGrade: number;
      totalObtainedGrade: number;
      totalMaxGrade: number;
      watch_percentage?: number;
    };
    lectureExams: unknown[];
    courseExams: unknown[];
  }>;
  overallStatistics: {
    totalCourses: number;
...
function getRandomMarketingTip(): string {
...
export async function handleTeacherMessage(
  text: string,
  teacherId: number,
  recentMessages?: TeacherChatContextMessage[],
): Promise<TeacherChatbotResult> {
  ...
  if (studentReportReq) {
    try {
      if (studentReportReq.type === 'id') {
        const report = await TeacherReportsService.getStudentDetailedReport(
          teacherId,
          studentReportReq.value,
        );
        return {
          intent: 'report',
          reply: '📋 **تقرير الطالب المطلوب:**\n\n' + formatStudentReportForChat(report),
      ...
      const result = await TeacherReportsService.getStudentReportByName(
        teacherId,
        studentReportReq.value,
      );
      if ('report' in result) {
  ...
  if (hasStudentReportIntentWithoutName(trimmed)) {
    return {
      intent: 'report',
      reply:
        'لتقرير **طالب معين** اكتب: **تقرير الطالب** ثم اسم الطالب أو كود الطالب (الرقم).\n\nمثال: تقرير الطالب أحمد — أو: تقرير الطالب 12345',
    };
  }
  ...
  let intent: TeacherChatIntent = 'other';
  try {
  ...
  return {
    intent: 'other',
    reply:
      'يمكنني مساعدتك في: تقرير مفصل عن مستوى طلابك، أفكار تسويقية أو تحسين التفاعل، أو تسجيل أي مشكلة للإدارة. اكتب ما تحتاجه بشكل مختصر.',
  };
}
