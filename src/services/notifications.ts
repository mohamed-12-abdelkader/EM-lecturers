...
export function setIOGetter(getter: () => SocketIOServer | null) {
...
export function emitLectureLockUpdated(studentId: number, courseId: number) {
  const io = getIO();
  if (io) {
...
function broadcastNotificationRemoved(userId: number, notificationId: number) {
  const io = getIO();
  if (io) {
...
export interface NotificationData {
  ...
  exam_id?: number;
  video_id?: number;
  /** معرف جلسة اللايف (لنوع live_stream_started) لفتح اللايف من الإشعار */
  meeting_id?: string;
...
export class NotificationService {
  ...
  static async notifyCourseStudents(
    ...
    try {
      ...
      const studentsResult = await pool.query(
        `SELECT user_id FROM enrollments
         WHERE course_id = $1
           ...
           AND (is_blocked_by_teacher IS NULL OR is_blocked_by_teacher = false)`,
        [courseId],
      );
      ...
      console.log(`📋 [Notification] Student IDs: ${studentsResult.rows.map((r: any) => r.user_id).join(', ')}`);
      ...
      for (const notification of notifications) {
        try {
          const insertResult = hasMetadata
            ? await pool.query(
                `INSERT INTO notifications (user_id, title, message, type, course_id, lecture_id, exam_id, metadata) 
            ...
            : await pool.query(
                `INSERT INTO notifications (user_id, title, message, type, course_id, lecture_id, exam_id) 
          ...
          if (insertedNotification.metadata) broadcastPayload.metadata = insertedNotification.metadata;
      ...
      if (notificationData.meeting_id) pushPayload.meeting_id = notificationData.meeting_id;
      try {
      ...
      ExpoPushService.sendPushNotificationToMany(
  ...
  static async sendNotification(
    ...
    senderId?: number,
    groupId?: number,
    generalCourseId?: number,
    description?: string,
  ) {
    try {
      const result = await pool.query(
        `INSERT INTO notifications (user_id, title, message, description, type, course_id, general_course_id, lecture_id, post_id, comment_id, sender_id, group_id) 
   ...
   */
  static async notifyUserAboutTask(
    userId: number,
    taskId: number,
    title: string,
    message: string,
    type: 'task_assigned' | 'task_deadline_reminder' | 'task_rejected' | 'task_approved',
    description?: string,
  ) {
    try {
      const result = await pool.query(
        `INSERT INTO notifications (user_id, title, message, description, type, task_id)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING *`,
        [userId, title, message, description || null, type, taskId],
      );
   ...
   */
  static async notifyTeacherCreativeReminder(userId: number) {
    const title = 'مساعد السوشيال ميديا جاهز لك';
    const message =
      'ابدأ يومك بفكرة منشور أو تصميم جديد لطلابك. افتح مساعد المدرسين الإبداعي واكتب المطلوب بالعربي.';
    ...
    try {
      ...
      ExpoPushService.sendPushNotification(userId, title, message, {
        ...
        console.error('❌ [Notification] Expo push error (teacher creative):', e),
      );
   ...
   */
  static async notifyLectureAdded(
    courseId: number,
    lectureId: number,
    lectureTitle: string,
    courseTitle: string,
   ...
   */
  static async notifySocialInteraction(
    userId: number,
    postId: number | null,
    commentId: number | null,
    actorName: string,
    content: string,
    actionType: 'comment' | 'reply' | 'like' | 'reaction',
    senderId?: number,
  ) {
    try {
      let title: string;
      ...
      const result = await pool.query(
        `INSERT INTO notifications (user_id, title, message, type, post_id, comment_id, sender_id) 
         VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
   ...
   */
  static async notifyGroupMessage(
    groupId: number,
    senderId: number,
    senderName: string,
    messageText: string,
    ...
    try {
      ...
      for (const member of membersResult.rows) {
        ...
        const existingNotification = await pool.query(
          `SELECT id, message, metadata FROM notifications 
        ...
        if (existingNotification.rowCount && existingNotification.rowCount > 0) {
          ...
          await pool.query(
            `UPDATE notifications 
             ...
             WHERE id = $4`,
            [updatedMessage, JSON.stringify(updatedMetadata), senderId, existing.id],
          );

          await sendPushNotification([member.user_id], 'رسالة جديدة في المجموعة', updatedMessage, {
            type: 'group_message',
            group_id: groupId,
            sender_id: senderId,
          });
          ExpoPushService.sendPushNotification(
            ...
            updatedMessage,
            { type: 'group_message', group_id: groupId, sender_id: senderId },
          ).catch((e) => console.error('❌ [Notification] Expo push error:', e));
          ...
          await pool.query(
            `INSERT INTO notifications (user_id, title, message, type, group_id, sender_id, metadata) 
   ...
   */
  static async getUserNotifications(userId: number, limit: number = 20, offset: number = 0, userRole?: string) {
    try {
      ...
      const regularNotifications = await pool.query(query, [userId]);
      ...
      if (userRole === 'student' || userRole === 'teacher') {
        try {
          ...
          chatNotifs = chatNotifications.items
            .filter((item: any) => (item.unread_count ?? 0) > 0) // فقط الجروبات اللي فيها رسائل غير مقروءة
            .map((item: any) => {
      ...
      const allNotifications = [...regularNotifs, ...chatNotifs].sort(
   ...
   */
  static async getLiveStreamNotifications(
    userId: number,
    limit: number = 20,
    offset: number = 0,
    userRole?: string,
    ...
    try {
      ...
      const notificationsRes = await pool.query(query, [userId, limit, offset]);
      ...
      const notifications = notificationsRes.rows.map((row) => ({
        ...
        metadata: row.metadata
          ...
          : null,
        meeting_id:
          row.metadata && typeof row.metadata === 'object' ? (row.metadata as any).meeting_id || null : null,
        is_read: row.is_read,
        created_at: row.created_at,
        course_title: row.course_title,
        general_course_title: row.general_course_title,
   ...
   */
  static async getMessageNotificationsUnified(
    userId: number,
    ...
    try {
      ...
      for (const item of chatNotifs.items) {
        ...
        if (lastMsg) {
          if (lastMsg.text) body = lastMsg.text.slice(0, 120) + (lastMsg.text.length > 120 ? '...' : '');
          else if (lastMsg.attachment_type === 'image') body = 'صورة';
          else if (lastMsg.attachment_type === 'audio') body = 'رسالة صوتية';
          else if (lastMsg.attachment_type === 'file') body = 'ملف';
        }
   ...
   */
  static async getStudentGradeFeed(userId: number, limit: number = 20, offset: number = 0) {
    try {
      const query = `
        (
          SELECT 
   ...
   */
  static async notifyEssayExamCreated(
    courseId: number,
    examTitle: string,
    lectureTitle: string,
    createdBy: number,
    ...
    try {
      ...
      for (const student of studentsResult.rows) {
        await NotificationService.sendNotification(
          student.user_id,
          'امتحان مقالي جديد',
          `تم إضافة امتحان مقالي جديد: ${examTitle} في محاضرة ${lectureTitle}`,
          'essay_exam_created',
          courseId,
          undefined,
          undefined,
   ...
   */
  static async notifyPackageStudents(
    packageId: number,
    notificationData: {
      title: string;
      message: string;
      type: 'package_lesson_added' | 'package_video_added' | 'package_assignment_added' | 'package_exam_added' | 'package_file_added';
      package_id?: number;
      subject_id?: number;
      lesson_id?: number;
      assignment_id?: number;
      ...
      for (const studentId of studentIds) {
        try {
          const insertResult = await pool.query(
            `INSERT INTO notifications (user_id, title, message, type, package_id, subject_id, lesson_id, assignment_id, exam_id, video_id) 
   ...
   */
  static async notifyPackageLessonAdded(
    packageId: number,
    subjectId: number,
    lessonId: number,
    lessonName: string,
    ...
    const notificationData = {
      ...
      type: 'package_lesson_added' as const,
   ...
   */
  static async notifyPackageVideoAdded(
    packageId: number,
    subjectId: number,
    lessonId: number,
    videoId: number,
    ...
    const notificationData = {
      ...
      type: 'package_video_added' as const,
      subject_id: subjectId,
      lesson_id: lessonId,
      video_id: videoId,
    };
   ...
   */
  static async notifyPackageAssignmentAdded(
    packageId: number,
    subjectId: number,
    lessonId: number,
    assignmentId: number,
    ...
    const notificationData = {
      ...
      type: 'package_assignment_added' as const,
      subject_id: subjectId,
      lesson_id: lessonId,
   ...
   */
  static async notifyPackageExamAdded(
    packageId: number,
    subjectId: number,
    examId: number,
    examName: string,
    ...
    const notificationData = {
      ...
      type: 'package_exam_added' as const,
      subject_id: subjectId,
      exam_id: examId,
    };
   ...
   */
  static async notifyPackageFileAdded(
    packageId: number,
    subjectId: number,
    fileName: string,
    subjectName: string,
    ...
    const notificationData = {
      ...
      type: 'package_file_added' as const,
      subject_id: subjectId,
   ...
   */
  static async notifyPackageExamUpdated(
    packageId: number,
    subjectId: number,
    examId: number,
    examName: string,
    ...
    const notificationData = {
      ...
      type: 'package_exam_added' as const, // Using same type for now
      subject_id: subjectId,
      exam_id: examId,
    };
   ...
   */
  static async notifyGeneralCourseLectureAdded(
    generalCourseId: number,
    lectureId: number,
    lectureTitle: string,
  ) {
    try {
      ...
      const studentsResult = await pool.query(
        'SELECT student_id FROM general_course_enrollments WHERE general_course_id = $1',
        [generalCourseId],
      );
      ...
      // جلب معلومات الكورس
      const courseResult = await pool.query('SELECT title FROM general_courses WHERE id = $1', [generalCourseId]);
      ...
      for (const student of studentsResult.rows) {
        await this.sendNotification(
          ...
          undefined, // sender_id
          undefined, // group_id
          generalCourseId, // general_course_id
        );
      }

      const studentIds = studentsResult.rows.map((row) => row.student_id);
      await sendPushNotification(studentIds, notificationData.title, notificationData.message, {
        type: notificationData.type,
        general_course_id: generalCourseId,
        lecture_id: lectureId,
      });
      ExpoPushService.sendPushNotificationToMany(
        ...
        notificationData.title,
        notificationData.message,
        { type: notificationData.type, general_course_id: generalCourseId, lecture_id: lectureId },
      ).catch((e) => console.error('❌ [Notification] Expo push error:', e));
   ...
   */
  static async notifyGeneralCourseVideoAdded(
    generalCourseId: number,
    lectureId: number,
   ...
   */
  static async notifyGeneralCourseExamAdded(
    generalCourseId: number,
    examId: number,
    examTitle: string,
    lectureTitle?: string,
   ...
   */
  static async notifyGeneralCourseExamUpdated(
    generalCourseId: number,
    examId: number,
    examTitle: string,
    lectureTitle?: string,
   ...
   */
  static async notifyExamUpdated(
    courseId: number,
    lectureId: number | undefined,
    examId: number,
    examTitle: string,
   ...
   */
  static async notifyDirectMessage(
    recipientId: number,
    senderId: number,
    senderName: string,
    messageText: string,
   ...
   */
  static async notifyLiveStreamStarted(
    courseId: number,
    meetingTitle: string,
    courseTitle: string,
   ...
   */
  static async removeLiveStreamNotificationsByMeetingId(meetingId: string) {
    try {
      const deleted = await pool.query<{ id: number; user_id: number }>(
   ...
   */
  static async notifyGeneralCourseGroupLiveStreamStarted(
    groupId: number,
    generalCourseId: number,
    meetingTitle: string,
    courseTitle: string,
    ...
    try {
      const studentsResult = await pool.query(
        'SELECT student_id FROM general_course_enrollments WHERE group_id = $1',
        [groupId],
      );
      if (studentsResult.rowCount === 0) return { success: true, notifiedCount: 0 };
      ...
      for (const row of studentsResult.rows) {
        const userId = (row as { student_id: number }).student_id;
        try {
          await this.sendNotification(
            userId,
            title,
