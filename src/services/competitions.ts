import {
  ...
  CompetitionCreate,
  CompetitionUpdate,
  SolveCompetitionRequest,
  SolveCompetitionResponse,
...
export class CompetitionsService {
  // إنشاء مسابقة جديدة
  static async create(competition: CompetitionCreate, createdBy: number): Promise<Competition> {
    const { title, description, image_url, duration, grade_id, is_visible, is_active } =
      competition;
    ...
    const result = await pool.query<Competition>(query, values);
  ...
  static async getVisible(): Promise<Competition[]> {
    const query = `
  ...
  static async getById(id: number): Promise<Competition | null> {
    const query = `
      SELECT c.*, g.name as grade_name, u.name as creator_name
      FROM competitions c
      LEFT JOIN grades g ON c.grade_id = g.id
    ...
    const result = await pool.query<Competition>(query, [id]);
    return result.rows[0] || null;
  }

  // الحصول على مسابقات صف دراسي معين
  static async getByGrade(gradeId: number): Promise<Competition[]> {
    const query = `
      SELECT c.*, g.name as grade_name
      FROM competitions c
      LEFT JOIN grades g ON c.grade_id = g.id
  ...
  // تحديث مسابقة
  static async update(id: number, competition: CompetitionUpdate): Promise<Competition | null> {
    const fields = Object.keys(competition).filter(
      (key) => competition[key as keyof CompetitionUpdate] !== undefined,
    );
    ...
    const values = [id, ...fields.map((field) => competition[field as keyof CompetitionUpdate])];
  ...
  // تغيير حالة الرؤية
  static async toggleVisibility(id: number): Promise<Competition | null> {
    const query = `
      UPDATE competitions 
  ...
  // تغيير حالة النشاط
  static async toggleActive(id: number): Promise<Competition | null> {
    const query = `
      UPDATE competitions 
  ...
  static async getStudentCompetitions(studentId: number): Promise<Competition[]> {
    ...
    const competitionsCountResult = await pool.query(competitionsCountQuery);
    console.log('Total competitions in database:', competitionsCountResult.rows[0].count);

    // التحقق من وجود مسابقات مرئية ونشطة
    const visibleActiveCountQuery =
      'SELECT COUNT(*) as count FROM competitions WHERE is_visible = true AND is_active = true';
    const visibleActiveCountResult = await pool.query(visibleActiveCountQuery);
    ...
    const userGradesResult = await pool.query(userGradesQuery, [studentId]);
  ...
  static async isStudentEnrolled(competitionId: number, studentId: number): Promise<boolean> {
    const query = `
      SELECT 1 FROM competition_students 
      WHERE competition_id = $1 AND student_id = $2 AND is_active = true
    `;

    const result = await pool.query(query, [competitionId, studentId]);
    return (result.rowCount ?? 0) > 0;
  }

  // اشتراك الطالب في مسابقة
  static async enrollStudent(competitionId: number, studentId: number): Promise<boolean> {
  ...
  static async getStudentCompetitionDetails(
    competitionId: number,
    studentId: number,
  ): Promise<StudentCompetitionDetails> {
    try {
      ...
      const resultResult = await pool.query(resultQuery, [competitionId, studentId]);
      ...
      if (hasSolved) {
        ...
        const questionsResult = await pool.query(questionsQuery, [competitionId]);
  ...
  static async getStudentCompetitionsNew(studentId: number): Promise<Competition[]> {
    try {
      ...
      const competitionsResult = await pool.query(competitionsQuery, [studentId]);
  ...
  static async getStudentCompetitionsSimple(studentId: number): Promise<Competition[]> {
    try {
  ...
  static async solveCompetition(
    competitionId: number,
    studentId: number,
    answers: SolveCompetitionRequest['answers'],
  ): Promise<SolveCompetitionResponse> {
    ...
    try {
      ...
      const existingResult = await client.query(
        'SELECT id FROM competition_results WHERE competition_id = $1 AND student_id = $2',
  ...
  static async getStudentResult(competitionId: number, studentId: number): Promise<any> {
    try {
      ...
      const result = await pool.query(resultQuery, [competitionId, studentId]);
      ...
      const answersResult = await pool.query(answersQuery, [competitionResult.id]);
  ...
  static async getCompetitionLeaderboard(
    ...
    try {
      ...
      const totalResult = await pool.query(totalQuery, [competitionId]);
      ...
      const leaderboardResult = await pool.query(leaderboardQuery, [competitionId, limit, offset]);
      ...
      const competitionResult = await pool.query(competitionQuery, [competitionId]);
  ...
  static async getCompetitionLeaderboardForAdmin(
    competitionId: number,
    ...
    try {
      ...
      const leaderboardResult = await pool.query(leaderboardQuery, params);
  ...
  static async getGlobalLeaderboard(
    ...
    try {
      ...
      const totalResult = await pool.query(totalQuery, gradeId ? [gradeId] : []);
      const total = parseInt(totalResult.rows[0].total);
