...
export async function findStudentByPhoneAndName(
  phone: string,
  name: string,
): Promise<{ id: number; name: string; phone: string } | null> {
  const phoneDigits = normalizePhoneForLookup(phone);
...
export class DeepSeekChatbotService {
  public static readonly MAX_BOT_ATTEMPTS = 10;
  private static readonly API_URL = `${config.DEEPSEEK_API_URL}/v1/chat/completions`;
...
Be concise and accurate.`;
    ...
    try {
      ...
      try {
        ...
        result = JSON.parse(jsonContent) as IntentDetectionResult;
      } catch (_parseError) {
  ...
  static async generateResponse(
    ...
    if (escalationKeywords.some((keyword) => messageLower.includes(keyword.toLowerCase()))) {
      return {
        message:
          'أفهم أن الحل المقترح لم يعمل. سأقوم بنقل المحادثة إلى فريق الدعم الفني. سيقوم أحد المسؤولين بالرد عليك قريباً.',
        intent,
        solved: false,
        shouldEscalate: true,
    ...
    const looksLikePasswordProblem =
    ...
    if (
      (intent === 'LOGIN_PROBLEM' || intent === 'PASSWORD_RESET' || intent === 'OTHER') &&
    ...
    if (
      (intent === 'PASSWORD_RESET' || intent === 'LOGIN_PROBLEM') &&
      botAskedForPhone &&
    ...
    if (intent === 'ACTIVATION_CODE') {
      ...
      if (askedToActivateNow) {
        ...
        const isAgreement =
          positiveWords.some((w) => msg === w || msg.startsWith(w + ' ') || msg.includes(' ' + w)) ||
        ...
        if (isPositive) {
          ...
          if (codeToUse) {
            const result = await activateCourseByCodeForStudent(context.studentId, codeToUse);
            if (result.success) {
              return {
            ...
            const friendlyMessage =
              errorMessages[result.message] || `لم نتمكن من التفعيل: ${result.message}`;
            return {
      ...
      if (askedIfSameStudent) {
        ...
        if (isPositive) {
          return {
            message:
              'يبدو أنك تحاول تفعيل الكود بحساب آخر. الكود صالح لحساب واحد فقط. قم بتسجيل الخروج والدخول بالحساب الأول. لو ناسي الباسورد في صفحة تسجيل الدخول اضغط "نسيت كلمة السر" واتبع الخطوات. لو معرفتش ابعت للرقم 01111272393 وقول إنك محتاج تغير كلمة السر.',
            intent: 'ACTIVATION_CODE',
            solved: false,
            shouldEscalate: false,
          };
        ...
        if (isNegative) {
          return {
            message:
              'يبدو أن الكود مستخدم من قبل. ارجع للمدرس بتاعك واطلب كود جديد وقول للمدرس إن الكود ده طالب تاني استخدمه.',
            intent: 'ACTIVATION_CODE',
            solved: false,
            shouldEscalate: false,
          };
      ...
      const lastBotWasCodeUsedOrIsThisYou =
        lastBotMessage &&
      ...
      const normMsg = message
        ...
        .replace(/[أإآ]/g, 'ا')
        .replace(/ة/g, 'ه');
      const userSaysNewCodeOrError =
        lastBotWasCodeUsedOrIsThisYou &&
        newCodeOrErrorKeywords.some((k) => normMsg.includes(k.replace(/ة/g, 'ه').toLowerCase()));
      if (userSaysNewCodeOrError) {
        return {
          message:
            'تمام، نتعامل مع الكود الجديد أو المشكلة الجديدة. اكتب الكود الجديد (8 أرقام) أو اذكر لي رسالة الخطأ اللي بتظهرلك وسأساعدك.',
          intent: 'ACTIVATION_CODE',
          solved: false,
          shouldEscalate: false,
        };
      ...
      const normalizeForSub = (s: string) =>
        s
          .trim()
          .toLowerCase()
          .replace(/\s+/g, ' ')
      ...
      const isSubscriptionRequest =
        subscriptionKeywords.some((k) => msgForSub.includes(normalizeForSub(k))) &&
        message.length >= 10 &&
        !message.replace(/\s/g, '').match(/^\d{8}$/); // ليس مجرد كود فقط
      const notWaitingForReply =
        !lastBotMessage ||
        (!lastBotMessage.includes('هل هذا حسابك') &&
      ...
      if (code) {
        ...
        if (details) {
          ...
          if (details.is_expired) {
            ...
            const usedByName =
              details.used_by.length > 0 ? details.used_by.map((u) => u.name).join(' أو ') : 'طالب';
...
Generate a short, friendly response in Arabic that addresses the student and helps solve the problem.`;
      ...
      const data = (await response.json()) as any;
      let botMessage =
        data.choices[0]?.message?.content?.trim() || responseTemplate.defaultMessage;
   ...
   */
  private static getResponseTemplate(
    intent: IntentType,
...
يرجى وصف مشكلتك أو سؤالك بالتفصيل، وسأبذل قصارى جهدي لمساعدتك.`,
   ...
   */
  static async checkIfSolved(studentResponse: string): Promise<boolean> {
   ...
   */
  static isPositiveConfirmationForClosing(message: string): boolean {
    const normalize = (s: string) =>
      s
        .trim()
        .toLowerCase()
   ...
   */
  static async generateClosingResponse(studentMessage: string): Promise<string> {
    try {
...
Generate a similar response based on the student's message.`;
   ...
   */
  static async getChatContext(chatId: number, studentId: number): Promise<ChatContext> {
    // Get recent messages
    ...
    const messages = messagesResult.rows.reverse().map((row) => ({
      role: (row.is_auto_reply ? 'bot' : row.sender_role === 'student' ? 'student' : 'admin') as
        | 'student'
        | 'admin'
        | 'bot',
      text: row.text || '',
    ...
    const chatResult = await pool.query(`SELECT current_intent FROM support_chats WHERE id = $1`, [
      chatId,
    ]);
    ...
    return {
      ...
      messages,
      botAttempts,
      currentIntent: chatResult.rows[0]?.current_intent as IntentType | undefined,
    };
  }
}
