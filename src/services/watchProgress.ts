 ...
 */
export async function syncLectureWatchCompletion(
  userId: number,
  lectureId: number,
): Promise<number> {
  const counts = await pool.query<{ total: string; watched: string }>(
    `SELECT
       (SELECT COUNT(*)::text FROM lecture_videos WHERE lecture_id = $2) AS total,
       (SELECT COUNT(DISTINCT vv.video_id)::text
        FROM video_views vv
        WHERE vv.user_id = $1 AND vv.lecture_id = $2) AS watched`,
    [userId, lectureId],
  );
  ...
  await pool.query(
    ...
    [userId, lectureId, percentage, isLectureCompleted],
  );
...
export async function getStudentCourseWatchPercentage(
  userId: number,
  courseId: number,
): Promise<number> {
  const result = await pool.query<{ watch_percentage: string }>(
    `SELECT
...
export async function getCourseAverageWatchPercentage(courseId: number): Promise<number> {
  const result = await pool.query<{ avg_watch: string }>(
    `WITH enrolled AS (
       SELECT e.user_id
       FROM enrollments e
       WHERE e.course_id = $1
...
export async function getStudentLectureWatchPercentage(
  userId: number,
  lectureId: number,
): Promise<number> {
  const result = await pool.query<{ watch_percentage: string }>(
