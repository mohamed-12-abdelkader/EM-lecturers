...
export interface TeacherSubjectPermissions {
  can_edit: boolean;
  can_delete: boolean;
...
export class TeacherSubjectService {
  ...
  static async assignSubjectToTeacher(
    teacherId: number,
    subjectId: number,
    permissions: TeacherSubjectPermissions,
    assignedBy: number,
  ) {
  ...
  static async removeSubjectFromTeacher(teacherId: number, subjectId: number) {
    const result = await pool.query(
  ...
  static async getTeacherSubjects(teacherId: number) {
    const result = await pool.query(
      `SELECT ts.*, s.name as subject_name, s.description as subject_description, s.image as subject_image,
  ...
  static async checkTeacherPermission(
    teacherId: number,
    subjectId: number,
    permission: keyof TeacherSubjectPermissions,
  ) {
    const result = await pool.query(
      `SELECT ${permission} FROM teacher_subjects 
  ...
  static async updateTeacherPermissions(
    teacherId: number,
    subjectId: number,
    permissions: Partial<TeacherSubjectPermissions>,
    updatedBy: number,
  ) {
    const updateFields = [];
    const values = [];
  ...
  static async getTeacherSubjectStats(teacherId: number) {
    const result = await pool.query(
      `SELECT 
  ...
  static async teacherExists(teacherId: number) {
    const result = await pool.query('SELECT id FROM users WHERE id = $1 AND role = $2', [
      teacherId,
      'teacher',
    ]);
  ...
  static async getTeacherSubjectsWithContent(teacherId: number) {
    const result = await pool.query(
      `SELECT
