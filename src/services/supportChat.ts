...
export interface SupportChat {
  ...
  status: 'open' | 'closed' | 'resolved' | 'bot_handling' | 'waiting_for_admin' | 'admin_handling';
...
export interface TeacherSupportChat {
  id: number;
  teacher_id: number;
  admin_id: number | null;
  status: string;
...
export class SupportChatService {
  ...
  static async getOrCreateStudentChat(studentId: number): Promise<SupportChat> {
    let chat: SupportChat;
    const existing = await pool.query<SupportChat>(
    ...
    if (existing.rowCount && existing.rows[0]) {
      ...
      const insertResult = await pool.query<SupportChat>(
        `INSERT INTO support_chats (student_id, status)
         VALUES ($1, 'bot_handling')
         RETURNING *`,
        [studentId],
      );
      if (!insertResult.rowCount || !insertResult.rows[0]) {
        const retry = await pool.query<SupportChat>(
          `SELECT * FROM support_chats WHERE student_id = $1 LIMIT 1`,
          [studentId],
        );
    ...
    const studentResult = await pool.query(`SELECT name, email FROM users WHERE id = $1`, [
      studentId,
    ]);
  ...
  static async getOrCreateGuestChat(guestToken?: string | null): Promise<SupportChat & { guest_token: string }> {
    const token = (guestToken || '').trim();
    if (token) {
      const found = await pool.query<SupportChat>(
        `SELECT * FROM support_chats WHERE guest_token = $1`,
        [token],
      );
      if (found.rowCount && found.rows[0]) {
        return { ...found.rows[0], guest_token: token } as SupportChat & { guest_token: string };
      }
    }
    const newToken = randomUUID();
    const result = await pool.query<SupportChat>(
      `INSERT INTO support_chats (guest_token, student_id, status)
       VALUES ($1, NULL, 'bot_handling')
       RETURNING *`,
      [newToken],
    );
    const chat = result.rows[0];
    return { ...chat, guest_token: newToken } as SupportChat & { guest_token: string };
  }

  /** جلب شات الضيف بالـ token (للتحقق) */
  static async getChatByGuestToken(guestToken: string): Promise<(SupportChat & { guest_token: string }) | null> {
    const r = await pool.query<SupportChat>(
      `SELECT * FROM support_chats WHERE guest_token = $1`,
      [guestToken],
    );
    if (!r.rowCount || !r.rows[0]) return null;
    return { ...r.rows[0], guest_token: guestToken } as SupportChat & { guest_token: string };
  }
  ...
  static async getChatMessages(
    ...
    return result.rows
      ...
      .reverse() as any; // عكس الترتيب لعرض الأقدم أولاً
  }
  ...
  static async saveMessage(
    ...
    if (is_auto_reply) {
      ...
      const userResult = await pool.query(`SELECT name FROM users WHERE id = $1`, [senderId]);
  ...
  static async updateChatStatus(
    ...
    await pool.query(`UPDATE support_chats SET status = $1 WHERE id = $2`, [status, chatId]);
  }

  // تحديث معلومات البوت (intent, attempts, escalation)
  static async updateChatBotInfo(
    chatId: number,
    data: {
      current_intent?: string;
      bot_attempts?: number;
    ...
    params.push(chatId);
    await pool.query(
      `UPDATE support_chats SET ${updates.join(', ')} WHERE id = $${paramIndex}`,
      params,
    );
  }

  // تصعيد الشات إلى الأدمن
  static async escalateChat(chatId: number, reason: string): Promise<void> {
    await pool.query(
  ...
  // تعيين أدمن للشات
  static async assignAdmin(chatId: number, adminId: number): Promise<void> {
    await pool.query(`UPDATE support_chats SET admin_id = $1 WHERE id = $2`, [adminId, chatId]);
  }

  // تحديث حالة الرسالة (delivered/read)
  static async updateMessageStatus(messageId: number, status: 'delivered' | 'read'): Promise<void> {
    if (status === 'delivered') {
      await pool.query(
        `UPDATE support_messages SET delivered_at = NOW() WHERE id = $1 AND delivered_at IS NULL`,
        [messageId],
      );
    } else if (status === 'read') {
      await pool.query(
        `UPDATE support_messages SET read_at = NOW() WHERE id = $1 AND read_at IS NULL`,
        [messageId],
      );
    }
  }
  ...
  static async markChatAsRead(chatId: number, _readerId: number): Promise<void> {
    await pool.query(
  ...
  static async findMatchingFAQ(question: string): Promise<SupportFAQ | null> {
    const result = await pool.query<SupportFAQ>(
  ...
  // جلب جميع FAQs
  static async getAllFAQs(activeOnly: boolean = false): Promise<SupportFAQ[]> {
    ...
    const result = await pool.query<SupportFAQ>(query, params);
    return result.rows;
  }
  ...
  static async createFAQ(
    ...
    const result = await pool.query<SupportFAQ>(
      `INSERT INTO support_faq (question, answer, keywords, priority, created_by)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`,
      [question, answer, keywords, priority, createdBy],
    );
  ...
  static async getChatById(chatId: number): Promise<SupportChat | null> {
    const result = await pool.query<SupportChat>(
      `SELECT 
        sc.*,
        u.name AS student_name,
    ...
    return {
      ...
      student_email: row.student_email,
      current_intent: row.current_intent,
      bot_attempts: row.bot_attempts ? parseInt(row.bot_attempts as any) : 0,
      escalation_reason: row.escalation_reason,
      escalated_at: row.escalated_at,
    };
  }
  ...
  static async getMessageNotifications(
    ...
    const result = await pool.query(query, params);
    const countResult = await pool.query(countQuery, userRole === 'student' ? [userId] : []);
  ...
  static async getStudentChatId(studentId: number): Promise<number | null> {
    const row = await pool.query(
      `SELECT id FROM support_chats WHERE student_id = $1 LIMIT 1`,
      [studentId],
    );
    if (!row.rowCount || !row.rows[0]) return null;
    return (row.rows[0] as any).id;
  }
   ...
   */
  static async getStudentLatestUnreadNotification(studentId: number): Promise<{
    message_id: number;
    chat_id: number;
    sender_id: number | null;
    sender_role: string;
    ...
    title: string;
    body: string;
    data: { type: string; chat_id: number; message_id: number; sender_id?: number };
  } | null> {
    const chatRow = await pool.query(
      `SELECT id FROM support_chats WHERE student_id = $1 LIMIT 1`,
      [studentId],
    );
    if (!chatRow.rowCount || !chatRow.rows[0]) return null;
    const chatId = (chatRow.rows[0] as any).id;
    const row = await pool.query(
      `SELECT 
        sm.id AS message_id,
        sm.chat_id,
  ...
  static async getOrCreateTeacherChat(teacherId: number): Promise<TeacherSupportChat> {
    const result = await pool.query(
      `INSERT INTO teacher_support_chats (teacher_id, status)
       VALUES ($1, 'bot_handling')
    ...
    const chat = result.rows[0];
    const userRes = await pool.query(`SELECT name, email FROM users WHERE id = $1`, [teacherId]);
    if (userRes.rows[0]) {
      return {
        ...chat,
  ...
  static async getTeacherChatById(chatId: number): Promise<TeacherSupportChat | null> {
    const result = await pool.query(
  ...
  static async getTeacherChatByTeacherId(teacherId: number): Promise<TeacherSupportChat | null> {
    const result = await pool.query(
      `SELECT tsc.*, u.name AS teacher_name, u.email AS teacher_email
       FROM teacher_support_chats tsc
       JOIN users u ON u.id = tsc.teacher_id
  ...
  static async getTeacherChatMessages(
    chatId: number,
    limit: number = 50,
    before?: string,
  ): Promise<TeacherSupportMessage[]> {
    let query = `
  ...
  static async getTeacherSupportNotifications(
    teacherId: number,
    limit: number = 20,
    offset: number = 0,
    ...
    const result = await pool.query(query, params);
    const countResult = await pool.query(countQuery, unreadOnly ? [chat.id] : [chat.id]);
   ...
   * يرجع null إذا لا يوجد إشعار غير مقروء.
   */
  static async getTeacherLatestUnreadNotification(teacherId: number): Promise<{
    message_id: number;
    chat_id: number;
    sender_id: number | null;
    sender_role: string;
  ...
  static async markTeacherChatAsRead(chatId: number): Promise<void> {
    await pool.query(
      `UPDATE teacher_support_messages 
       SET read_at = NOW() 
       WHERE chat_id = $1 AND sender_role = 'admin' AND read_at IS NULL`,
      [chatId],
    );
  }

  static async saveTeacherMessage(
    chatId: number,
    senderId: number,
    ...
    const result = await pool.query(
      `INSERT INTO teacher_support_messages (
        chat_id, sender_id, sender_role, message_type, text,
    ...
    await pool.query(
      `UPDATE teacher_support_chats SET last_message_at = NOW(), updated_at = NOW() WHERE id = $1`,
      [chatId],
    );
    const msg = result.rows[0];
    let senderName = 'مستخدم';
    if (is_auto_reply) senderName = 'رد تلقائي';
    else {
      const u = await pool.query(`SELECT name FROM users WHERE id = $1`, [senderId]);
      senderName = u.rows[0]?.name || 'مستخدم';
    }
    return { ...msg, sender_name: senderName };
  }

  static async updateTeacherChatStatus(
    chatId: number,
    status: 'open' | 'closed' | 'resolved' | 'bot_handling' | 'waiting_for_admin' | 'admin_handling',
  ): Promise<void> {
    await pool.query(
      `UPDATE teacher_support_chats SET status = $1, updated_at = NOW() WHERE id = $2`,
      [status, chatId],
    );
  }

  static async escalateTeacherChat(chatId: number, reason: string): Promise<void> {
    await pool.query(
      `UPDATE teacher_support_chats
  ...
  static async createSupportTicket(
    teacherSupportChatId: number,
    teacherId: number,
    messageText: string,
  ): Promise<number> {
    const result = await pool.query(
      `INSERT INTO support_tickets (teacher_support_chat_id, teacher_id, message_text, status)
       VALUES ($1, $2, $3, 'open')
       RETURNING id`,
      [teacherSupportChatId, teacherId, messageText],
    );
  ...
  static async getSupportTicketById(ticketId: number): Promise<any | null> {
    const result = await pool.query(
      `SELECT st.id, st.teacher_support_chat_id AS chat_id, st.teacher_id,
              st.message_text, st.status, st.admin_notes, st.created_at, st.updated_at,
              u.name AS teacher_name, u.email AS teacher_email
  ...
  static async updateSupportTicket(
    ...
    if (updates.length === 0) return this.getSupportTicketById(ticketId);
    updates.push(`updated_at = NOW()`);
  ...
  static async getAllTeacherChats(
    limit: number = 50,
    offset: number = 0,
    status?: string,
  ): Promise<{ chats: TeacherSupportChat[]; total: number }> {
    let query = `
      SELECT tsc.*, u.name AS teacher_name, u.email AS teacher_email
      FROM teacher_support_chats tsc
      JOIN users u ON u.id = tsc.teacher_id
