...
export type ChatGroup = {
  ...
  package_subject_group_id?: number | null;
  direct_student_id?: number | null;
  direct_teacher_id?: number | null;
  allow_student_send: boolean;
  created_at: string;
};
...
export type ChatMessage = {
  ...
  attachment_type?: 'image' | 'file' | 'audio' | null;
  ...
  reply_preview?: ChatMessage['reply'];
};

export class ChatService {
  static async getOrCreateTeacherGradeGroup(
    gradeId: number,
    teacherId: number,
  ): Promise<ChatGroup> {
    const res = await pool.query<ChatGroup>(
      `INSERT INTO chat_groups (grade_id, owner_teacher_id, name)
  ...
  static async getOrCreatePackageSubjectGroupChat(packageSubjectGroupId: number): Promise<ChatGroup> {
    ...
    if (!meta.rowCount) throw new Error('Package subject group not found');
    const row = meta.rows[0] as any;
    const gradeId = Number(row.grade_id);
    const name = `${row.subject_name} - ${row.group_name}`;

    const res = await pool.query<ChatGroup>(
      `INSERT INTO chat_groups (grade_id, owner_teacher_id, name, package_subject_group_id, allow_student_send)
       -- IMPORTANT:
       ...
       DO UPDATE SET
         ...
         name = EXCLUDED.name
       RETURNING *`,
      [gradeId, name, packageSubjectGroupId],
    );
    return res.rows[0];
  }

  static async listDirectTeachersForStudent(studentId: number) {
    ...
    const res = await pool.query(
      `WITH course_teachers AS (
       ...
       ORDER BY COALESCE(lm.last_message_time, '1970-01-01'::timestamp) DESC, lm.name`,
      [studentId],
    );
    return res.rows;
  }

  static async studentCanChatWithTeacher(studentId: number, teacherId: number): Promise<boolean> {
    const res = await pool.query(
      `SELECT 1
       FROM (
         SELECT 1
  ...
  static async teacherCanChatWithStudent(teacherId: number, studentId: number): Promise<boolean> {
    // Eligible if:
  ...
  static async listDirectStudentsForTeacher(teacherId: number) {
  ...
  static async getOrCreateDirectChat(studentId: number, teacherId: number): Promise<ChatGroup> {
    ...
    const gradeRes = await pool.query(
      `SELECT grade_id FROM user_grades WHERE user_id = $1 ORDER BY grade_id LIMIT 1`,
      [studentId],
    );
    let gradeId: number | null = gradeRes.rowCount ? Number(gradeRes.rows[0].grade_id) : null;
    if (!gradeId) {
      const tGrade = await pool.query(`SELECT grade_id FROM teacher_grades WHERE teacher_id = $1 ORDER BY grade_id LIMIT 1`, [teacherId]);
      gradeId = tGrade.rowCount ? Number(tGrade.rows[0].grade_id) : null;
    }
    if (!gradeId) {
      const any = await pool.query(`SELECT id FROM grades ORDER BY id LIMIT 1`);
      gradeId = any.rowCount ? Number(any.rows[0].id) : 1;
    }

    const res = await pool.query<ChatGroup>(
      `INSERT INTO chat_groups (grade_id, owner_teacher_id, name, allow_student_send, direct_student_id, direct_teacher_id)
       VALUES ($1, NULL, $2, TRUE, $3, $4)
       ON CONFLICT (direct_student_id, direct_teacher_id)
       DO UPDATE SET name = EXCLUDED.name
       RETURNING *`,
      [gradeId, `Direct Chat`, studentId, teacherId],
    );
  ...
  static async addMember(groupId: number, userId: number, role: 'student' | 'teacher' = 'student') {
    await pool.query(
  ...
  static async removeMember(groupId: number, userId: number) {
    await pool.query(`DELETE FROM chat_group_members WHERE group_id = $1 AND user_id = $2`, [
      groupId,
      userId,
    ]);
  }

  static async listMembers(groupId: number) {
    const res = await pool.query(
      `SELECT u.id, u.name, u.role, cgm.joined_at
  ...
  static async setStudentPermission(groupId: number, allow: boolean) {
    await pool.query(`UPDATE chat_groups SET allow_student_send = $1 WHERE id = $2`, [
      allow,
      groupId,
    ]);
  }

  static async canStudentSend(groupId: number): Promise<boolean> {
    const res = await pool.query(`SELECT allow_student_send FROM chat_groups WHERE id = $1`, [
      groupId,
    ]);
  ...
  static async saveMessage(
    ...
    const res = await pool.query<ChatMessage>(
      `INSERT INTO chat_messages (group_id, sender_id, text, reply_to_message_id)
       VALUES ($1, $2, $3, $4)
       RETURNING *`,
      [groupId, senderId, text, replyTo ?? null],
    );
    return res.rows[0];
  }

  static async saveAttachmentMessage(
    groupId: number,
    senderId: number,
    attachment: {
      url: string;
    ...
    const res = await pool.query<ChatMessage>(
      `INSERT INTO chat_messages (group_id, sender_id, text, attachment_url, attachment_type, attachment_name, attachment_mime, attachment_size, attachment_duration_ms)
  ...
  static async getHistory(groupId: number, limit = 50, before?: string) {
  ...
  static async getGroupsForStudent(userId: number): Promise<ChatGroup[]> {
    const res = await pool.query<ChatGroup>(
      `SELECT DISTINCT cg.*
       FROM enrollments e
       JOIN courses c ON c.id = e.course_id
       ...
       WHERE e.user_id = $1
       ORDER BY cg.grade_id, cg.owner_teacher_id`,
      [userId],
    );
    return res.rows;
  }

  static async ensureStudentMembershipForEnrollments(userId: number) {
  ...
  static async getTeacherContacts(studentId: number) {
  ...
  static async getStudentContacts(teacherId: number) {
    ...
    const res = await pool.query(
      `SELECT DISTINCT u.id, u.name, u.role, u.avatar
         ...
         JOIN users u ON u.id = cgm.user_id
         WHERE cg.owner_teacher_id = $1 AND cgm.role = 'student'`,
      [teacherId]
    );
    return res.rows;
  }

  static async ensureStudentMembershipForPackageSubjectGroups(studentId: number) {
    ...
    for (const r of groupsRes.rows as any[]) {
      ...
      const teacherRes = await pool.query(`SELECT teacher_id FROM package_subject_item_groups WHERE id = $1`, [packageGroupId]);
      const teacherId = teacherRes.rowCount ? (teacherRes.rows[0].teacher_id as number | null) : null;
  ...
  static async getChatNotifications(params: {
    ...
    if (params.role === 'student') {
      ...
      const totalRes = await pool.query(
        ...
        FROM user_groups ug
        ${unreadOnly ? 'LEFT JOIN unread_counts uc ON uc.chat_group_id = ug.chat_group_id WHERE COALESCE(uc.unread_count, 0) > 0' : ''}`,
        [params.userId],
      );
      ...
      return {
        items: res.rows.map((row: any) => ({
          ...
          last_message:
            row.unread_count > 0 && row.last_message_id
              ? {
