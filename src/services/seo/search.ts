...
import { tenantCourseUrl } from './urls';
...
function cacheKey(prefix: string, filters: SearchFilters): string {
...
async function logSearch(
  ...
  if (!query) return;
  await pool.query(
    `INSERT INTO seo_search_logs (tenant_id, query, filters, result_count)
     VALUES ($1, $2, $3::jsonb, $4)`,
    [tenantId, query, JSON.stringify(filters), resultCount],
  );
}

function rankTeacher(row: Record<string, unknown>, query: string): number {
  const name = String(row.display_name ?? '').toLowerCase();
  const q = query.toLowerCase();
  let score = Number(row.fts_rank ?? 0) * 10;
  if (q && name === q) score += 100;
  else if (q && name.startsWith(q)) score += 60;
  else if (q && name.includes(q)) score += 30;
  score += Math.min(Number(row.students_count ?? 0) / 10, 20);
  score += Math.min(Number(row.courses_count ?? 0), 10);
  if (row.rating_average != null) score += Number(row.rating_average) * 2;
  return score;
}

function rankCourse(row: Record<string, unknown>, query: string): number {
  const title = String(row.title ?? '').toLowerCase();
  const q = query.toLowerCase();
  let score = Number(row.fts_rank ?? 0) * 10;
  if (q && title === q) score += 100;
  else if (q && title.startsWith(q)) score += 60;
  else if (q && title.includes(q)) score += 30;
  score += Math.min(Number(row.students_count ?? 0) / 5, 25);
  score += Math.min(Number(row.view_count ?? 0) / 20, 15);
  if (row.rating_average != null) score += Number(row.rating_average) * 3;
  return score;
}

export class SeoSearchService {
  static async search(filters: SearchFilters): Promise<{ items: SearchResultItem[]; total: number }> {
    const query = normalizeQuery(filters.q ?? filters.keywords);
    const key = cacheKey('search', { ...filters, q: query });
    const cached = seoCacheGet<{ items: SearchResultItem[]; total: number }>(key);
    ...
    seoCacheSet(key, result, SEO_CACHE_TTL.search);
    await logSearch(filters.tenant_id ?? null, query, filters, items.length);
    return result;
  }

  private static async searchTeachers(
    query: string,
    filters: SearchFilters,
    limit: number,
    offset: number,
    ...
    let i = 1;
    const where: string[] = ['t.is_active = TRUE', "t.subdomain <> 'default'"];
    ...
    return res.rows.map((row) => ({
      ...
      view_count: 0,
      public_url: buildTenantPublicUrl(String(row.subdomain)),
  ...
  private static async searchCourses(
    ...
    return res.rows.map((row) => ({
      ...
      public_url: tenantCourseUrl(String(row.subdomain), String(row.slug)),
      score: rankCourse(row, query),
      updated_at: row.updated_at ? String(row.updated_at) : String(row.created_at),
    }));
  }

  static async suggestions(q: string, tenantId?: number): Promise<string[]> {
    const query = normalizeQuery(q);
    if (query.length < 2) return [];
    const key = cacheKey('suggestions', { q: query, tenant_id: tenantId });
    const cached = seoCacheGet<string[]>(key);
    if (cached) return cached;
    ...
    const res = await pool.query<{ label: string }>(
      `(SELECT display_name AS label FROM tenants t
    ...
    seoCacheSet(key, suggestions, SEO_CACHE_TTL.suggestions);
    return suggestions;
  }

  static async trending(tenantId?: number, days = 7): Promise<string[]> {
    ...
    const res = await pool.query<{ query: string }>(
    ...
    seoCacheSet(key, trending, SEO_CACHE_TTL.trending);
    return trending;
  }

  static async popularTeachers(limit = 10): Promise<SearchResultItem[]> {
    const key = cacheKey('popular-teachers', { limit });
    const cached = seoCacheGet<SearchResultItem[]>(key);
    if (cached) return cached;
    ...
    seoCacheSet(key, items, SEO_CACHE_TTL.popular);
    return items;
  }

  static async popularCourses(limit = 10, tenantId?: number): Promise<SearchResultItem[]> {
