...
export interface CenterGroupStudent {
  id: number;
  /** رقم الطالب داخل المجموعة (يبدأ من 1 في كل مجموعة) */
...
export class CenterGroupsService {
    ...
    try {
      ...
      if (phone) {
        const existingStudentRes = await client.query(
          `SELECT id FROM users WHERE phone = $1 AND role = 'student'`,
          [phone],
        );
    ...
    // @ts-ignore
        if (existingStudentRes.rowCount > 0) {
          ...
          const newStudentRes = await client.query(
            `INSERT INTO users (name, phone, password, role, parent_phone)
             VALUES ($1, $2, $3, 'student', $4)
             RETURNING id`,
            [name, phone, randomPassword, parentPhone],
          );
        ...
        const randomPassword = Math.random().toString(36).slice(-8);
        const newStudentRes = await client.query(
          `INSERT INTO users (name, password, role, parent_phone)
           VALUES ($1, $2, 'student', $3)
           RETURNING id`,
          [name, randomPassword, parentPhone],
        );
        studentId = newStudentRes.rows[0].id;
      }
    ...
    // @ts-ignore
      ...
      const membershipRes = await client.query(
        `INSERT INTO group_students (group_id, student_id, number_in_group)
         VALUES ($1, $2, $3)
         RETURNING id, student_id, number_in_group, joined_at`,
      ...
      if (error?.message === 'الطالب موجود بالفعل في المجموعة') {
        logger.warn({ groupId, name, phone }, 'Student already exists in center group');
      } else {
        logger.error(
          { err: error, groupId, name, phone },
          'Error adding student to center group',
        );
      }
      throw error;
    } finally {
   ...
   */
  static async addStudentsByNamesToGroup(
    groupId: number,
    names: string[],
  ): Promise<BulkAddStudentsResult> {
    const added: CenterGroupStudent[] = [];
   ...
   */
  static async getGroupStudents(groupId: number): Promise<CenterGroupStudent[]> {
   ...
   */
  static generateAttendanceQrPayload(groupId: number, studentId: number): string {
    return jwt.sign(
      { groupId, studentId, type: 'attendance' },
      config.SECRET_KEY,
      { expiresIn: '365d' },
    );
  }
   ...
   */
  static verifyAttendanceQrPayload(token: string): { groupId: number; studentId: number } {
  ...
  static async recordAttendance(
    ...
    const result = await pool.query(
      `INSERT INTO group_attendance (group_id, student_id, date, status)
       ...
       RETURNING id, student_id, date, status, created_at, updated_at`,
      [groupId, studentId, date, status],
    );
   ...
   */
  static async recordBulkAttendance(
    groupId: number,
    date: string,
    attendanceList: Array<{ student_id: number; status: 'present' | 'absent' }>,
    ...
    try {
      ...
      const membersRes = await client.query(
        `SELECT student_id FROM group_students WHERE group_id = $1 AND student_id = ANY($2::int[])`,
        [groupId, studentIds],
      );
   ...
   */
  static async getAttendanceByDate(groupId: number, date: string): Promise<AttendanceRecord[]> {
    const result = await pool.query(
      `SELECT 
         ga.id,
         ga.student_id,
  ...
  static async getStudentAttendance(
    ...
    const params: any[] = [groupId, studentId];
