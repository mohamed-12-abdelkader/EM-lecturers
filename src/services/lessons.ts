...
import { Lesson, CreateLesson, UpdateLesson } from '../db/types/questionBank';
...
export class LessonService {
  ...
  static async create(
    ...
    `;
    const verifyResult = await pool.query(verifyQuery, [chapterId, subjectId, questionBankId]);
  ...
  static async getByChapter(
    ...
    const values: any[] = [chapterId, subjectId, questionBankId];
    let valueIndex = 4;
  ...
  static async delete(
    ...
    const query = `
      ...
      WHERE id = $1 AND chapter_id = $2 AND subject_id = $3 AND question_bank_id = $4
    `;
    const result = await pool.query(query, [lessonId, chapterId, subjectId, questionBankId]);
