...
import { Question, CreateQuestion, UpdateQuestionStatus } from '../db/types/questionBank';
...
export class QuestionService {
  ...
  static async getByLesson(
    ...
    const values: any[] = [lessonId, chapterId, subjectId, questionBankId];
    let valueIndex = 5;
  ...
  static async approve(
    ...
    const result = await pool.query(query, [status, adminId, questionId]);
  ...
  static async reject(
    ...
    const result = await pool.query(query, [status, rejection_reason, adminId, questionId]);
