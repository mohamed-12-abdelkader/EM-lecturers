...
export const TEACHER_CREATIVE_TONES = [
...
export const TEACHER_CREATIVE_ASPECT_RATIOS = [
...
const TONE_GUIDANCE: Record<TeacherCreativeTone, string> = {
...
export function normalizeTeacherCreativePlatform(platform?: string): TeacherCreativePlatform {
  const value = String(platform || '')
    .trim()
    .toLowerCase();
  return ['facebook', 'instagram', 'whatsapp', 'tiktok', 'general'].includes(value)
    ? (value as TeacherCreativePlatform)
    : 'general';
}

export function normalizeTeacherCreativeTone(tone?: string): TeacherCreativeTone {
  const value = String(tone || '')
    .trim()
    .toLowerCase();
  return ['friendly', 'professional', 'motivational', 'promotional'].includes(value)
    ? (value as TeacherCreativeTone)
    : 'friendly';
}

export function normalizeTeacherCreativeAspectRatio(
  aspectRatio?: string,
): TeacherCreativeAspectRatio {
...
export function buildTeacherPostUserPrompt(input: {
  prompt: string;
  platform?: string;
  tone?: string;
}): string {
...
export function buildTeacherImageCopySystemPrompt(): string {
...
export function buildTeacherImageCopyUserPrompt(input: {
  prompt: string;
  platform?: string;
  aspectRatio?: string;
}): string {
...
export function buildTeacherImagePrompt(input: {
  prompt: string;
  platform?: string;
  aspectRatio?: string;
