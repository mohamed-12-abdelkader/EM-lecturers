...
export class TeacherReportsService {
   ...
   */
  static async findStudentsByName(
    teacherId: number,
    name: string,
  ): Promise<{ id: number; name: string; email: string; phone: string; courses_count: string }[]> {
   ...
   */
  static async getStudentReportByName(
    teacherId: number,
    name: string,
  ): Promise<
    | { report: Awaited<ReturnType<typeof TeacherReportsService.getStudentDetailedReport>> }
    | { matches: { id: number; name: string; email: string; courses_count: string }[]; message: string }
  > {
    const matches = await this.findStudentsByName(teacherId, name);
    if (matches.length === 0) {
      throw new HttpError(404, 'لا يوجد طالب مشترك معك بهذا الاسم');
    }
    if (matches.length > 1) {
      return {
        ...
        message:
          'وجد أكثر من طالب بهذا الاسم. اختر المعرّف (id) وأرسل الطلب إلى /teacher/students/:studentId/report أو أضف جزءاً من الاسم لتحديد الطالب.',
      };
    }
   ...
   */
  static async getTeacherStudents(teacherId: number) {
    const result = await pool.query(
      `SELECT DISTINCT 
         u.id,
         u.name,
   ...
   */
  static async getStudentDetailedReport(teacherId: number, studentId: number) {
    // Verify student is enrolled in teacher's courses
    const enrollmentCheck = await pool.query(
      `SELECT DISTINCT e.course_id, c.title as course_title
       FROM enrollments e
       JOIN courses c ON e.course_id = c.id
    ...
    if (!enrollmentCheck.rowCount) {
      throw new HttpError(404, 'Student not found or not enrolled in your courses');
    }
    ...
    const coursesDetails = await Promise.all(
      courses.map(async (course) => {
        try {
          ...
          const watchedLecturesRes = await pool.query(
            `SELECT l.id, l.title, lv.viewed_at
          ...
          const allLecturesRes = await pool.query(
          ...
          const lectureExamsRes = await pool.query(
            `SELECT e.id, e.title, e.type, l.title as lecture_title
           FROM exams e
           JOIN lectures l ON e.lecture_id = l.id
           WHERE l.course_id = $1 AND e.type = 'exam'
           ORDER BY l.created_at ASC, e.created_at ASC`,
            [courseId],
          );
          ...
          const courseExamsRes = await pool.query(
            `SELECT id, title, questions_count, duration_minutes, created_at
          ...
          const courseExamAttemptsRes = await pool.query(
            `SELECT a.exam_id, a.attempt_number, a.status, a.total_grade, a.obtained_grade, 
          ...
          const submittedExams =
            examsWithStatus.filter((e) => e.hasSubmitted).length +
    ...
    const watchedLecturesOverall = coursesDetails.reduce(
      (sum, c) => sum + c.statistics.watchedLecturesCount,
      0,
    );
    const totalExamsOverall = coursesDetails.reduce((sum, c) => sum + c.statistics.totalExams, 0);
    const submittedExamsOverall = coursesDetails.reduce(
      (sum, c) => sum + c.statistics.submittedExams,
      0,
    );
    const overallWatchPercentage =
      coursesDetails.length > 0
        ? Math.round(
            (coursesDetails.reduce((sum, c) => sum + (c.statistics.watch_percentage ?? 0), 0) /
    ...
    return {
      ...
      overallStatistics: {
        ...
        overallAverageGrade:
          coursesDetails.length > 0
            ? Math.round(
                (coursesDetails.reduce((sum, c) => sum + c.statistics.averageGrade, 0) /
                  coursesDetails.length) *
                  100,
              ) / 100
            : 0,
