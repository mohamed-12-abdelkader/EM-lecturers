...
} from '../db/types/questionBank';

export class QuestionBankService {
  // Create new question bank
  static async create(data: CreateQuestionBank, createdBy: number): Promise<QuestionBank> {
  ...
  static async list(params: {
    ...
    is_active?: boolean;
    search?: string;
  }): Promise<z.infer<typeof QuestionBankListResponse>> {
    return this.getAll(
      params.page ?? 1,
      params.limit ?? 20,
      params.grade_id,
  ...
  static async getAll(
    ...
    const questionBanks = result.rows.map((bank: any) => ({
      ...bank,
  ...
  static async getById(id: number): Promise<QuestionBank | null> {
    const query = `
      SELECT 
        qb.*,
        g.name as grade_name,
  ...
  static async update(id: number, data: UpdateQuestionBank): Promise<QuestionBank> {
    const { name, description, grade_id, is_active, image_url, price } = data;
    ...
    const gradeResult = await pool.query(gradeQuery, [questionBank.grade_id]);
  ...
  static async delete(id: number, force: boolean = false): Promise<void> {
    // Check if question bank exists
    const existingBank = await this.getById(id);
    if (!existingBank) {
      throw new Error('بنك الأسئلة غير موجود');
      ...
      try {
        ...
        await pool.query(
          `DELETE FROM questions q
           ...
           WHERE q.lesson_id = l.id AND l.chapter_id = c.id AND c.subject_id = s.id AND s.question_bank_id = $1`,
          [id],
        );
        // Delete lessons
        await pool.query(
          `DELETE FROM lessons l
           ...
           WHERE l.chapter_id = c.id AND c.subject_id = s.id AND s.question_bank_id = $1`,
          [id],
        );
        // Delete chapters
        await pool.query(
          `DELETE FROM chapters c
           USING subjects s
           WHERE c.subject_id = s.id AND s.question_bank_id = $1`,
          [id],
        );
        ...
        await pool.query(
          `DELETE FROM teacher_subjects ts
           USING subjects s
           WHERE ts.subject_id = s.id AND s.question_bank_id = $1`,
          [id],
        );
        // Delete subjects
        await pool.query(`DELETE FROM subjects WHERE question_bank_id = $1`, [id]);
        // Finally delete the bank
        const result = await pool.query(`DELETE FROM question_banks WHERE id = $1`, [id]);
        if (result.rowCount === 0) throw new Error('فشل في حذف بنك الأسئلة');
  ...
  static async getStats(id: number): Promise<any> {
    ...
    // Get grade info
    const gradeQuery = `SELECT name, level FROM grades WHERE id = $1`;
    const gradeResult = await pool.query(gradeQuery, [existingBank.grade_id]);
