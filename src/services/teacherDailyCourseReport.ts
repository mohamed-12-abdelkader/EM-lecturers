...
export interface DailyCourseReportCourse {
  ...
  /** معرف الصف الدراسي (إن وُجد) */
  grade_id?: number | null;
  /** اسم الصف (مثل: أولى ثانوي، ثانية ثانوي) */
  grade_name?: string | null;
}

export interface DailyCourseReportStudentsStats {
...
export interface DailyCourseReportLectureStats {
  last_lecture_watchers_count: number;
  average_watch_percentage: number;
}

export interface DailyCourseReportLastExam {
  exam_id: number;
  exam_title: string;
  exam_type: 'lecture' | 'course';
...
export interface WeakQuestion {
  question_id: number;
  question_source: 'lecture_exam' | 'course_exam';
  question_text: string | null;
  question_image: string | null;
  wrong_rate: number;
  students_wrong_count: number;
}

export interface TeacherDailyCourseReport {
  course: DailyCourseReportCourse | null;
  students_stats: DailyCourseReportStudentsStats;
  lecture_stats: DailyCourseReportLectureStats;
  last_exam: DailyCourseReportLastExam | null;
 ...
 */
export class TeacherDailyCourseReportService {
  /**
   * آخر كورس: آخر كورس أنشأه المدرس (حسب created_at).
   */
  static async getLastCourseId(teacherId: number): Promise<number | null> {
   ...
   */
  static async getLastCoursesPerGrade(
    teacherId: number,
  ): Promise<{ course_id: number; grade_id: number; grade_name: string }[]> {
    const res = await pool.query(
      `SELECT DISTINCT ON (c.grade_id)
              c.id AS course_id, c.grade_id, g.name AS grade_name
       FROM courses c
       LEFT JOIN grades g ON g.id = c.grade_id
       WHERE c.teacher_id = $1 AND c.grade_id IS NOT NULL
       ORDER BY c.grade_id, c.created_at DESC`,
      [teacherId],
    );
  ...
  static async getReport(teacherId: number): Promise<TeacherDailyCourseReport> {
    const reports = await this.getReports(teacherId);
    if (reports.length === 0) {
      throw new HttpError(404, 'لا يوجد كورس للمدرس');
    }
    return reports[0];
  }
   ...
   */
  static async getReports(teacherId: number): Promise<TeacherDailyCourseReport[]> {
    ...
    if (coursesPerGrade.length === 0) {
      ...
      if (lastId) coursesPerGrade = [{ course_id: lastId, grade_id: 0, grade_name: '' }];
    }
    const reports: TeacherDailyCourseReport[] = [];
    for (const { course_id, grade_id, grade_name } of coursesPerGrade) {
      try {
        const report = await this.getReportForCourse(
          teacherId,
          course_id,
          grade_id || undefined,
   ...
   */
  static async getReportForCourse(
    teacherId: number,
    courseId: number,
    gradeId?: number,
    gradeName?: string,
  ): Promise<TeacherDailyCourseReport> {
    const generated_at = new Date().toISOString();

    const emptyReport: TeacherDailyCourseReport = {
      course: null,
      students_stats: {
        total_enrolled: 0,
        enrolled_today: 0,
    ...
    const courseCheck = await pool.query(
      `SELECT id, title, created_at, grade_id FROM courses WHERE id = $1 AND teacher_id = $2`,
      [courseId, teacherId],
    );
    if (!courseCheck.rowCount) {
      throw new HttpError(404, 'الكورس غير موجود أو لا يخصك');
    }
    ...
    if (courseGradeId != null && !courseGradeName) {
      const g = await pool.query('SELECT name FROM grades WHERE id = $1', [courseGradeId]);
      courseGradeName = g.rows[0]?.name ?? null;
    }

    // 1) Course: lectures count
    const lecturesCountRes = await pool.query(
      'SELECT COUNT(*) as cnt FROM lectures WHERE course_id = $1',
      [courseId],
    );
    const lectures_count = parseInt(lecturesCountRes.rows[0]?.cnt || '0', 10);

    const course: DailyCourseReportCourse = {
    ...
    const totalEnrolledRes = await pool.query(
    ...
    const enrolledTodayRes = await pool.query(
      `SELECT COUNT(*) as cnt FROM enrollments e
       WHERE e.course_id = $1
    ...
    const activeIdsRes = await pool.query(
    ...
    const activeUserIds = (activeIdsRes.rows || []).map((r: any) => r.user_id);

    const enrolledUserIdsRes = await pool.query(
    ...
    const enrolledUserIds = new Set((enrolledUserIdsRes.rows || []).map((r: any) => r.user_id));
    ...
    const students_stats: DailyCourseReportStudentsStats = {
      total_enrolled,
      enrolled_today,
      active_count,
      inactive_count,
    ...
    let last_lecture_id: number | null = null;
    if (lectures_count > 0) {
      const lastLectureRes = await pool.query(
        `SELECT id FROM lectures WHERE course_id = $1 ORDER BY position DESC NULLS LAST, id DESC LIMIT 1`,
        [courseId],
      );
    ...
    if (last_lecture_id) {
      const watchersRes = await pool.query(
        `SELECT COUNT(DISTINCT vv.user_id) as cnt
         FROM video_views vv
    ...
    const lastExamRow = await pool.query(
      `SELECT e.id, e.title, e.created_at, 'lecture' AS exam_type
       FROM exams e
       JOIN lectures l ON e.lecture_id = l.id
       WHERE l.course_id = $1
       UNION ALL
    ...
    let last_exam: DailyCourseReportLastExam | null = null;
    if (lastExamRow.rowCount && lastExamRow.rows[0]) {
      const row = lastExamRow.rows[0];
      const examId = row.id;
      const examType = row.exam_type as 'lecture' | 'course';
    ...
    const lectureWeakRes = await pool.query(
      `SELECT eq.id AS question_id,
       ...
       HAVING COUNT(*) > 0 AND (COUNT(*) FILTER (WHERE ea.is_correct = false)::float / COUNT(*)) > 0.5`,
      [courseId],
    );
    ...
    const courseWeakRes = await pool.query(
      `SELECT q.id AS question_id,
       ...
       HAVING COUNT(*) > 0 AND (COUNT(*) FILTER (WHERE a.is_correct = false)::float / COUNT(*)) > 0.5`,
      [courseId],
    );
