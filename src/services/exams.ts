...
import {
  ...
  normalizeNonNegativeHoursIfProvided,
  toDateOrNull,
  toDateOrNullIfProvided,
  toNumberOrNullIfProvided,
} from './courseExamSettings';

export class ExamsService {
  static async createCourseExam({
    courseId,
    title,
    image,
    questionsCount,
    ...
    const normalizedShowAt = toDateOrNull(showAt, 'showAt');
    const normalizedHideAt = toDateOrNull(hideAt, 'hideAt');
    const normalizedStartWindow = toDateOrNull(startWindow, 'startWindow');
    const normalizedEndWindow = toDateOrNull(endWindow, 'endWindow');
    const normalizedAnswersReleaseDate = toDateOrNull(answersReleaseDate, 'answersReleaseDate');
    const normalizedTimeLimitMinutesValue = toNumberOrNullIfProvided(
      timeLimitMinutes,
      'timeLimitMinutes',
    );
    const normalizedTimeLimitMinutes =
      normalizedTimeLimitMinutesValue === undefined ? null : normalizedTimeLimitMinutesValue;
    const normalizedShowAnswersAfterHours = normalizeNonNegativeHours(
      showAnswersAfterHours,
      'showAnswersAfterHours',
    );
    const normalizedShowAnswersImmediately =
      showAnswersImmediately === undefined ? true : !!showAnswersImmediately;
    const normalizedLockNextLectures = !!lockNextLectures;
    let normalizedAttemptLimit: number | null = null;
    if (attemptLimit !== undefined) {
      if (attemptLimit === null) {
        ...
        if (!Number.isInteger(parsedLimit) || parsedLimit <= 0) {
          throw buildValidationError('attemptLimit must be a positive integer when provided');
        }
        normalizedAttemptLimit = parsedLimit;
      }
    }

    const normalizedAllowMultipleAttempts =
      normalizedAttemptLimit !== null ? normalizedAttemptLimit > 1 : !!allowMultipleAttempts;
    ...
    if (
      ...
      throw buildValidationError(
        'Provide a positive timeLimitMinutes value when enabling the timer for course exam',
      );
    }
    if (normalizedShowAnswersLater && !normalizedAnswersReleaseDate) {
      throw buildValidationError(
        'answersReleaseDate is required when showAnswersLater is enabled for course exam',
      );
    }
    if (
      ...
      throw buildValidationError('startWindow must be earlier than endWindow');
    }
  ...
  static async updateCourseExamSettings(
    examId: number,
    settings: {
      showAt?: string | null;
      hideAt?: string | null;
    ...
    const providedKeys = Object.keys(settings).filter(
      (key) => (settings as Record<string, unknown>)[key] !== undefined,
    );
    if (!providedKeys.length) {
      return null;
    }

    const examRes = await pool.query('SELECT * FROM course_exams WHERE id = $1', [examId]);
    if (!examRes.rowCount) {
    ...
    const futureSettings = {
      startWindow: exam.start_window as Date | null,
      endWindow: exam.end_window as Date | null,
      showAnswersLater: exam.show_answers_later as boolean | null,
      answersReleaseDate: exam.answers_release_date as Date | null,
      timeLimitEnabled: exam.time_limit_enabled as boolean | null,
      timeLimitMinutes: exam.time_limit_minutes as number | null,
    };

    if (settings.showAt !== undefined) {
      const normalized = toDateOrNullIfProvided(settings.showAt, 'showAt');
      updateFields.push(`show_at = $${paramIndex++}`);
      values.push(normalized);
    }
    if (settings.hideAt !== undefined) {
      const normalized = toDateOrNullIfProvided(settings.hideAt, 'hideAt');
    ...
    if (settings.showAnswersAfterHours !== undefined) {
      const normalized = normalizeNonNegativeHoursIfProvided(
        settings.showAnswersAfterHours,
        'showAnswersAfterHours',
      );
    ...
    if (settings.answersReleaseDate !== undefined) {
      const normalized = toDateOrNullIfProvided(settings.answersReleaseDate, 'answersReleaseDate');
    ...
    if (settings.timeLimitMinutes !== undefined) {
      const normalized = toNumberOrNullIfProvided(settings.timeLimitMinutes, 'timeLimitMinutes');
    ...
    if (settings.startWindow !== undefined) {
      const normalized = toDateOrNullIfProvided(settings.startWindow, 'startWindow');
      updateFields.push(`start_window = $${paramIndex++}`);
      values.push(normalized);
      futureSettings.startWindow = normalized ?? null;
    }
    if (settings.endWindow !== undefined) {
      const normalized = toDateOrNullIfProvided(settings.endWindow, 'endWindow');
    ...
    if (
      futureSettings.timeLimitEnabled &&
    ...
    const result = await pool.query(
      `UPDATE course_exams SET ${updateFields.join(', ')} WHERE id = $${paramIndex++} RETURNING *`,
      values,
    );
    return result.rows[0];
  }

  static async getCourseExams(courseId: number) {
    const res = await pool.query(
      `SELECT * FROM course_exams WHERE course_id = $1 ORDER BY created_at DESC`,
      [courseId],
    );
    return res.rows;
  }

  static async getVisibleCourseExams(courseId: number) {
    const res = await pool.query(
      `SELECT * FROM course_exams WHERE course_id = $1 AND is_visible = true ORDER BY created_at DESC`,
      [courseId],
    );
    return res.rows;
  }
  ...
  static async addQuestionsFromBank({
    ...
    const posRes = await pool.query(
      'SELECT COALESCE(MAX(position), 0) AS max_pos FROM course_exam_questions WHERE course_exam_id = $1',
      [examId],
    );
    ...
    for (const qid of questionIds) {
      pos++;
      await pool.query(
        `INSERT INTO course_exam_questions (course_exam_id, question_id, position, grade) VALUES ($1, $2, $3, 1)`,
        [examId, qid, pos],
      );
    }
  }
  ...
  static async bulkAddQuestionsFromText({
    ...
    if (!parsedQuestions.length) {
      throw new Error(
        'لم يتم العثور على أي أسئلة في النص. تأكد من استخدام تنسيق صحيح لكل سؤال مع الاختيارات A/B/C/D.',
      );
    }
  ...
  private static parseQuestionsFromText(text: string) {
    if (!text) {
      return [];
    }
    ...
    if (invalidBlocks.length && structuredSplit.length) {
      throw new Error(
        `هناك مشكلة في الأسئلة التالية: ${invalidBlocks.join(
          ', ',
        )}. تأكد أن كل سؤال يحتوي على نص وأربع اختيارات.`,
      );
    }
    ...
    const compactPattern =
      /(?<question>[\s\S]+?)A[).:,-]?\s*(?<choiceA>[\s\S]+?)B[).:,-]?\s*(?<choiceB>[\s\S]+?)C[).:,-]?\s*(?<choiceC>[\s\S]+?)D[).:,-]?\s*(?<choiceD>[\s\S]+?)(?=(?:\s+[A-Za-z].+?A[).:,-]?)|$)/g;
    ...
    const compactQuestions = compactMatches
      ...
      .filter(Boolean) as { text: string; choices: { text: string; is_correct: boolean }[] }[];

    return compactQuestions;
  }

  static async addImageQuestionToCourseExam({
    examId,
    text,
    imageUrl,
    choices,
    ...
    await pool.query(
      `INSERT INTO course_exam_questions (course_exam_id, question_id, position, grade) VALUES ($1, $2, $3, 1)`,
      [examId, questionId, nextPosition],
    );

    return questionId;
  }

  static async bulkAddImageQuestionsToCourseExam({
    examId,
    questions,
  }: {
    examId: number;
    questions: { imageUrl: string; text?: string }[];
  }) {
    if (!questions.length) {
  ...
  static async getCourseExamQuestions(examId: number) {
    // جلب الأسئلة المرتبطة بالامتحان مع ترتيبها
    const qRes = await pool.query(
      `SELECT q.id, q.text, q.type, q.image, ceq.position
       FROM course_exam_questions ceq
       JOIN questions q ON ceq.question_id = q.id
       WHERE ceq.course_exam_id = $1
       ORDER BY ceq.position, ceq.id`,
      [examId],
    );
    const questions = qRes.rows;
    // جلب كل الاختيارات دفعة واحدة
    const questionIds = questions.map((q) => q.id);
    const choicesMap: Record<number, any[]> = {};
    if (questionIds.length) {
      const chRes = await pool.query(
        `SELECT * FROM question_choices WHERE question_id = ANY($1::int[])`,
        [questionIds],
      );
      for (const ch of chRes.rows) {
        if (!choicesMap[ch.question_id]) choicesMap[ch.question_id] = [];
  ...
  static async deleteCourseExamQuestion(questionId: number) {
    // حذف الاختيارات أولاً
    await pool.query(`DELETE FROM question_choices WHERE question_id = $1`, [questionId]);
    // حذف السؤال من جدول course_exam_questions
    await pool.query(`DELETE FROM course_exam_questions WHERE question_id = $1`, [questionId]);
    // حذف السؤال نفسه
    const result = await pool.query(`DELETE FROM questions WHERE id = $1 RETURNING *`, [
      questionId,
    ]);
    if (!result.rowCount) {
  ...
  static async updateCourseExamQuestion(
    questionId: number,
    text?: string,
    grade?: number,
    ...
    // تحديث نص السؤال
    if (text !== undefined) {
      await pool.query(`UPDATE questions SET text = $1 WHERE id = $2`, [text, questionId]);
    }
    // تحديث صورة السؤال
    if (image !== undefined) {
      await pool.query(`UPDATE questions SET image = $1 WHERE id = $2`, [image, questionId]);
    }
    // تحديث الدرجة إلى 1 دائماً (كل سؤال بدرجة واحدة)
    await pool.query(`UPDATE course_exam_questions SET grade = 1 WHERE question_id = $1`, [
      questionId,
    ]);
    // تحديث الاختيارات
  ...
  static async setCourseExamQuestionCorrectAnswer(questionId: number, correctChoiceId: number) {
    // اجلب كل الاختيارات لهذا السؤال
    const choicesRes = await pool.query(`SELECT id FROM question_choices WHERE question_id = $1`, [
      questionId,
    ]);
    ...
    const updateRes = await pool.query(
      `UPDATE question_choices SET is_correct = true WHERE id = $1 AND question_id = $2 RETURNING *`,
  ...
  static async submitCourseExam(
    ...
    const examInfoRes = await pool.query(
      `SELECT ce.id as exam_id, ce.course_id, ce.total_grade, ce.allow_multiple_attempts, ce.attempt_limit
    ...
    if (!enrollRes.rowCount) {
      const err: any = new Error('غير مسموح لك بحل هذا الامتحان. يجب أن تكون مشتركاً في الكورس.');
      err.status = 403;
      throw err;
    }
    // جلب جميع الأسئلة واختياراتها
    const questionsRes = await pool.query(
      `SELECT ceq.question_id, q.text as question_text, q.image as question_image, qc.id as choice_id, qc.text as choice_text, qc.is_correct
       FROM course_exam_questions ceq
       JOIN questions q ON ceq.question_id = q.id
       LEFT JOIN question_choices qc ON ceq.question_id = qc.question_id
       WHERE ceq.course_exam_id = $1
       ORDER BY ceq.position, ceq.question_id, qc.id`,
      [examId],
    );
    ...
    for (const [questionId, q] of questionsMap.entries()) {
      ...
      if (answer && correctChoice && answer.choiceId === correctChoice.id) {
        ...
        wrongQuestions.push({
          ...
          yourChoice: answer ? q.choices.find((c: any) => c.id === answer.choiceId) : null,
        });
      }
    }
    ...
    const attemptLimitValue =
      attempt_limit !== null && attempt_limit !== undefined
        ? Number(attempt_limit)
        : allow_multiple_attempts
          ? null
          : 1;
    const existingSubmissionRes = await pool.query(
      'SELECT id, attempts_count FROM course_exam_submissions WHERE exam_id = $1 AND student_id = $2',
      [examId, studentId],
    );
    ...
    if (attemptLimitValue !== null && attemptsCount >= attemptLimitValue) {
      const err: any = new Error('You have used all allowed attempts for this exam.');
      err.status = 403;
      throw err;
    }
    ...
    if (existingSubmissionRes.rowCount) {
      await pool.query(
        'UPDATE course_exam_submissions SET total_grade = $1, passed = $2, submitted_at = NOW(), attempts_count = attempts_count + 1 WHERE exam_id = $3 AND student_id = $4',
        [total, passed, examId, studentId],
      );
    } else {
      await pool.query(
        'INSERT INTO course_exam_submissions (exam_id, student_id, total_grade, passed, submitted_at, attempts_count) VALUES ($1, $2, $3, $4, NOW(), 1)',
        [examId, studentId, total, passed],
      );
    }
