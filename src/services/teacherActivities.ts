...
export class TeacherActivityService {
   ...
   */
  static async logActivity(activityData: TeacherActivityData) {
    try {
   ...
   */
  static async getTeacherActivities(
    teacherId: number,
    limit: number = 20,
    offset: number = 0,
    activityType?: string,
   ...
   */
  static async getTeacherActivityStats(teacherId: number) {
    try {
      // إجمالي النشاطات
      const totalResult = await pool.query(
        'SELECT COUNT(*) as total FROM teacher_activities WHERE teacher_id = $1',
        [teacherId],
      );

      // نشاطات هذا الشهر
      const monthlyResult = await pool.query(
        `SELECT COUNT(*) as monthly FROM teacher_activities 
      ...
      const weeklyResult = await pool.query(
        `SELECT COUNT(*) as weekly FROM teacher_activities 
         WHERE teacher_id = $1 AND created_at >= DATE_TRUNC('week', NOW())`,
        [teacherId],
      );
   ...
   */
  static async logCourseCreated(teacherId: number, courseId: number, courseTitle: string) {
