...
export interface StudentDailyReport {
  pending_lectures: PendingLecture[];
  pending_exams: PendingExam[];
  summary: {
    pending_lectures_count: number;
 ...
 */
export class StudentDailyReportService {
  static async getReport(studentId: number): Promise<StudentDailyReport> {
    ...
    const courseIds = enrollments.map((r: any) => r.course_id);
    ...
    const lecturesRes = await pool.query(
      `SELECT l.id, l.title, l.course_id, l.position
       FROM lectures l
    ...
    let videoCountByLecture: Record<number, number> = {};
    let watchedCountByLecture: Record<number, number> = {};
    ...
    const lectureExamsRes = await pool.query(
      `SELECT e.id, e.title, e.lecture_id, e.total_grade, l.course_id
       FROM exams e
    ...
    for (const exam of lectureExams) {
      const subRes = await pool.query(
        `SELECT id, status, submitted_at, total_grade, passed
    ...
    const courseExamsRes = await pool.query(
      `SELECT ce.id, ce.title, ce.course_id, ce.total_grade, ce.attempt_limit
       FROM course_exams ce
       WHERE ce.course_id = ANY($1::int[])`,
      [courseIds],
    );
    for (const exam of courseExamsRes.rows) {
      const subRes = await pool.query(
        `SELECT id, attempts_count, submitted_at, passed
