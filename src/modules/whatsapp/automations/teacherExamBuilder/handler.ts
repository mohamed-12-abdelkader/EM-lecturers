...
import {
  runExamBuilderApprove,
  runExamBuilderChat,
  runExamBuilderHelp,
  runExamBuilderRegenerate,
} from './bridge';
import { parseExamBuilderCommand } from './commands';

export const TEACHER_EXAM_BUILDER_BOT_KEY = 'teacher_exam_builder_bot';
...
const VOICE_UNSUPPORTED_REPLY =
  'معلش، الرسائل الصوتية مش مدعومة هنا 🙏 لو سمحت اكتب طلبك نصاً.';

const IMAGE_UNSUPPORTED_REPLY =
  'مساعد الامتحانات عبر واتساب يدعم الرسائل النصية فقط. اكتب مثلاً: أنشئ امتحان 10 أسئلة من الفصل الأول.';
...
async function maybeResumeFromHumanMute(
  ...
  logger.info(
    ...
    'teacher_exam_builder_bot resumed after human mute expired',
  );
...
async function onInbound(ctx: InboundContext): Promise<HandlerResult> {
  ...
  if (conversation && conversation.status !== 'bot') {
    ...
    if (conversation.status !== 'bot') {
      logger.info(
        ...
        'teacher_exam_builder_bot skipped (human mute active)',
      );
      return { handled: true };
    }
  }
  ...
  if (!ctx.body?.trim() && !ctx.media && !ctx.mediaError) {
    logger.info(
      ...
      'teacher_exam_builder_bot skipped (empty non-media inbound)',
    );
    return { handled: true };
  }
  ...
  try {
    await enforcePlanFeature(teacher.id, 'exam_builder_ai');
  } catch (err) {
    const message =
      err instanceof HttpError
        ? err.message
        : 'إنشاء الامتحانات بالـ AI متاح في باقة الذهبية أو أعلى';
    return {
      handled: true,
      reply: message,
      metadata: { access_denied: 'plan', teacher_id: teacher.id },
    };
  }

  const command = parseExamBuilderCommand(ctx.body || '');

  try {
    if (command.type === 'help') {
      const help = await runExamBuilderHelp();
      return {
        handled: true,
        reply: help.reply,
        metadata: { ...help.metadata, teacher_id: teacher.id },
      };
    }

    if (command.type === 'approve') {
      const result = await runExamBuilderApprove(ctx, teacher.id);
      return {
        handled: true,
        reply: result.reply,
    ...
    if (command.type === 'regenerate') {
      const result = await runExamBuilderRegenerate(ctx, teacher.id);
      return {
        handled: true,
        reply: result.reply,
        metadata: result.metadata,
    ...
    const result = await runExamBuilderChat(ctx, teacher.id, command.text);
    return {
      handled: true,
      reply: result.reply,
      metadata: result.metadata,
    ...
    logger.error(
      ...
      'teacher_exam_builder_bot handler error',
    );
    const message =
      err instanceof HttpError
        ? err.message
...
export function registerTeacherExamBuilderBot(): void {
  registerWhatsAppHandler({
    key: TEACHER_EXAM_BUILDER_BOT_KEY,
    onInbound,
  });
  logger.info('Registered WhatsApp handler: teacher_exam_builder_bot');
}
