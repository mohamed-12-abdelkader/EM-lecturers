...
export class SessionPoolService {
  ...
  /**
   * Pick a session for teacher-owned outbound messaging.
   * Same sticky + weighted logic as pickSession, but pool members must belong to the teacher.
   */
  static async pickSessionForTeacher(
    serviceKey: string,
    teacherId: number,
    contactPhone?: string | null,
  ): Promise<{ sessionSlug: string; serviceId: number }> {
    ...
    if (phone) {
      const sticky = await pool.query<{ session_slug: string }>(
        `SELECT c.session_slug
         FROM wa_conversations c
         JOIN wa_sessions s ON s.slug = c.session_slug
    ...
    if (!members.rowCount) {
      throw new HttpError(
        503,
        `لا توجد جلسات واتساب جاهزة لخدمتك. اربط رقماً وعيّنه للخدمة وتأكد من حالة «متصلة».`,
      );
    }
