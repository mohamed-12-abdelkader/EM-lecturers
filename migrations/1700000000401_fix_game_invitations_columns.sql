-- Migration: Add missing columns to game_invitations table and fix column names
-- Date: 2024-01-01
-- Description: Adds all required columns if they don't exist and fixes column name mismatch
...
BEGIN
    ...
    IF EXISTS (
        ...
        -- Handle selected_lessons vs lesson_ids mismatch
        -- If selected_lessons exists but lesson_ids doesn't, rename it
        IF EXISTS (
            SELECT 1 
            FROM information_schema.columns 
            WHERE table_name = 'game_invitations' 
            AND column_name = 'selected_lessons'
        ) AND NOT EXISTS (
            SELECT 1 
            FROM information_schema.columns 
            WHERE table_name = 'game_invitations' 
            ...
            RENAME COLUMN selected_lessons TO lesson_ids;
        END IF;

        -- Add lesson_ids column if it doesn't exist (and selected_lessons doesn't exist either)
        IF NOT EXISTS (
            SELECT 1 
            FROM information_schema.columns 
            WHERE table_name = 'game_invitations' 
