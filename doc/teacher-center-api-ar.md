...
## بطاقة ولي الأمر داخل الـ QR

صورة الـ QR فيها **نص عربي يظهر فور المسح** داخل أي قارئ — من غير رابط ومن غير صفحات.

المحتوى: اسم الطالب، المجموعات، الحضور والغياب، اشتراك الشهر، آخر الحصص، والدرجات.

`GET /students/:studentId/qr` يحدّث النص بأحدث الأرقام قبل الطباعة.

مسح الحضور: أرسل النص كامل في `qr_payload` أو الـ UUID في `qr_token`.

يوجد مسار HTML اختياري `GET /api/public/center/students/:qrToken` لكنه **غير مضمّن** في الكود.
