...
صورة الـ QR فيها **نص عربي** (مش رابط). أي قارئ باركود يعرض بيانات الطالب فور المسح من غير فتح صفحات:

- اسم الطالب والكود واسم المدرس
- كل مجموعة: المادة / الصف / الجدول / الحضور والغياب
- اشتراك الشهر الحالي والمتبقي
- آخر الحصص ودرجات السنتر

مسح الحضور من تطبيق المدرس كما هو: `POST /attendance/scan` يقبل النص كامل في `qr_payload` أو الـ UUID.

افتح `GET /students/:studentId/qr` قبل الطباعة حتى تتحدث الأرقام داخل الكود.
