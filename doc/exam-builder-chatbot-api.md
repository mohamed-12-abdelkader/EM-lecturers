...
| عرض مقترح + حذف/استبدال أسئلة معينة + اعتماد | |

### تعديل المقترح (حذف / استبدال)

بعد ظهور الأسئلة يمكنك التعديل تدريجياً حتى تعتمد النسخة النهائية:
...
{
  "remove_positions": [3],
  "replace_positions": [1, 2],
  "remove_ids": [101],
  "replace_ids": [205],
  "refill_removed": false
}
```

أو من الشات مع `session_id`:

```json
{ "message": "شيل السؤال 3", "session_id": "..." }
```
