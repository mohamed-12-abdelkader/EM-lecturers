...
exports.TeacherFilesRepository = void 0;
...
class TeacherFilesRepository {
    static async create(input) {
        const result = await pool_1.default.query(`INSERT INTO teacher_files (
    ...
    static async findById(id, teacherId) {
        const result = await pool_1.default.query(`SELECT f.*, c.name AS category_name
       FROM teacher_files f
       LEFT JOIN file_categories c ON c.id = f.category_id
       WHERE f.id = $1 AND f.teacher_id = $2 AND f.deleted_at IS NULL`, [id, teacherId]);
        return result.rows[0] ?? null;
    }
    static async list(query) {
        const params = [query.teacherId];
        const where = ['f.teacher_id = $1', 'f.deleted_at IS NULL'];
        ...
        const countResult = await pool_1.default.query(`SELECT COUNT(*)::text AS count FROM teacher_files f WHERE ${whereSql}`, params);
        const total = parseInt(countResult.rows[0]?.count ?? '0', 10);
        const offset = (query.page - 1) * query.limit;
        params.push(query.limit, offset);
        const listResult = await pool_1.default.query(`SELECT f.*, c.name AS category_name
       FROM teacher_files f
       LEFT JOIN file_categories c ON c.id = f.category_id
       WHERE ${whereSql}
    ...
    static async update(id, teacherId, fields) {
        ...
        const result = await pool_1.default.query(`UPDATE teacher_files f
       SET ${sets.join(', ')}
       WHERE f.id = $1 AND f.teacher_id = $2 AND f.deleted_at IS NULL
       RETURNING f.*, (
         SELECT c.name FROM file_categories c WHERE c.id = f.category_id
       ) AS category_name`, params);
        return result.rows[0] ?? null;
    }
    static async softDelete(id, teacherId) {
        const result = await pool_1.default.query(`UPDATE teacher_files
       SET deleted_at = NOW(), updated_at = NOW()
       WHERE id = $1 AND teacher_id = $2 AND deleted_at IS NULL`, [id, teacherId]);
        return (result.rowCount ?? 0) > 0;
    }
    static async bulkSoftDelete(ids, teacherId) {
        if (ids.length === 0)
            return 0;
        const result = await pool_1.default.query(`UPDATE teacher_files
       SET deleted_at = NOW(), updated_at = NOW()
       WHERE teacher_id = $1 AND id = ANY($2::int[]) AND deleted_at IS NULL`, [teacherId, ids]);
        return result.rowCount ?? 0;
    }
    static async incrementDownloads(id, teacherId) {
        const result = await pool_1.default.query(`UPDATE teacher_files
    ...
    static async getStatistics(teacherId) {
       ...
       WHERE teacher_id = $1 AND deleted_at IS NULL`, [teacherId]);
        const byType = await pool_1.default.query(`SELECT
         CASE
    ...
    static async findManyByIds(ids, teacherId) {
        if (ids.length === 0)
            return [];
        const result = await pool_1.default.query(`SELECT * FROM teacher_files
       WHERE teacher_id = $1 AND id = ANY($2::int[]) AND deleted_at IS NULL`, [teacherId, ids]);
        return result.rows;
    }
}
exports.TeacherFilesRepository = TeacherFilesRepository;
