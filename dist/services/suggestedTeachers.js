...
exports.SuggestedTeachersService = void 0;
const pool_1 = __importDefault(require("../db/pool"));
class SuggestedTeachersService {
    /**
     * إضافة مدرس مقترح
     */
    static async addSuggestedTeacher(teacherId, suggestedTeacherId) {
        try {
            // التحقق من أن المدرس المقترح موجود وله دور مدرس
            const suggestedTeacherCheck = await pool_1.default.query('SELECT id, name, email, avatar, description, subject FROM users WHERE id = $1 AND role = $2', [suggestedTeacherId, 'teacher']);
            ...
            const existingCheck = await pool_1.default.query('SELECT id FROM suggested_teachers WHERE teacher_id = $1 AND suggested_teacher_id = $2', [teacherId, suggestedTeacherId]);
            ...
            const result = await pool_1.default.query(`INSERT INTO suggested_teachers (teacher_id, suggested_teacher_id)
         VALUES ($1, $2)
         RETURNING id, teacher_id, suggested_teacher_id, created_at`, [teacherId, suggestedTeacherId]);
     ...
     * حذف مدرس مقترح
     */
    static async removeSuggestedTeacher(teacherId, suggestedTeacherId) {
        try {
            const result = await pool_1.default.query('DELETE FROM suggested_teachers WHERE teacher_id = $1 AND suggested_teacher_id = $2', [teacherId, suggestedTeacherId]);
     ...
     */
    static async getSuggestedTeachers(teacherId) {
        try {
         ...
         ORDER BY st.created_at DESC`, [teacherId]);
     ...
     */
    static async getSuggestedTeachersForStudent(teacherId) {
        try {
            const result = await pool_1.default.query(`SELECT 
          u.id,
          u.name,
...
exports.SuggestedTeachersService = SuggestedTeachersService;
