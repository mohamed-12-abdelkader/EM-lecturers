...
class SubjectService {
    // Create new subject
    static async create(questionBankId, data, createdBy) {
        ...
        const bankResult = await pool_1.default.query(bankQuery, [questionBankId]);
        ...
        const existsResult = await pool_1.default.query(existsQuery, [questionBankId, name]);
    ...
    static async getByQuestionBank(questionBankId, is_active) {
    ...
    // Update subject
    static async update(questionBankId, subjectId, data) {
        const { name, description, image_url, color, is_active } = data;
        ...
        if (name !== undefined) {
            ...
            const dupResult = await pool_1.default.query(dupQuery, [questionBankId, name, subjectId]);
    ...
    `;
        const verifyResult = await pool_1.default.query(verifyQuery, [subjectId, questionBankId]);
    ...
    // Delete subject
    static async delete(questionBankId, subjectId) {
        // Verify subject exists and belongs to the question bank
        const verifyQuery = `
      SELECT id FROM subjects 
      WHERE id = $1 AND question_bank_id = $2
    ...
    `;
        ...
        try {
         ...
         WHERE q.lesson_id = l.id AND l.chapter_id = c.id AND c.subject_id = $1`, [subjectId]);
         ...
         WHERE l.chapter_id = c.id AND c.subject_id = $1`, [subjectId]);
            ...
            const result = await pool_1.default.query(`DELETE FROM subjects WHERE id = $1 AND question_bank_id = $2`, [subjectId, questionBankId]);
    ...
    static async getTeacherSubjects(teacherId) {
      ...
      ORDER BY s.name ASC
    `;
        const result = await pool_1.default.query(query, [teacherId]);
        return result.rows.map((subject) => ({
            ...subject,
            created_at: new Date(subject.created_at),
            updated_at: new Date(subject.updated_at),
