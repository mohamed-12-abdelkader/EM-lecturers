...
exports.AnalyticsIntelligenceService = void 0;
...
class AnalyticsIntelligenceService {
    static async getCourseAnalytics(scope, courseId, range) {
        const { from, to } = resolveRange(range);
        const [enrollment, videoStats, completionStats, lectureCompletion, topStudents] = await Promise.all([
            pool_1.default.query(`SELECT COUNT(*)::text AS total_students
         FROM enrollments e
         JOIN users u ON u.id = e.user_id
         WHERE e.course_id = $1
           AND u.role = 'student'
           AND u.tenant_id = $2`, [courseId, scope.tenantId]),
         ...
         WHERE vv.course_id = $1
           AND u.tenant_id = $2
           AND vv.updated_at BETWEEN $3 AND $4`, [courseId, scope.tenantId, from, to]),
            pool_1.default.query(`WITH per_student AS (
           SELECT
             vv.user_id,
         ...
         FROM per_student`, [courseId, scope.tenantId, from, to]),
            pool_1.default.query(`SELECT
           l.id AS lecture_id,
         ...
         ORDER BY l.position ASC, l.id ASC`, [courseId, scope.tenantId, from, to]),
            pool_1.default.query(`SELECT
           u.id AS student_id,
         ...
         LIMIT 20`, [courseId, scope.tenantId, from, to]),
        ]);
    ...
    static async getLectureAnalytics(scope, lectureId, range) {
        const { from, to } = resolveRange(range);
        const [stats, replayHeatmap, skipHeatmap] = await Promise.all([
         ...
         GROUP BY l.id, l.title`, [lectureId, from, to, scope.tenantId]),
            pool_1.default.query(`SELECT
         ...
         ORDER BY bucket_second`, [scope.tenantId, from, to, lectureId]),
            pool_1.default.query(`SELECT
    ...
    static async getStudentAnalytics(scope, studentId, range) {
        const { from, to } = resolveRange(range);
        const [profile, lectureStats, weeklyActivity, examStats] = await Promise.all([
         ...
         WHERE u.id = $1
           AND u.role = 'student'
           AND u.tenant_id = $2
         LIMIT 1`, [studentId, scope.tenantId]),
            pool_1.default.query(`SELECT
           l.id AS lecture_id,
           l.title AS lecture_title,
         ...
         GROUP BY l.id, l.title
         ORDER BY MAX(vv.updated_at) DESC`, [studentId, from, to]),
            pool_1.default.query(`SELECT COUNT(DISTINCT DATE(occurred_at))::text AS active_days
         FROM analytics_student_activity_logs
         WHERE tenant_id = $1
           AND student_id = $2
           AND occurred_at >= NOW() - INTERVAL '7 days'`, [scope.tenantId, studentId]),
            pool_1.default.query(`SELECT
         ...
         WHERE tenant_id = $1
           AND student_id = $2
           AND created_at BETWEEN $3 AND $4`, [scope.tenantId, studentId, from, to]),
        ]);
    ...
    static async getExamAnalytics(scope, examId, range) {
        const { from, to } = resolveRange(range);
        const legacyStats = await pool_1.default.query(`SELECT
         COUNT(*)::text AS attempts,
       ...
       WHERE es.exam_id = $1
         AND u.tenant_id = $2
         AND es.submitted_at BETWEEN $3 AND $4`, [examId, scope.tenantId, from, to]);
    ...
    static async getDifficultQuestions(scope, limit = 20) {
    ...
    static async getTopStudents(scope, limit = 20) {
        const result = await pool_1.default.query(`WITH watch AS (
         SELECT
    ...
    static async getAtRiskStudents(scope, limit = 20) {
        const result = await pool_1.default.query(`WITH latest_activity AS (
         SELECT
           u.id AS student_id,
    ...
    static async getPerformanceSummary(scope) {
        const [students, activeWeekly, completion, difficultQuestions] = await Promise.all([
            pool_1.default.query(`SELECT COUNT(*)::text AS total_students
         FROM users
         WHERE role = 'student'
           AND tenant_id = $1`, [scope.tenantId]),
            pool_1.default.query(`SELECT COUNT(DISTINCT student_id)::text AS active_students
         FROM analytics_student_activity_logs
         WHERE tenant_id = $1
           AND occurred_at >= NOW() - INTERVAL '7 days'`, [scope.tenantId]),
            pool_1.default.query(`SELECT COALESCE(AVG(completion_percentage), 0)::text AS avg_completion
         FROM video_views vv
         JOIN users u ON u.id = vv.user_id
         WHERE u.tenant_id = $1`, [scope.tenantId]),
...
exports.AnalyticsIntelligenceService = AnalyticsIntelligenceService;
