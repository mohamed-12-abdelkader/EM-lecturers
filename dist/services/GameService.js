...
exports.GameService = exports.GAME_CONFIG = void 0;
...
exports.GAME_CONFIG = {
    ...
    APPROVED_QUESTION_STATUS: ['approved', null, ''],
...
class GameService {
     ...
     */
    static async createInvitation(inviterId, inviteeId, lessonIds, questionsCount = exports.GAME_CONFIG.DEFAULT_QUESTIONS_COUNT) {
        console.log(`[createInvitation] Creating invitation with inviterId: ${inviterId} (type: ${typeof inviterId}), inviteeId: ${inviteeId}`);
        ...
        try {
            const lessonsCheck = await pool_1.default.query(`SELECT l.id 
         FROM lessons l
         WHERE l.id = ANY($1::INTEGER[])
           AND EXISTS (
            ...
            if (error.message?.includes('timeout') || error.message?.includes('Connection')) {
                ...
                const simpleCheck = await pool_1.default.query(`SELECT id FROM lessons WHERE id = ANY($1::INTEGER[])`, [lessonIds]);
       ...
       WHERE ((inviter_id = $1::INTEGER AND invitee_id = $2::INTEGER) OR (inviter_id = $2::INTEGER AND invitee_id = $1::INTEGER))
        ...
        try {
            ...
            result = await pool_1.default.query(`INSERT INTO game_invitations (inviter_id, invitee_id, lesson_ids, questions_count, expires_at)
         VALUES ($1::INTEGER, $2::INTEGER, $3::TEXT[], $4::INTEGER, NOW() + INTERVAL '3 minutes')
         RETURNING *`, [inviterId, inviteeId, lessonIdsAsText, questionsCount]);
            ...
            console.log(`[createInvitation] Saved lesson_ids (type):`, typeof savedRow.lesson_ids, Array.isArray(savedRow.lesson_ids));
            // Verify the data was actually saved by querying it back
            const verifyCheck = await pool_1.default.query(`SELECT lesson_ids, selected_lessons FROM game_invitations WHERE id = $1::INTEGER`, [savedRow.id]);
            ...
            if (error.message?.includes('lesson_ids') || error.message?.includes('column')) {
                console.log(`[createInvitation] lesson_ids column not found, using selected_lessons with:`, lessonIds);
                result = await pool_1.default.query(`INSERT INTO game_invitations (inviter_id, invitee_id, selected_lessons, questions_count, expires_at)
           VALUES ($1::INTEGER, $2::INTEGER, $3::INTEGER[], $4::INTEGER, NOW() + INTERVAL '3 minutes')
           RETURNING *`, [inviterId, inviteeId, lessonIds, questionsCount]);
                const savedRow = result.rows[0];
                console.log(`[createInvitation] Successfully created invitation using selected_lessons, id: ${savedRow.id}`);
                ...
                if (savedRow.selected_lessons && Array.isArray(savedRow.selected_lessons)) {
                    savedRow.lesson_ids = savedRow.selected_lessons.map((id) => String(id));
                    console.log(`[createInvitation] Mapped selected_lessons to lesson_ids:`, savedRow.lesson_ids);
                }
                return savedRow;
            }
            else {
     ...
     */
    static async createBulkInvitations(inviterId, inviteeIds, lessonIds, questionsCount = exports.GAME_CONFIG.DEFAULT_QUESTIONS_COUNT) {
        console.log(`[createBulkInvitations] Creating bulk invitations for ${inviteeIds.length} students`);
        const results = [];
        // التحقق من أن جميع الطلاب المدعوين موجودون
        const inviteeCheck = await pool_1.default.query('SELECT id, name FROM users WHERE id = ANY($1::INTEGER[]) AND role = $2', [inviteeIds, 'student']);
           ...
           WHERE ((inviter_id = $1::INTEGER AND invitee_id = $2::INTEGER) OR (inviter_id = $2::INTEGER AND invitee_id = $1::INTEGER))
             ...
             AND expires_at > NOW()`, [inviterId, inviteeId]);
                ...
                try {
                    ...
                    result = await pool_1.default.query(`INSERT INTO game_invitations (inviter_id, invitee_id, lesson_ids, questions_count, expires_at)
             VALUES ($1::INTEGER, $2::INTEGER, $3::TEXT[], $4::INTEGER, NOW() + INTERVAL '3 minutes')
             RETURNING id, inviter_id, invitee_id, lesson_ids, questions_count`, [inviterId, inviteeId, lessonIdsAsText, questionsCount]);
                    console.log(`[createBulkInvitations] Successfully created invitation for student ${inviteeId}, id: ${result.rows[0].id}`);
               ...
               RETURNING *`, [inviterId, inviteeId, lessonIds, questionsCount]);
                        ...
                        if (result.rows[0] && result.rows[0].selected_lessons) {
                            result.rows[0].lesson_ids = result.rows[0].selected_lessons.map((id) => String(id));
                        }
                        console.log(`[createBulkInvitations] Successfully created invitation using selected_lessons for student ${inviteeId}, id: ${result.rows[0].id}`);
                    }
                    else {
                        throw error;
                    }
                ...
                console.error(`[createBulkInvitations] Error creating invitation for student ${inviteeId}:`, error);
                results.push({
                    inviteeId,
                    success: false,
        ...
        console.log(`[createBulkInvitations] Completed: ${results.filter((r) => r.success).length} successful, ${results.filter((r) => !r.success).length} failed`);
        return results;
    }
    /**
    ...
    static async acceptInvitation(invitationId, inviteeId) {
        ...
        const invitationResult = await pool_1.default.query(`SELECT id, inviter_id, invitee_id, lesson_ids, selected_lessons, 
              questions_count, status, created_at, expires_at, 
              accepted_at, rejected_at,
              (expires_at > NOW()) as is_not_expired,
              (expires_at < NOW()) as is_expired
       FROM game_invitations 
       WHERE id = $1::INTEGER AND invitee_id = $2::INTEGER`, [invitationId, inviteeId]);
        ...
        if (invitation.status === 'accepted') {
            ...
            const roomCheck = await pool_1.default.query(`SELECT * FROM game_rooms WHERE invitation_id = $1`, [
                invitationId,
            ]);
        ...
        const isNotExpired = invitation.is_not_expired === true ||
        ...
        if (invitation.status === 'pending' && isNotExpired) {
            console.log(`[acceptInvitation] Invitation ${invitationId} is pending and not expired - proceeding with normal acceptance`);
        ...
        // إذا كانت الدعوة pending ولكن انتهت - نتحقق من وجود طلاب آخرين قبلوا
            ...
            console.log(`[acceptInvitation] Invitation ${invitationId} is expired - checking for other accepted invitations`);
            ...
            const otherAcceptedInvitations = await pool_1.default.query(`SELECT id FROM game_invitations 
         WHERE inviter_id = $1::INTEGER
           ...
           AND id != $5::INTEGER`, [
            ...
            console.log(`[acceptInvitation] Found ${otherAcceptedInvitations.rowCount} other accepted invitations in the same group`);
        ...
        const lessonIdsCheck = await pool_1.default.query(`SELECT lesson_ids, selected_lessons,
       ...
       FROM game_invitations 
       WHERE id = $1::INTEGER`, [invitationId]);
        ...
        if (rawData?.lesson_ids !== null && rawData?.lesson_ids !== undefined) {
            if (Array.isArray(rawData.lesson_ids)) {
                ...
                try {
                    ...
                    console.error(`[acceptInvitation] Failed to parse lesson_ids string:`, rawData.lesson_ids);
                }
            }
        }
       ...
       WHERE id = $1`, [invitationId]);
        ...
        try {
            ...
            roomResult = await pool_1.default.query(`INSERT INTO game_rooms (invitation_id, player1_id, player2_id, questions_count, total_questions, time_per_question, total_time)
         ...
         RETURNING *`, [
            ...
            if (error.message?.includes('total_questions')) {
                ...
                try {
                    roomResult = await pool_1.default.query(`INSERT INTO game_rooms (invitation_id, player1_id, player2_id, questions_count, time_per_question, total_time)
             ...
             RETURNING *`, [
                    ...
                    if (error2.message?.includes('questions_count')) {
                        ...
                        roomResult = await pool_1.default.query(`INSERT INTO game_rooms (invitation_id, player1_id, player2_id, total_questions, time_per_question, total_time)
               VALUES ($1::INTEGER, $2::INTEGER, $3::INTEGER, $4::INTEGER, $5::INTEGER, $6::INTEGER)
               RETURNING *`, [
                            invitationId,
                            invitation.inviter_id,
                ...
                console.warn(`[acceptInvitation] questions_count column not found, trying with total_questions only`);
                try {
                    roomResult = await pool_1.default.query(`INSERT INTO game_rooms (invitation_id, player1_id, player2_id, total_questions, time_per_question, total_time)
             VALUES ($1::INTEGER, $2::INTEGER, $3::INTEGER, $4::INTEGER, $5::INTEGER, $6::INTEGER)
             RETURNING *`, [
                    ...
                    roomResult = await pool_1.default.query(`INSERT INTO game_rooms (invitation_id, player1_id, player2_id, time_per_question, total_time)
        ...
        try {
            ...
            console.warn(`[acceptInvitation] Room ${room.id} created but questions generation failed. Questions can be generated later via API.`);
        }
        return room;
    }
    // رفض الدعوة
    static async rejectInvitation(invitationId, inviteeId) {
        const result = await pool_1.default.query(`UPDATE game_invitations 
       SET status = 'rejected', rejected_at = NOW() 
       WHERE id = $1 AND invitee_id = $2 AND status = 'pending'`, [invitationId, inviteeId]);
        if (!result.rowCount) {
     ...
     */
    static async generateGameQuestions(roomId, lessonIds, questionsCount) {
        ...
        const questionsCheck = await pool_1.default.query(`SELECT COUNT(*) as total, 
              COUNT(CASE WHEN q.status IS NULL OR q.status = '' OR q.status = 'approved' THEN 1 END) as approved_count
       FROM questions q
       WHERE q.lesson_id = ANY($1::INTEGER[])`, [lessonIdsInt]);
        ...
        const questionsResult = await pool_1.default.query(`SELECT q.id, q.text, q.options::text as options_text, q.options as options_json, 
              q.image, q.correct_answer, q.points
       FROM questions q
       WHERE q.lesson_id = ANY($1::INTEGER[])
         AND (q.status IS NULL OR q.status = '' OR q.status = 'approved')
       ORDER BY RANDOM()
       LIMIT $2::INTEGER`, [lessonIdsInt, questionsCount]);
        console.log(`[generateGameQuestions] ✅ Found ${questionsResult.rowCount} questions from database`);
        ...
        if (questionsResult.rowCount === 0) {
            if (totalQuestions === 0) {
                ...
                throw new Error(`لا توجد أسئلة مقبولة في الدروس المحددة. ` +
        ...
        if (questionsResult.rowCount && questionsResult.rowCount < questionsCount) {
            console.warn(`[generateGameQuestions] ⚠️ Only ${questionsResult.rowCount} questions available ` +
                `(requested ${questionsCount})`);
        }
        // إدراج الأسئلة في جدول game_questions
        console.log(`[generateGameQuestions] Inserting ${questionsResult.rows.length} questions into game_questions`);
        for (let i = 0; i < questionsResult.rows.length; i++) {
            ...
            try {
                ...
                if (question.options_json) {
                    if (typeof question.options_json === 'object' && question.options_json !== null) {
                        ...
                        if (Array.isArray(question.options_json)) {
                            ...
                            console.log(`[generateGameQuestions] Converted options array to object:`, optionsJson);
                        }
                        else {
                ...
                try {
                    if (optionsJson) {
                        try {
                            ...
                            if (typeof optionsJson === 'object' && !Array.isArray(optionsJson)) {
                                ...
                                console.warn(`[generateGameQuestions] optionsJson is not a valid object, skipping options`);
                                optionsValue = null;
                            }
                        }
                        catch (e) {
                    ...
                    // محاولة INSERT مع جميع الأعمدة المحتملة
                    await pool_1.default.query(`INSERT INTO game_questions (room_id, question_id, question_order, question_text, question_image, options, correct_answer, points, time_limit)
             VALUES ($1::INTEGER, $2::INTEGER, $3::INTEGER, $4::TEXT, $5::TEXT, $6::jsonb, $7::TEXT, $8::INTEGER, $9::INTEGER)`, [
                        ...
                        question.points || 1,
                        exports.GAME_CONFIG.DEFAULT_QUESTION_TIME_LIMIT, // time_limit from config
                    ]);
                }
                catch (insertError) {
                    ...
                    if (insertError.message?.includes('time_limit')) {
                        ...
                        try {
                            await pool_1.default.query(`INSERT INTO game_questions (room_id, question_id, question_order, question_text, question_image, options, correct_answer, points)
                 VALUES ($1::INTEGER, $2::INTEGER, $3::INTEGER, $4::TEXT, $5::TEXT, $6::jsonb, $7::TEXT, $8::INTEGER)`, [
                            ...
                            if (insertError2.message?.includes('points') ||
                                ...
                                await pool_1.default.query(`INSERT INTO game_questions (room_id, question_id, question_order, question_text, question_image, options, correct_answer)
                   VALUES ($1::INTEGER, $2::INTEGER, $3::INTEGER, $4::TEXT, $5::TEXT, $6::jsonb, $7::TEXT)`, [
                    ...
                    else if (insertError.message?.includes('points') ||
                        insertError.message?.includes('column')) {
                        console.warn(`[generateGameQuestions] points column not found, inserting without it`);
                        try {
                            await pool_1.default.query(`INSERT INTO game_questions (room_id, question_id, question_order, question_text, question_image, options, correct_answer, time_limit)
                 VALUES ($1::INTEGER, $2::INTEGER, $3::INTEGER, $4::TEXT, $5::TEXT, $6::jsonb, $7::TEXT, $8::INTEGER)`, [
                                roomId,
                                question.id,
                                i + 1,
                                ...
                                optionsValue,
                                String(question.correct_answer ?? ''),
                                exports.GAME_CONFIG.DEFAULT_QUESTION_TIME_LIMIT,
                            ]);
                        }
                        catch (insertError3) {
                   ...
                   VALUES ($1::INTEGER, $2::INTEGER, $3::INTEGER, $4::TEXT, $5::TEXT, $6::jsonb, $7::TEXT)`, [
                        ...
                        console.warn(`[generateGameQuestions] JSON error with options, inserting without options:`, insertError.message);
                        try {
                            await pool_1.default.query(`INSERT INTO game_questions (room_id, question_id, question_order, question_text, question_image, correct_answer, points, time_limit)
                 VALUES ($1::INTEGER, $2::INTEGER, $3::INTEGER, $4::TEXT, $5::TEXT, $6::TEXT, $7::INTEGER, $8::INTEGER)`, [
                            ...
                            if (insertError2.message?.includes('points')) {
                                try {
                                    await pool_1.default.query(`INSERT INTO game_questions (room_id, question_id, question_order, question_text, question_image, correct_answer, time_limit)
                     VALUES ($1::INTEGER, $2::INTEGER, $3::INTEGER, $4::TEXT, $5::TEXT, $6::TEXT, $7::INTEGER)`, [
                                    ...
                                    if (insertError3.message?.includes('time_limit')) {
                                        await pool_1.default.query(`INSERT INTO game_questions (room_id, question_id, question_order, question_text, question_image, correct_answer)
                       VALUES ($1::INTEGER, $2::INTEGER, $3::INTEGER, $4::TEXT, $5::TEXT, $6::TEXT)`, [
                                ...
                                await pool_1.default.query(`INSERT INTO game_questions (room_id, question_id, question_order, question_text, question_image, correct_answer, points)
                   VALUES ($1::INTEGER, $2::INTEGER, $3::INTEGER, $4::TEXT, $5::TEXT, $6::TEXT, $7::INTEGER)`, [
                                    roomId,
                                    question.id,
                                    i + 1,
                ...
                console.log(`[generateGameQuestions] Inserted question ${i + 1}/${questionsResult.rows.length}: question_id=${question.id}`);
            }
            catch (error) {
        ...
        console.log(`[generateGameQuestions] Successfully generated ${questionsResult.rows.length} questions for room ${roomId}`);
    }
    // بدء اللعبة
    static async startGame(roomId) {
        const result = await pool_1.default.query(`UPDATE game_rooms 
    ...
    // تسجيل إجابة اللاعب
    static async submitAnswer(roomId, questionId, playerId, answer, timeTaken) {
        // التحقق من أن اللاعب جزء من الغرفة
        const roomCheck = await pool_1.default.query(`SELECT id FROM game_rooms WHERE id = $1 AND (player1_id = $2 OR player2_id = $2)`, [roomId, playerId]);
        if (!roomCheck.rowCount) {
            throw new Error('اللاعب غير مسجل في هذه الغرفة');
        }
        // جلب السؤال للتحقق من الإجابة الصحيحة
        const questionResult = await pool_1.default.query(`SELECT correct_answer FROM game_questions WHERE id = $1 AND room_id = $2`, [questionId, roomId]);
        ...
        // تسجيل الإجابة
        const answerResult = await pool_1.default.query(`INSERT INTO game_answers (room_id, question_id, player_id, answer, is_correct, time_taken)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`, [roomId, questionId, playerId, answer, isCorrect, timeTaken]);
        return answerResult.rows[0];
    }
    // إنهاء اللعبة وحساب النتائج
    static async endGame(roomId) {
        // التحقق من أن الغرفة نشطة
        const roomResult = await pool_1.default.query(`SELECT * FROM game_rooms WHERE id = $1 AND status = 'active'`, [roomId]);
        if (!roomResult.rowCount) {
            throw new Error('الغرفة غير نشطة');
        }
        const room = roomResult.rows[0];
        // جلب إجابات اللاعبين
        const answersResult = await pool_1.default.query(`SELECT player_id, answer, is_correct, time_taken
       FROM game_answers 
       WHERE room_id = $1`, [roomId]);
        ...
        // تحديث حالة الغرفة
        await pool_1.default.query(`UPDATE game_rooms 
       SET status = 'completed', completed_at = NOW()
       WHERE id = $1`, [roomId]);
        // إنشاء النتيجة
        const resultQuery = await pool_1.default.query(`INSERT INTO game_results (
      ...
      RETURNING *`, [
        ...
        await this.updatePlayerStats(room.player1_id, player1Score, player2Score, winnerId === room.player1_id, isTie);
        await this.updatePlayerStats(room.player2_id, player2Score, player1Score, winnerId === room.player2_id, isTie);
        return resultQuery.rows[0];
    }
    // تحديث إحصائيات اللاعب
    static async updatePlayerStats(playerId, playerScore, opponentScore, isWinner, isTie) {
        await pool_1.default.query(`INSERT INTO player_game_stats (player_id, total_games, games_won, games_lost, games_tied, total_score, total_correct_answers, total_questions_answered, last_played_at)
    ...
    static async getIncomingInvitations(playerId) {
        try {
            const result = await pool_1.default.query(`SELECT gi.*, u.name as inviter_name
       FROM game_invitations gi
       JOIN users u ON u.id = gi.inviter_id
       WHERE gi.invitee_id = $1 AND gi.status = 'pending' AND gi.expires_at > NOW()
       ORDER BY gi.created_at DESC`, [playerId]);
            // Ensure lesson_ids field exists and handle both column names
            // Also fetch lesson names
            const invitationsWithLessons = await Promise.all(result.rows.map(async (row) => {
    ...
    static async getOutgoingInvitations(playerId) {
        try {
            console.log(`[getOutgoingInvitations] Getting outgoing invitations for playerId: ${playerId} (type: ${typeof playerId})`);
            ...
            const allInvitationsRaw = await pool_1.default.query(`SELECT * FROM game_invitations ORDER BY created_at DESC`);
            console.log(`[getOutgoingInvitations] Total invitations in DB: ${allInvitationsRaw.rowCount}`);
            ...
            const matchingInvitations = allInvitationsRaw.rows.filter((row) => {
                ...
                const matches = rowInviterId === playerId ||
            ...
            console.log(`[getOutgoingInvitations] Found ${matchingInvitations.length} matching invitations after JavaScript filter`);
            if (matchingInvitations.length === 0) {
                console.log(`[getOutgoingInvitations] No matches found. Sample inviter_ids in DB:`, [
                    ...new Set(allInvitationsRaw.rows.slice(0, 10).map((r) => ({
            ...
            const usersResult = await pool_1.default.query(`SELECT id, name FROM users WHERE id = ANY($1::INTEGER[])`, [inviteeIds]);
            ...
            console.log(`[getOutgoingInvitations] Built ${invitationsWithNames.length} invitations with names`);
            if (invitationsWithNames.length > 0) {
                ...
                try {
                    const invitationsWithLessons = await Promise.all(result.rows.map(async (row) => {
                        try {
                            ...
                            const lessonIdsArray = Array.isArray(lessonIds)
                                ? lessonIds.map((id) => parseInt(String(id))).filter((id) => !isNaN(id))
                                : [];
                            console.log(`[getOutgoingInvitations] Processing invitation ${row.id}, lessonIds:`, lessonIdsArray);
                            // Fetch lesson names if we have lesson IDs
                            let lessonNames = [];
                            if (lessonIdsArray.length > 0) {
                                try {
                                    ...
                                    console.log(`[getOutgoingInvitations] Fetched ${lessonNames.length} lesson names for invitation ${row.id}`);
                                }
                                catch (error) {
                                    console.error(`Error fetching lesson names for invitation ${row.id}:`, error);
                                }
                    ...
                    console.log(`[getOutgoingInvitations] Returning ${invitationsWithLessons.length} processed invitations`);
                    return invitationsWithLessons;
                }
                catch (processError) {
    ...
    // جلب تفاصيل الغرفة
    static async getRoomDetails(roomId, playerId) {
        try {
            const result = await pool_1.default.query(`SELECT id, invitation_id, player1_id, player2_id, status, questions_count, 
                time_per_question, total_time, current_question, started_at, completed_at, created_at
         FROM game_rooms 
         WHERE id = $1::INTEGER AND (player1_id = $2::INTEGER OR player2_id = $2::INTEGER)`, [roomId, playerId]);
            if (result.rowCount === 0) {
                console.log(`[getRoomDetails] Room ${roomId} not found or player ${playerId} is not a participant`);
                return null;
            }
    ...
    // جلب أسئلة الغرفة
    static async getRoomQuestions(roomId) {
        ...
        try {
            ...
            let result;
            try {
                result = await pool_1.default.query(`SELECT id, room_id, question_id, question_order, question_text, question_image, 
                  options::jsonb as options, points, created_at
           FROM game_questions 
           WHERE room_id = $1::INTEGER
           ORDER BY question_order ASC`, [roomId]);
            }
            catch (error) {
            ...
            if (result.rowCount === 0) {
                ...
                const roomCheck = await pool_1.default.query(`SELECT id, invitation_id FROM game_rooms WHERE id = $1::INTEGER`, [roomId]);
                if (roomCheck.rowCount && roomCheck.rowCount > 0) {
                    console.warn(`[getRoomQuestions] Room ${roomId} exists but has no questions. This might indicate questions were not generated.`);
                }
                else {
    ...
    static async getRoomAnswers(roomId) {
        const result = await pool_1.default.query(`SELECT * FROM game_answers 
       WHERE room_id = $1 
       ORDER BY answered_at`, [roomId]);
        return result.rows;
    }
    // جلب نتيجة اللعبة
    static async getGameResult(roomId) {
        const result = await pool_1.default.query(`SELECT * FROM game_results WHERE room_id = $1`, [roomId]);
        return result.rows[0] || null;
    }
    // جلب إحصائيات اللاعب
    static async getPlayerStats(playerId) {
        const result = await pool_1.default.query(`SELECT * FROM player_game_stats WHERE player_id = $1`, [
            playerId,
        ]);
        return (result.rows[0] || {
            total_games: 0,
