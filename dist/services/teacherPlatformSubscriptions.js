...
exports.TeacherPlatformSubscriptionsService = exports.DEFAULT_GRACE_PERIOD_DAYS = exports.DEFAULT_EXPIRY_ALERT_DAYS = void 0;
const pool_1 = __importDefault(require("../db/pool"));
const utils_1 = require("../utils");
const accounting_1 = require("./accounting");
const financialAudit_1 = require("./financialAudit");
const teacherCustomPricing_1 = require("./teacherCustomPricing");
const teacherSubscriptionInvoices_1 = require("./teacherSubscriptionInvoices");
...
function resolvePaymentAmounts(actualPrice, paidAmountInput) {
    const paid = roundMoney(paidAmountInput ?? actualPrice);
    if (paid < 0) {
        throw new utils_1.HttpError(400, 'المبلغ المدفوع لا يمكن أن يكون سالباً');
    }
    if (paid > actualPrice + 0.001) {
        throw new utils_1.HttpError(400, 'المبلغ المدفوع لا يمكن أن يتجاوز قيمة الاشتراك');
    }
    const remaining = roundMoney(Math.max(0, actualPrice - paid));
    const payment_status = remaining <= 0 ? 'paid' : paid > 0 ? 'partial' : 'unpaid';
...
async function insertSubscriptionPayment(client, input) {
    const result = await client.query(`INSERT INTO teacher_subscription_payments (
...
async function applyPaymentToOpenInvoice(client, subscriptionId, paymentAmount, paymentMethod, incomeId) {
    await client.query(`UPDATE teacher_subscription_invoices i
     ...
     WHERE i.id = latest.id`, [paymentAmount, paymentMethod ?? null, incomeId ?? null, subscriptionId]);
}
exports.DEFAULT_EXPIRY_ALERT_DAYS = 3;
exports.DEFAULT_GRACE_PERIOD_DAYS = 3;
function addDays(dateStr, days) {
    const d = new Date(dateStr);
    d.setUTCDate(d.getUTCDate() + days);
    return d.toISOString().slice(0, 10);
}
async function generateSubscriptionNumber(client) {
    const year = new Date().getFullYear();
    const result = await client.query(`SELECT COUNT(*)::text AS n FROM teacher_platform_subscriptions
     WHERE EXTRACT(YEAR FROM created_at) = $1`, [year]);
    const seq = Number(result.rows[0]?.n ?? 0) + 1;
    return `SUB-${year}-${String(seq).padStart(6, '0')}`;
}
function arabicDayCount(n) {
    if (n === 0)
        return 'اليوم';
    if (n === 1)
        return 'يوم واحد';
...
async function reactivateTeacherPlatform(client, teacherId) {
    await client.query(`UPDATE tenants SET is_active = true, updated_at = NOW()
     WHERE owner_user_id = $1`, [teacherId]);
}
...
class TeacherPlatformSubscriptionsService {
    static async syncExpiredStatuses() {
        await pool_1.default.query(`UPDATE teacher_platform_subscriptions
       SET status = 'expired', updated_at = NOW()
       WHERE status = 'active' AND ends_at < CURRENT_DATE`);
    }
    /** Mark expired → restore grace → suspend past grace & deactivate tenant */
    static async syncSubscriptionLifecycle(graceDays = exports.DEFAULT_GRACE_PERIOD_DAYS) {
    ...
    static async restoreTeachersInGracePeriod(graceDays) {
        const grace = Math.min(Math.max(graceDays, 1), 30);
        await pool_1.default.query(`WITH ${LAST_LAPSED_SUBSCRIPTION_CTE},
       ...
       WHERE tn.owner_user_id = t.teacher_id
         AND tn.is_active = false`, [grace]);
    }
    static async suspendTeachersPastGracePeriod(graceDays) {
        const grace = Math.min(Math.max(graceDays, 1), 30);
        await pool_1.default.query(`WITH ${LAST_LAPSED_SUBSCRIPTION_CTE},
       ...
       FROM teachers_past_grace t
       WHERE tn.owner_user_id = t.teacher_id
         AND tn.is_active = true`, [grace]);
    }
    static async create(input, actorId) {
        const client = await pool_1.default.connect();
        try {
            await client.query('BEGIN');
            const pricing = await teacherCustomPricing_1.TeacherCustomPricingService.resolvePrice(input.teacher_id, input.plan_id);
            ...
            const endsAt = input.ends_at ?? addDays(startsAt, Number(pricing.plan.duration_days));
            if (endsAt < startsAt) {
            ...
            await client.query(`UPDATE teacher_platform_subscriptions
         SET status = 'cancelled', updated_at = NOW()
         WHERE teacher_id = $1 AND status <> 'cancelled'`, [input.teacher_id]);
            const subResult = await client.query(`INSERT INTO teacher_platform_subscriptions (
         ...
         RETURNING *`, [
            ...
            if (payment.paid_amount > 0) {
                const income = await accounting_1.AccountingService.addIncomeWithClient(client, {
                ...
                incomeId = income.id;
                await client.query(`UPDATE teacher_platform_subscriptions SET income_id = $1 WHERE id = $2`, [income.id, subscription.id]);
            ...
            await teacherSubscriptionInvoices_1.TeacherSubscriptionInvoicesService.createInTransaction(client, {
                teacher_id: input.teacher_id,
                subscription_id: subscription.id,
         ...
         WHERE id = $3 AND role = 'teacher'`, [pricing.plan.code, startsAt, input.teacher_id]);
            await reactivateTeacherPlatform(client, input.teacher_id);
            if (payment.paid_amount > 0) {
                await (0, financialAudit_1.recordFinancialTransaction)({
    ...
    static async renew(subscriptionId, input, actorId) {
        const client = await pool_1.default.connect();
        try {
            await client.query('BEGIN');
            ...
            const pricing = await teacherCustomPricing_1.TeacherCustomPricingService.resolvePrice(sub.teacher_id, planId);
            const actualPrice = input.actual_price ?? pricing.price;
            const payment = resolvePaymentAmounts(actualPrice, input.paid_amount);
            const periodStart = sub.status === 'active' && sub.ends_at >= new Date().toISOString().slice(0, 10)
                ? addDays(sub.ends_at, 1)
                : new Date().toISOString().slice(0, 10);
            const periodEnd = addDays(periodStart, Number(pricing.plan.duration_days));
            const renewalResult = await client.query(`INSERT INTO teacher_subscription_renewals (
         ...
         RETURNING *`, [
            ...
            if (payment.paid_amount > 0) {
                ...
                await client.query(`UPDATE teacher_subscription_renewals SET income_id = $1 WHERE id = $2`, [income.id, renewal.id]);
                await insertSubscriptionPayment(client, {
         ...
         WHERE id = $11`, [
            ...
            await client.query(`UPDATE users
         SET subscription_package = $1, subscription_package_assigned_at = $2::date
         WHERE id = $3 AND role = 'teacher'`, [pricing.plan.code, periodStart, sub.teacher_id]);
    ...
    static async loadSubscriptionForUpgrade(subscriptionId) {
        const subRes = await pool_1.default.query(`SELECT s.*, p.code AS plan_code, p.name_ar AS plan_name, p.sort_order AS plan_sort_order
       ...
       WHERE s.id = $1`, [subscriptionId]);
        ...
        if (sub.status !== 'active') {
            throw new utils_1.HttpError(400, 'يمكن ترقية الاشتراك النشط فقط');
        }
        const today = new Date().toISOString().slice(0, 10);
        const endsAt = typeof sub.ends_at === 'string'
            ? sub.ends_at.slice(0, 10)
            : toDateString(sub.ends_at);
        if (endsAt && endsAt < today) {
            throw new utils_1.HttpError(400, 'انتهت فترة الاشتراك — استخدم التجديد بدلاً من الترقية');
        }
        return sub;
    }
    static assertHigherPlan(fromCode, toCode, fromSort, toSort) {
        const fromPkg = fromCode;
        const toPkg = toCode;
        const byCode = (0, teacherPlanPolicy_1.packageLevel)(fromPkg) >= 0 &&
            (0, teacherPlanPolicy_1.packageLevel)(toPkg) >= 0 &&
            (0, teacherPlanPolicy_1.packageLevel)(toPkg) > (0, teacherPlanPolicy_1.packageLevel)(fromPkg);
        if (byCode)
            return;
        if (toSort > fromSort)
            return;
        throw new utils_1.HttpError(400, 'يمكن الترقية إلى باقة أعلى فقط');
    }
    static async getUpgradeQuote(subscriptionId, planId, actualPrice) {
        const sub = await this.loadSubscriptionForUpgrade(subscriptionId);
        if (sub.plan_id === planId) {
            throw new utils_1.HttpError(400, 'المدرس مشترك بالفعل في هذه الباقة');
        }
        const newPricing = await teacherCustomPricing_1.TeacherCustomPricingService.resolvePrice(sub.teacher_id, planId);
        this.assertHigherPlan(sub.plan_code, newPricing.plan.code, Number(sub.plan_sort_order), Number(newPricing.plan.sort_order));
        ...
        if (upgradeAmount <= 0) {
            throw new utils_1.HttpError(400, 'قيمة الباقة الجديدة يجب أن تكون أعلى من قيمة الاشتراك الحالي');
        }
        ...
        return {
            ...
            after_upgrade: {
                ...
                payment_status: afterRemaining <= 0 ? 'paid' : afterPaid > 0 ? 'partial' : 'unpaid',
            },
        };
    }
    static async upgrade(subscriptionId, input, actorId) {
        const quote = await this.getUpgradeQuote(subscriptionId, input.plan_id, input.actual_price);
        const client = await pool_1.default.connect();
        try {
            await client.query('BEGIN');
            const subRes = await client.query(`SELECT * FROM teacher_platform_subscriptions WHERE id = $1 FOR UPDATE`, [subscriptionId]);
            const sub = subRes.rows[0];
            const newPricing = await teacherCustomPricing_1.TeacherCustomPricingService.resolvePrice(sub.teacher_id, input.plan_id);
            ...
            const newStatus = newRemainingTotal <= 0 ? 'paid' : newPaidTotal > 0 ? 'partial' : 'unpaid';
            const upgradeDate = new Date().toISOString().slice(0, 10);
            const startsAt = quote.period.starts_at;
            const endsAt = quote.period.ends_at;
            const upgradeResult = await client.query(`INSERT INTO teacher_subscription_upgrades (
         ...
         RETURNING *`, [
            ...
            if (upgradePayment.paid_amount > 0) {
                ...
                incomeId = income.id;
                await client.query(`UPDATE teacher_subscription_upgrades SET income_id = $1 WHERE id = $2`, [income.id, upgrade.id]);
                await insertSubscriptionPayment(client, {
                    subscription_id: subscriptionId,
                    teacher_id: sub.teacher_id,
         ...
         WHERE id = $2 AND role = 'teacher'`, [newPricing.plan.code, sub.teacher_id]);
            await teacherSubscriptionInvoices_1.TeacherSubscriptionInvoicesService.createInTransaction(client, {
                teacher_id: sub.teacher_id,
                subscription_id: subscriptionId,
                upgrade_id: upgrade.id,
    ...
    static async recordPayment(subscriptionId, input, actorId) {
        if (input.amount <= 0) {
            throw new utils_1.HttpError(400, 'مبلغ الدفع يجب أن يكون أكبر من صفر');
        }
        const client = await pool_1.default.connect();
        try {
            await client.query('BEGIN');
            const subRes = await client.query(`SELECT s.*, p.code AS plan_code, p.name_ar AS plan_name
         FROM teacher_platform_subscriptions s
         JOIN teacher_subscription_plans p ON p.id = s.plan_id
         WHERE s.id = $1
         FOR UPDATE`, [subscriptionId]);
            ...
            if (remaining <= 0) {
                throw new utils_1.HttpError(400, 'لا يوجد مبلغ متبقي على هذا الاشتراك');
            }
         ...
         WHERE id = $5`, [
            ...
            await applyPaymentToOpenInvoice(client, subscriptionId, paymentAmount, input.payment_method ?? null, income.id);
            await (0, financialAudit_1.recordFinancialTransaction)({
    ...
    static async listOutstandingBalances(pagination = {}) {
        await this.syncExpiredStatuses();
        ...
        const summaryResult = await pool_1.default.query(`SELECT COALESCE(SUM(s.remaining_amount), 0)::text AS total_outstanding,
              COUNT(*)::text AS count
       FROM teacher_platform_subscriptions s
       ${where}`, values);
        const listResult = await pool_1.default.query(`SELECT s.*, p.code AS plan_code, p.name_ar AS plan_name,
    ...
    static async updateStatus(id, status, actorId, notes) {
        const existing = await pool_1.default.query(`SELECT * FROM teacher_platform_subscriptions WHERE id = $1`, [
            id,
        ]);
        if (!existing.rowCount)
            throw new Error('Subscription not found');
        const result = await pool_1.default.query(`UPDATE teacher_platform_subscriptions
       SET status = $1, notes = COALESCE($2, notes), updated_at = NOW()
       WHERE id = $3
       RETURNING *`, [status, notes ?? null, id]);
        await financialAudit_1.FinancialAuditService.log({
            entity_type: 'teacher_platform_subscription',
            entity_id: id,
            action: 'update',
    ...
    static async getById(id) {
        const result = await pool_1.default.query(`SELECT s.*,
       ...
       WHERE s.id = $1`, [id]);
        if (!result.rowCount)
            return null;
        const renewals = await pool_1.default.query(`SELECT r.*, p.code AS plan_code, p.name_ar AS plan_name, u.name AS renewed_by_name
       ...
       ORDER BY r.created_at DESC`, [id]);
        const invoices = await teacherSubscriptionInvoices_1.TeacherSubscriptionInvoicesService.list({
            subscription_id: id,
            limit: 100,
            offset: 0,
        });
        const payments = await pool_1.default.query(`SELECT p.*, u.name AS created_by_name
       FROM teacher_subscription_payments p
       LEFT JOIN users u ON u.id = p.created_by
       WHERE p.subscription_id = $1
       ORDER BY p.payment_date DESC, p.id DESC`, [id]);
        const upgrades = await pool_1.default.query(`SELECT u.*,
    ...
    static async list(filters = {}) {
        ...
        if (filters.search) {
            conditions.push(`(s.subscription_number ILIKE $${i} OR t.name ILIKE $${i} OR t.email ILIKE $${i})`);
    ...
    static async listExpiringSoon(days = exports.DEFAULT_EXPIRY_ALERT_DAYS, pagination = {}) {
        await this.syncExpiredStatuses();
       ...
       WHERE s.status = 'active'
         AND s.ends_at BETWEEN CURRENT_DATE AND CURRENT_DATE + $1::int`, [withinDays]);
        const listResult = await pool_1.default.query(`SELECT s.*, p.code AS plan_code, p.name_ar AS plan_name,
              t.name AS teacher_name, t.email AS teacher_email,
       ...
       LIMIT $2 OFFSET $3`, [withinDays, limit, offset]);
        return {
            days: withinDays,
    ...
    static async getPlatformAccessState(teacherId, graceDays = exports.DEFAULT_GRACE_PERIOD_DAYS) {
        const grace = Math.min(Math.max(graceDays, 1), 30);
        const result = await pool_1.default.query(`WITH active_sub AS (
         SELECT 1 AS ok
         FROM teacher_platform_subscriptions
         WHERE teacher_id = $1
           AND status = 'active'
       ...
       FROM lapsed l`, [teacherId, grace]);
        ...
        const graceDaysRemaining = row.grace_days_remaining == null ? null : Number(row.grace_days_remaining);
        if (row.has_active) {
            return {
                allowed: true,
                phase: 'active',
    ...
    static async getTeacherExpiryAlert(teacherId, days = exports.DEFAULT_EXPIRY_ALERT_DAYS, graceDays = exports.DEFAULT_GRACE_PERIOD_DAYS) {
        const withinDays = Math.min(Math.max(days, 1), 90);
        const grace = Math.min(Math.max(graceDays, 1), 30);
        await this.syncSubscriptionLifecycle(grace);
        const expiringResult = await pool_1.default.query(`SELECT s.id, s.subscription_number, s.plan_id,
       ...
       LIMIT 1`, [teacherId, withinDays]);
        ...
        const lapsedResult = await pool_1.default.query(`WITH ${LAST_LAPSED_SUBSCRIPTION_CTE}
        ...
        if (isPastGrace) {
            return {
                ...
                alert: {
                    ...
                    message: 'تم إيقاف تنشيط منصتك بشكل نهائي لعدم تجديد الاشتراك. يرجى التواصل مع الإدارة لتجديد الباقة وإعادة التفعيل.',
                },
            };
        }
        return { show_alert: false, alert: null };
...
exports.TeacherPlatformSubscriptionsService = TeacherPlatformSubscriptionsService;
