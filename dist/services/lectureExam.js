...
exports.LectureExamService = void 0;
const pool_1 = __importDefault(require("../db/pool"));
class LectureExamService {
    /**
     * إنشاء امتحان MCQ جديد للمحاضرة
     */
    static async createExam(lectureId, title, totalGrade, createdBy, options) {
        const { type = 'exam', duration = null, isVisible = false, showAt = null, hideAt = null, lockNextLectures = false, showAnswersImmediately = true, showAnswersAfterHours = 0, } = options || {};
        const examType = typeof type === 'string' && type.trim().toLowerCase() === 'assignment'
            ? 'assignment'
            : 'exam';
     ...
     */
    static async updateExam(examId, updates, updatedBy) {
        const setClause = [];
        const values = [];
        let paramIndex = 1;
        ...
        const result = await pool_1.default.query(`UPDATE exams 
       SET ${setClause.join(', ')}
    ...
    static async deleteExam(examId, createdBy) {
        const result = await pool_1.default.query('DELETE FROM exams WHERE id = $1 AND created_by = $2', [
            examId,
            createdBy,
        ]);
     ...
     */
    static async getExamsByLectureId(lectureId, userId, userRole) {
        let query = `
      SELECT e.*, 
             COUNT(DISTINCT q.id) as questions_count,
             COUNT(DISTINCT s.id) as submissions_count
     ...
     */
    static async canStudentAccessExam(examId, studentId) {
        const now = new Date();
        const result = await pool_1.default.query(`SELECT e.*, l.id as lecture_id
       FROM exams e
       ...
       AND e.is_visible = true`, [examId, studentId, now]);
        return result.rowCount > 0;
    }
    /**
     ...
     */
    static async canStudentAccessNextLectures(lectureId, studentId) {
        const now = new Date();
        ...
        const examResult = await pool_1.default.query(`SELECT e.id FROM exams e
       JOIN lectures l ON l.id = e.lecture_id
       JOIN courses c ON c.id = l.course_id
       JOIN enrollments en ON en.course_id = c.id
       WHERE e.lecture_id = $1 
       ...
       AND (e.show_at IS NULL OR e.show_at <= $3)
       AND (e.hide_at IS NULL OR e.hide_at >= $3)`, [lectureId, studentId, now]);
        console.log('Debug - Exam result:', examResult.rows.length);
        // فحص إضافي لمعرفة سبب عدم وجود امتحان مانع
        if (examResult.rowCount === 0) {
            const debugExamResult = await pool_1.default.query(`SELECT e.id, e.is_visible, e.lock_next_lectures, e.show_at, e.hide_at,
                CASE 
         ...
         JOIN enrollments en ON en.course_id = c.id
         WHERE e.lecture_id = $1 
         AND en.user_id = $2`, [lectureId, studentId, now]);
        ...
        const passResult = await pool_1.default.query(`SELECT s.passed FROM exam_submissions s
       WHERE s.exam_id = $1 AND s.student_id = $2
       ORDER BY s.submitted_at DESC LIMIT 1`, [examResult.rows[0].id, studentId]);
        console.log('Debug - Pass result:', passResult.rows.length > 0 ? passResult.rows[0] : 'No submission');
     ...
     */
    static async debugBlockingExams(lectureId, studentId) {
        const now = new Date();
        console.log(`Debug - debugBlockingExams for lecture ${lectureId}:`);
        console.log('Debug - Student ID:', studentId);
        console.log('Debug - Current time:', now);
        // فحص جميع الامتحانات في المحاضرات السابقة
        const allExamsResult = await pool_1.default.query(`SELECT e.id, e.title, e.is_visible, e.lock_next_lectures, 
       ...
       ORDER BY l.position DESC, l.created_at DESC NULLS LAST, l.id DESC`, [lectureId, studentId, now]);
        // فحص الامتحانات المانعة للوصول فقط
        const blockingExamsResult = await pool_1.default.query(`SELECT e.id, e.title, e.is_visible, e.lock_next_lectures, 
              e.show_at, e.hide_at, l.title as lecture_title, l.position
       FROM lectures l
       JOIN courses c ON c.id = l.course_id
       JOIN enrollments en ON en.course_id = c.id
       ...
       ORDER BY l.position DESC, l.created_at DESC NULLS LAST, l.id DESC`, [lectureId, studentId, now]);
        return {
            ...
            logic_explanation: {
                exam_not_taken: 'إذا لم يخضع الطالب للامتحان بعد، المحاضرات التالية مقفلة (لأن الامتحان ظاهر)',
     ...
     */
    static async canStudentAccessLecture(lectureId, studentId) {
        const now = new Date();
        ...
        const blockingExamResult = await pool_1.default.query(`SELECT l.id as lecture_id, l.position
       FROM lectures l
       JOIN courses c ON c.id = l.course_id
       JOIN enrollments en ON en.course_id = c.id
       JOIN exams e ON e.lecture_id = l.id
       ...
       ORDER BY l.position DESC, l.created_at DESC NULLS LAST, l.id DESC`, [lectureId, studentId, now]);
        ...
        if (blockingExamResult.rowCount === 0) {
            ...
            const allExamsDebugResult = await pool_1.default.query(`SELECT e.id, e.title, e.is_visible, e.lock_next_lectures, 
                e.show_at, e.hide_at, l.title as lecture_title, l.position,
                CASE 
                  WHEN e.is_visible = false THEN 'hidden_by_teacher'
                  WHEN e.show_at IS NOT NULL AND e.show_at > $3 THEN 'not_yet_visible'
     ...
     */
    static async getBlockingExamsForLecture(lectureId, studentId) {
        const now = new Date();
        console.log(`Debug - getBlockingExamsForLecture for lecture ${lectureId}:`);
        console.log('Debug - Student ID:', studentId);
        console.log('Debug - Current time:', now);
        const result = await pool_1.default.query(`SELECT e.id, e.title, e.total_grade, l.title as lecture_title, l.position,
              CASE 
     ...
     */
    static async isExamVisibleToStudent(examId, studentId) {
        const now = new Date();
        const result = await pool_1.default.query(`SELECT e.id FROM exams e
       JOIN lectures l ON l.id = e.lecture_id
       JOIN courses c ON c.id = l.course_id
       JOIN enrollments en ON en.course_id = c.id
       WHERE e.id = $1 
       ...
       AND e.is_visible = true
       AND (e.show_at IS NULL OR e.show_at <= $3)
       AND (e.hide_at IS NULL OR e.hide_at >= $3)`, [examId, studentId, now]);
        return result.rowCount > 0;
    }
    /**
     * التحقق من إمكانية إظهار الإجابات للطالب
     */
    static async canStudentSeeAnswers(examId, studentId) {
        const result = await pool_1.default.query(`SELECT e.show_answers_immediately, e.show_answers_after_hours,
              s.submitted_at
       FROM exams e
       LEFT JOIN exam_submissions s ON s.exam_id = e.id AND s.student_id = $2
       WHERE e.id = $1`, [examId, studentId]);
        ...
        const hoursSinceSubmission = (Date.now() - new Date(exam.submitted_at).getTime()) / (1000 * 60 * 60);
        return hoursSinceSubmission >= exam.show_answers_after_hours;
    }
}
exports.LectureExamService = LectureExamService;
