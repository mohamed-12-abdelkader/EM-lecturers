...
class AssignmentSubmissionsService {
    ...
    static async submitAssignment(assignmentId, studentId, data) {
        ...
        try {
            ...
            const assignmentCheck = await client.query('SELECT id FROM package_subject_item_lesson_assignments WHERE id = $1', [assignmentId]);
            if (!assignmentCheck.rowCount) {
                throw new Error('الواجب غير موجود');
            }
            // التحقق من أن الطالب لم يسلم الواجب من قبل
            const existingSubmission = await client.query('SELECT id FROM assignment_submissions WHERE assignment_id = $1 AND student_id = $2', [assignmentId, studentId]);
            ...
            const questionsResult = await client.query(`SELECT q.id, q.correct_option_id, q.correct_answer 
         FROM assignment_questions q 
         WHERE q.assignment_id = $1 
         ORDER BY q.order_index ASC, q.created_at ASC`, [assignmentId]);
            ...
            if (optionsArray.length > 0) {
                const optionsResult = await client.query(`SELECT id, option_text, option_letter, question_id 
           FROM assignment_question_options 
           WHERE id = ANY($1::int[])`, [optionsArray]);
                optionsMap = new Map(optionsResult.rows.map((opt) => [opt.id, opt]));
            }
            // حساب النتائج
            ...
            for (const question of questions) {
                ...
                if (studentOption.question_id !== question.id) {
                    throw new Error(`الخيار المحدد (option_id: ${studentOptionId}) لا ينتمي للسؤال ${question.id}`);
                }
            ...
            // إدراج التسليم
            const submissionResult = await client.query(`INSERT INTO assignment_submissions 
         (assignment_id, student_id, total_questions, correct_answers, wrong_answers, score)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING *`, [assignmentId, studentId, totalQuestions, correctAnswers, wrongAnswers, scoreRounded]);
            ...
            if (submissionAnswers.length > 0) {
                ...
                await client.query(`INSERT INTO assignment_submission_answers 
    ...
    static async getStudentSubmission(assignmentId, studentId) {
        const submissionResult = await pool_1.default.query(`SELECT * FROM assignment_submissions 
       WHERE assignment_id = $1 AND student_id = $2`, [assignmentId, studentId]);
        if (!submissionResult.rowCount) {
            return null;
        }
        const submission = submissionResult.rows[0];
        // جلب إجابات الطالب مع تفاصيل الأسئلة والخيارات
        const answersResult = await pool_1.default.query(`SELECT 
         aqa.id,
         aqa.question_id,
         aqa.student_answer,
       ...
       ORDER BY aq.order_index ASC, aq.created_at ASC`, [submission.id]);
        ...
        for (const answer of answers) {
         ...
         FROM assignment_question_options 
         WHERE question_id = $1 
         ORDER BY order_index ASC, option_letter ASC`, [answer.question_id]);
            answer.options = optionsResult.rows;
            // جلب معلومات الخيار المختار
            if (answer.student_option_id) {
                const studentOptionResult = await pool_1.default.query('SELECT id, option_text, option_letter FROM assignment_question_options WHERE id = $1', [answer.student_option_id]);
                answer.student_option = studentOptionResult.rows[0] || null;
            }
            // جلب معلومات الخيار الصحيح
            if (answer.correct_option_id) {
                const correctOptionResult = await pool_1.default.query('SELECT id, option_text, option_letter FROM assignment_question_options WHERE id = $1', [answer.correct_option_id]);
           ...
           WHERE question_id = $1 
           ORDER BY order_index ASC`, [answer.question_id]);
    ...
    static async getSubmissionById(submissionId) {
        const submissionResult = await pool_1.default.query('SELECT * FROM assignment_submissions WHERE id = $1', [submissionId]);
        if (!submissionResult.rowCount) {
            return null;
        }
        const submission = submissionResult.rows[0];
       ...
       JOIN assignment_questions aq ON aqa.question_id = aq.id
       WHERE aqa.submission_id = $1
       ORDER BY aq.order_index ASC, aq.created_at ASC`, [submissionId]);
        const answers = answersResult.rows;
        // جلب الخيارات والصور لكل سؤال
        for (const answer of answers) {
            // جلب جميع خيارات السؤال
    ...
    static async hasStudentSubmitted(assignmentId, studentId) {
        const result = await pool_1.default.query('SELECT id FROM assignment_submissions WHERE assignment_id = $1 AND student_id = $2', [assignmentId, studentId]);
        // eslint-disable-next-line @typescript-eslint/ban-ts-comment
        // @ts-ignore
