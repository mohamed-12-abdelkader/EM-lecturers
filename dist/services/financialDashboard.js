...
const teacherPlatformSubscriptions_1 = require("./teacherPlatformSubscriptions");
...
class FinancialDashboardService {
    static async getDashboard(period) {
        await teacherPlatformSubscriptions_1.TeacherPlatformSubscriptionsService.syncExpiredStatuses();
        const { start_date, end_date } = periodToDates(period);
        const stats = await accounting_1.AccountingService.getFinancialStats({ start_date, end_date });
        const [activeSubs, expiredSubs, recentRenewals, topPlans, topTeachers, expiringSoon, outstanding] = await Promise.all([
            pool_1.default.query(`SELECT COUNT(*)::text AS c FROM teacher_platform_subscriptions WHERE status = 'active'`),
            pool_1.default.query(`SELECT COUNT(*)::text AS c FROM teacher_platform_subscriptions WHERE status = 'expired'`),
            pool_1.default.query(`SELECT r.*, s.subscription_number, t.name AS teacher_name, p.name_ar AS plan_name
         ...
         LIMIT 10`),
            pool_1.default.query(`SELECT p.code, p.name_ar, COALESCE(SUM(ft.amount), 0)::float AS total_revenue
         ...
         ORDER BY total_revenue DESC`, start_date && end_date ? [start_date, end_date] : start_date ? [start_date] : end_date ? [end_date] : []),
            pool_1.default.query(`SELECT t.id, t.name, t.email, COALESCE(SUM(ft.amount), 0)::float AS total_revenue
         FROM platform_financial_transactions ft
         ...
         LIMIT 10`, start_date && end_date ? [start_date, end_date] : start_date ? [start_date] : end_date ? [end_date] : []),
            teacherPlatformSubscriptions_1.TeacherPlatformSubscriptionsService.listExpiringSoon(3, { limit: 20, offset: 0 }),
            teacherPlatformSubscriptions_1.TeacherPlatformSubscriptionsService.listOutstandingBalances({ limit: 10, offset: 0 }),
        ]);
        const renewalRevenue = await pool_1.default.query(`SELECT COALESCE(SUM(amount), 0)::text AS total
       ...
       ${end_date ? `AND transaction_date <= $${start_date ? 2 : 1}` : ''}`, start_date && end_date ? [start_date, end_date] : start_date ? [start_date] : end_date ? [end_date] : []);
        return {
            period: period ?? 'all',
    ...
    static async reportRevenue(filters) {
        ...
        const conditions = [`ft.direction = 'in'`];
        const values = [];
        let i = 1;
        ...
        const result = await pool_1.default.query(`SELECT ${selectGroup},
       ...
       WHERE ${where}
       GROUP BY ${groupSql}
       ORDER BY total_amount DESC`, values);
    ...
    static async reportExpenses(filters) {
        ...
        const byCategory = await pool_1.default.query(`SELECT category, COALESCE(SUM(amount), 0)::float AS total, COUNT(*)::int AS count
       FROM platform_expenses
       WHERE ($1::date IS NULL OR transaction_date >= $1)
         AND ($2::date IS NULL OR transaction_date <= $2)
         AND ($3::text IS NULL OR category = $3)
       GROUP BY category
       ORDER BY total DESC`, [filters.start_date ?? null, filters.end_date ?? null, filters.category ?? null]);
        return { expenses, by_category: byCategory.rows };
    }
    static async reportProfit(period) {
    ...
    static async reportSubscriptions(filters) {
        await teacherPlatformSubscriptions_1.TeacherPlatformSubscriptionsService.syncExpiredStatuses();
        const list = await teacherPlatformSubscriptions_1.TeacherPlatformSubscriptionsService.list({
    ...
    static async addExpenseWithAudit(data, actorId) {
        const expense = await accounting_1.AccountingService.addExpense(data, actorId);
    ...
    static async updateExpenseWithAudit(id, patch, actorId) {
        const existing = await pool_1.default.query(`SELECT * FROM platform_expenses WHERE id = $1`, [id]);
        ...
        if (patch.expense_type !== undefined)
            add('expense_type', patch.expense_type);
        if (patch.payment_method !== undefined)
            add('payment_method', patch.payment_method);
        if (patch.transaction_date !== undefined)
            add('transaction_date', patch.transaction_date);
        if (!fields.length)
            return existing.rows[0];
        values.push(id);
        const result = await pool_1.default.query(`UPDATE platform_expenses SET ${fields.join(', ')}, updated_at = NOW() WHERE id = $${i} RETURNING *`, values);
        await financialAudit_1.FinancialAuditService.log({
            entity_type: 'platform_expense',
            entity_id: id,
            action: 'update',
    ...
    static async deleteExpenseWithAudit(id, actorId) {
        const existing = await pool_1.default.query(`SELECT * FROM platform_expenses WHERE id = $1`, [id]);
        if (!existing.rowCount)
            throw new Error('Expense not found');
        await accounting_1.AccountingService.deleteExpense(id);
        await financialAudit_1.FinancialAuditService.log({
            entity_type: 'platform_expense',
            entity_id: id,
            action: 'delete',
