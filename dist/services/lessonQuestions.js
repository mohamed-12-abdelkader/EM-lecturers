...
exports.LessonQuestionsService = void 0;
const pool_1 = __importDefault(require("../db/pool"));
const utils_1 = require("../utils");
class LessonQuestionsService {
    // إنشاء أسئلة دفعة واحدة للدرس
    static async createBulkQuestionsForLesson(lectureId, bulkText) {
        ...
        for (const q of questions) {
            ...
            const questionResult = await pool_1.default.query(`INSERT INTO questions (text, type) VALUES ($1, 'single_choice') RETURNING id`, [q.text]);
            ...
            const lessonQuestionResult = await pool_1.default.query(`INSERT INTO lesson_questions (lecture_id, question_text, grade, question_id) VALUES ($1, $2, $3, $4) RETURNING id`, [lectureId, q.text, 1, questionId]);
    ...
    static async getLessonQuestionsFormatted(lessonId) {
       ...
       WHERE l.id = $1`, [lessonId]);
        ...
        try {
            const newTableResult = await pool_1.default.query(`SELECT 
          q.id,
          q.text,
          q.image,
          q.correct_answer,
         ...
         FROM questions q
         WHERE q.lesson_id = $1
         ORDER BY q.id`, [lessonId]);
            console.log(`الجدول الجديد: ${newTableResult.rowCount} سؤال`);
            if (newTableResult.rowCount && newTableResult.rowCount > 0) {
                console.log('الأسئلة من الجدول الجديد:', newTableResult.rows.map((r) => ({ id: r.id, text: r.text, image: r.image })));
                const newQuestions = newTableResult.rows.map((row) => {
                    ...
                    if (row.question_options && typeof row.question_options === 'object') {
                        ...
                        else if (row.question_options.options &&
        ...
        try {
            const imageQuestionsResult = await pool_1.default.query(`SELECT 
          q.id,
          q.text,
          q.image,
          q.correct_answer,
         ...
         AND q.image != ''
         ORDER BY q.id`, [lessonId]);
            console.log(`الأسئلة الصورية من questions مع lesson_id: ${imageQuestionsResult.rowCount} سؤال`);
            if (imageQuestionsResult.rowCount && imageQuestionsResult.rowCount > 0) {
                console.log('الأسئلة الصورية:', imageQuestionsResult.rows.map((r) => ({ id: r.id, text: r.text, image: r.image })));
        ...
        try {
            const oldTableResult = await pool_1.default.query(`SELECT 
          lq.id,
        ...
        try {
            const examQuestionsResult = await pool_1.default.query(`SELECT 
          eq.id,
        ...
        uniqueQuestions.sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime());
        console.log(`إجمالي الأسئلة المُرجعَة (بعد إزالة التكرار): ${uniqueQuestions.length}`);
        console.log('الأسئلة:', uniqueQuestions.map((q) => ({ id: q.id, text: q.text, image: q.image })));
        return uniqueQuestions;
    }
    // جلب أسئلة درس معين
    static async getLessonQuestions(lectureId) {
        // التحقق من وجود الدرس
        const lectureCheck = await pool_1.default.query(`SELECT l.* FROM lectures l WHERE l.id = $1`, [
            lectureId,
        ]);
       ...
       LEFT JOIN question_choices qc ON q.id = qc.question_id
       WHERE lq.lecture_id = $1
       ORDER BY lq.id, qc.id`, [lectureId]);
        if (!result.rowCount) {
            return [];
        }
        // تنظيم البيانات
        ...
        result.rows.forEach((row) => {
            if (!questionsMap.has(row.id)) {
                ...
                const hasText = (row.text && row.text.trim() !== '') ||
            ...
            if (row.choice_id && row.choice_text) {
                ...
                const isDuplicate = existingChoices.some((choice) => choice.id === row.choice_id);
                if (!isDuplicate) {
    ...
    static async createTextQuestionsForQuestionBankLesson(bulkText, lessonId) {
        // التحقق من وجود الدرس في بنك الأسئلة
        const lessonCheck = await pool_1.default.query(`SELECT l.*, c.subject_id, s.question_bank_id 
       FROM lessons l 
       JOIN chapters c ON l.chapter_id = c.id 
       ...
       WHERE l.id = $1`, [lessonId]);
        ...
        for (const block of questionBlocks) {
            const lines = block
                ...
                .filter((l) => l.length > 0);
            if (lines.length < 5)
                continue; // يجب أن يحتوي على سؤال + 4 اختيارات على الأقل
            const questionText = lines[0];
            const choices = lines.slice(1, 5).map((line) => {
                // إزالة A) B) C) D) من بداية السطر
                return line.replace(/^[A-D]\)\s*/, '');
            });
            // إنشاء السؤال في جدول questions
            const questionResult = await pool_1.default.query(`INSERT INTO questions (text, type, lesson_id, options, correct_answer) 
         VALUES ($1, $2, $3, $4, $5) 
     ...
     */
    static async createBulkMcqForQuestionBankLesson(bulkText, lessonId, 
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    teacherId) {
        const lessonCheck = await pool_1.default.query(`SELECT l.id, c.subject_id, s.question_bank_id 
       FROM lessons l 
       JOIN chapters c ON l.chapter_id = c.id 
       JOIN subjects s ON c.subject_id = s.id 
       WHERE l.id = $1`, [lessonId]);
        ...
        for (let blockIndex = 0; blockIndex < questionBlocks.length; blockIndex++) {
         ...
         RETURNING id, text, correct_answer`, [questionText, lessonId, JSON.stringify(options), correctAnswer]);
        ...
        if (invalidBlocks.length > 0 && inserted.length === 0) {
            throw new Error(`لم يُعرَف أي سؤال صالح. تأكد أن كل كتلة تحتوي على: سؤال ثم أربعة أسطر أ) ب) ج) د)، واختياريًا سطر "الإجابة الصحيحة: أ/ب/ج/د". كتل بها خطأ: ${invalidBlocks.join(', ')}`);
        }
        return { inserted: inserted.length, questions: inserted };
    }
    // إنشاء أسئلة بالصور للدرس في بنك الأسئلة
    static async createImageQuestionsForQuestionBankLesson(files, lessonId) {
        const questions = [];
        // التحقق من وجود الدرس في بنك الأسئلة
        const lessonCheck = await pool_1.default.query(`SELECT l.*, c.subject_id, s.question_bank_id 
       FROM lessons l 
       ...
       WHERE l.id = $1`, [lessonId]);
        ...
        for (const file of files) {
            ...
            let questionResult;
            try {
                questionResult = await pool_1.default.query(`INSERT INTO questions (text, type, image, lesson_id, options, correct_answer) 
           VALUES ($1, $2, $3, $4, $5, $6) 
           RETURNING id, created_at, COALESCE(updated_at, created_at) as updated_at`, [
                ...
                if (error.message.includes('column "lesson_id" does not exist')) {
                    ...
                    questionResult = await pool_1.default.query(`INSERT INTO questions (text, type, image) 
             VALUES ($1, $2, $3) 
             RETURNING id, created_at`, [
                    ...
                    for (const choice of choices) {
                        await pool_1.default.query(`INSERT INTO question_choices (question_id, text, is_correct) VALUES ($1, $2, $3)`, [questionResult.rows[0].id, choice, false]);
                    }
                    // إضافة السؤال إلى جدول lesson_questions
                    await pool_1.default.query(`INSERT INTO lesson_questions (lecture_id, question_text, question_image, question_id) 
             VALUES ($1, $2, $3, $4)`, [lessonId, '', uploaded.secure_url, questionResult.rows[0].id]);
                    ...
                    questionResult = await pool_1.default.query(`INSERT INTO lesson_questions (lecture_id, question_text, question_image) 
             VALUES ($1, $2, $3) 
             RETURNING id, created_at, updated_at`, [lessonId, '', uploaded.secure_url]);
                }
            }
            const questionId = questionResult.rows[0].id;
            const createdAt = questionResult.rows[0].created_at;
    ...
    static async createImageQuestionsForLesson(files, lectureId) {
        const questions = [];
        for (const file of files) {
            // رفع الصورة على Cloudinary
            const uploaded = await (0, utils_1.uploadToCloudinary)(file.path);
            // إنشاء السؤال في جدول questions
            const questionResult = await pool_1.default.query(`INSERT INTO questions (text, type, image) VALUES ($1, 'single_choice', $2) RETURNING id`, ['', uploaded.secure_url]);
            const questionId = questionResult.rows[0].id;
            ...
            if (lectureId) {
                ...
                const lessonQuestionResult = await pool_1.default.query(`INSERT INTO lesson_questions (lecture_id, question_text, grade, question_id, question_image) VALUES ($1, $2, $3, $4, $5) RETURNING id`, [lectureId, '', 1, questionId, uploaded.secure_url]);
                const lessonQuestionId = lessonQuestionResult.rows[0].id;
                questions.push({
                    id: lessonQuestionId,
                    lesson_id: lectureId,
                ...
                const choicesResult = await pool_1.default.query(`SELECT id, text, is_correct FROM question_choices WHERE question_id = $1 ORDER BY id`, [questionId]);
                questions.push({
                    id: questionId,
                    text: '',
                    image: uploaded.secure_url,
    ...
    static async updateLessonQuestion(questionId, questionText, _grade, image) {
        ...
        const result = await pool_1.default.query(`UPDATE lesson_questions SET ${fields.join(', ')} WHERE id = $${i} RETURNING *`, values);
        if (!result.rowCount) {
    ...
    // تحديد الإجابة الصحيحة
    static async setLessonQuestionCorrectAnswer(questionId, correctChoiceId) {
        let actualQuestionId = questionId;
        let isDirectQuestion = false;
        // أولاً، تحقق إذا كان السؤال موجود مباشرة في جدول questions
        const directQuestionRes = await pool_1.default.query(`SELECT id FROM questions WHERE id = $1`, [
            questionId,
        ]);
        if (directQuestionRes.rowCount && directQuestionRes.rowCount > 0) {
            ...
            const lessonQuestionRes = await pool_1.default.query(`SELECT question_id FROM lesson_questions WHERE id = $1`, [questionId]);
        ...
        if (isDirectQuestion) {
            ...
            const questionRes = await pool_1.default.query(`SELECT options FROM questions WHERE id = $1`, [
                actualQuestionId,
            ]);
            ...
            if (Array.isArray(options) && correctChoiceId >= 0 && correctChoiceId < options.length) {
                ...
                await pool_1.default.query(`UPDATE questions SET correct_answer = $1 WHERE id = $2`, [
            ...
            const choiceCheckRes = await pool_1.default.query(`SELECT id FROM question_choices WHERE id = $1 AND question_id = $2`, [correctChoiceId, actualQuestionId]);
            ...
            await pool_1.default.query(`UPDATE question_choices SET is_correct = true WHERE id = $1 AND question_id = $2`, [correctChoiceId, actualQuestionId]);
        }
        return { message: 'تم تحديث الإجابة الصحيحة بنجاح' };
    }
    // جلب اختيارات السؤال
    static async getLessonQuestionChoices(questionId) {
        // أولاً، تحقق إذا كان السؤال موجود مباشرة في جدول questions
        const directQuestionRes = await pool_1.default.query(`SELECT id, options FROM questions WHERE id = $1`, [
            questionId,
        ]);
        if (directQuestionRes.rowCount && directQuestionRes.rowCount > 0) {
            // السؤال موجود مباشرة في جدول questions
        ...
        const choicesRes = await pool_1.default.query(`SELECT id, text, is_correct FROM question_choices WHERE question_id = $1 ORDER BY id`, [actualQuestionId]);
        return {
            question_id: questionId,
            choices: choicesRes.rows.map((choice) => ({
                id: choice.id,
                text: choice.text,
    ...
    static async updateQuestionFromLesson(questionId, updateData) {
        // أولاً، تحقق إذا كان السؤال موجود مباشرة في جدول questions
        const directQuestionRes = await pool_1.default.query(`SELECT id, lesson_id FROM questions WHERE id = $1`, [questionId]);
        if (directQuestionRes.rowCount && directQuestionRes.rowCount > 0) {
            // السؤال موجود مباشرة في جدول questions
            // eslint-disable-next-line @typescript-eslint/no-unused-vars
            const question = directQuestionRes.rows[0];
            ...
            if (updateFields.length > 0) {
                ...
                await pool_1.default.query(`UPDATE questions SET ${updateFields.join(', ')} WHERE id = $${paramCount}`, updateValues);
            }
    ...
    static async deleteQuestionFromLesson(questionId) {
        // أولاً، تحقق إذا كان السؤال موجود مباشرة في جدول questions
        const directQuestionRes = await pool_1.default.query(`SELECT id, lesson_id FROM questions WHERE id = $1`, [questionId]);
        if (directQuestionRes.rowCount && directQuestionRes.rowCount > 0) {
            // السؤال موجود مباشرة في جدول questions
            ...
            await pool_1.default.query(`DELETE FROM questions WHERE id = $1`, [questionId]);
        ...
        await pool_1.default.query(`DELETE FROM question_choices WHERE question_id = $1`, [actualQuestionId]);
        // حذف السؤال من جدول الأسئلة
        await pool_1.default.query(`DELETE FROM questions WHERE id = $1`, [actualQuestionId]);
        // حذف السؤال من جدول lesson_questions
        await pool_1.default.query(`DELETE FROM lesson_questions WHERE id = $1`, [questionId]);
        return { message: 'تم حذف السؤال بنجاح' };
    }
    // حذف سؤال (للتوافق مع الإصدارات السابقة)
    static async deleteLessonQuestion(questionId) {
        ...
        // حذف السؤال من جدول lesson_questions
        const result = await pool_1.default.query(`DELETE FROM lesson_questions WHERE id = $1 RETURNING *`, [
            questionId,
        ]);
        if (!result.rowCount) {
            throw new Error('السؤال غير موجود');
    ...
    static async getLessonDetails(lectureId) {
        // جلب بيانات الدرس
        const lectureRes = await pool_1.default.query('SELECT * FROM lectures WHERE id = $1', [lectureId]);
...
exports.LessonQuestionsService = LessonQuestionsService;
