...
exports.DataAnalystChatbotService = void 0;
const pool_1 = __importDefault(require("../db/pool"));
const utils_1 = require("../utils");
const dataAnalyst_prompts_1 = require("./dataAnalyst.prompts");
const dataAnalystReports_1 = require("./dataAnalystReports");
const dataAnalystExamReports_1 = require("./dataAnalystExamReports");
...
function parseIdAfterPrefixes(text, prefixes) {
    ...
    for (const prefix of prefixes) {
        ...
        if (!rest)
            continue;
        if (/^\d+$/.test(rest))
            return { type: 'id', value: Number(rest) };
        return { type: 'name', value: rest };
    }
    return null;
}
function extractCourseId(text) {
...
function parseCourseReportRequest(text) {
    const parsed = parseIdAfterPrefixes(text, COURSE_REPORT_PREFIXES);
    if (parsed)
        return parsed;
    const match = text.match(/(?:كورس|course)\s*[#:]?\s*(\d+)/i);
    if (match)
        return { type: 'id', value: Number(match[1]) };
    return null;
}
...
function isGeneralReportRequest(text) {
    if ((0, dataAnalystExamReports_1.isExamAnalysisRequest)(text))
        return false;
    if (parseIdAfterPrefixes(text, STUDENT_REPORT_PREFIXES))
        return false;
    if (parseCourseReportRequest(text))
        return false;
    return containsAny(text, GENERAL_REPORT_KEYWORDS) || containsAny(text, ['تقرير', 'إحصائيات', 'تحليل']);
}
function isLastBotAskingForStudentId(messages) {
    if (!messages.length)
        return false;
...
function isLastBotAskingForCourseId(messages) {
    if (!messages.length)
        return false;
    const last = messages[messages.length - 1];
    if (last.role !== 'assistant')
...
function formatStudentReportFallback(report) {
    const lines = [];
    ...
    report.courses.forEach((course, index) => {
        ...
        const strengths = [];
        const weaknesses = [];
        if ((course.statistics.watch_percentage ?? 0) >= 70)
            strengths.push('التزام جيد بالمشاهدة');
        else
            weaknesses.push('مشاهدة منخفضة');
        if (course.statistics.averageGrade >= 70)
            strengths.push('أداء امتحانات جيد');
        else if (course.statistics.submittedExams > 0)
            weaknesses.push('درجات امتحانات تحتاج تحسين');
        if (course.statistics.submittedExams < course.statistics.totalExams)
            weaknesses.push('امتحانات غير مكتملة');
        if (strengths.length)
            lines.push(`- نقاط القوة: ${strengths.join('، ')}`);
        if (weaknesses.length)
            lines.push(`- نقاط الضعف: ${weaknesses.join('، ')}`);
        lines.push('');
    });
    return lines.join('\n');
}
function formatGeneralReportFallback(data) {
    ...
    if (!overview.top_students.length) {
        ...
        overview.top_students.forEach((student) => {
            lines.push(`${student.rank}. ${student.student_name} (كود ${student.student_id}) — مشاهدة ${student.watch_percentage.toFixed(1)}% | امتحانات ${student.exam_percentage.toFixed(1)}%`);
        });
    }
    lines.push('');
    ...
    if (!overview.at_risk_students.length) {
        ...
        overview.at_risk_students.slice(0, 5).forEach((student) => {
            lines.push(`- ${student.student_name} (كود ${student.student_id}) — خمول ${student.inactivity_days} يوم | مشاهدة ${student.watch_percentage.toFixed(1)}% | درجة خطورة ${student.risk_score.toFixed(0)}`);
        });
    }
    lines.push('');
...
function isLastBotAskingForExamChoice(messages) {
    if (!messages.length)
        return false;
    const last = messages[messages.length - 1];
    if (last.role !== 'assistant')
...
function formatExamReportFallback(data) {
    const lines = [];
    ...
    if (!data.insights.strengths.length)
        lines.push('- لا توجد بيانات كافية بعد.');
    else
        data.insights.strengths.forEach((item) => lines.push(`- ${item}`));
    lines.push('');
    lines.push('## نقاط الضعف');
    if (!data.insights.weaknesses.length)
        lines.push('- لا توجد ملاحظات سلبية واضحة.');
    else
        data.insights.weaknesses.forEach((item) => lines.push(`- ${item}`));
    lines.push('');
    lines.push('## من امتحن');
    if (!data.participation.submittedStudents.length)
        lines.push('- لا أحد بعد.');
    else {
        data.participation.submittedStudents.forEach((student) => {
            const grade = 'totalGrade' in student && student.totalGrade != null
    ...
    if (!data.participation.notSubmittedStudents.length)
        lines.push('- الجميع سلّم/امتحن.');
    else {
...
function formatCourseExamReportFallback(data) {
    return formatExamReportFallback(data);
}
function formatCourseReportFallback(data) {
    const { daily_report: report, analytics } = data;
    const lines = [];
    if (!report.course)
        return 'لا توجد بيانات لهذا الكورس.';
    ...
    const top = analytics.top_students || [];
    if (!top.length)
        lines.push('- لا توجد بيانات مشاهدة كافية.');
    else {
        top.slice(0, 5).forEach((student, index) => {
            lines.push(`${index + 1}. ${student.student_name} — مشاهدة ${Number(student.completion_percentage ?? 0).toFixed(1)}%`);
        });
    }
    lines.push('');
...
function formatExamMatches(matches, kind) {
...
async function generateReportWithAi(reportType, userRequest, rawData, recentMessages) {
    try {
        ...
        const response = await fetch(DEEPSEEK_API_URL, {
            ...
            body: JSON.stringify({
                model: 'deepseek-chat',
                messages: [
                    { role: 'system', content: dataAnalyst_prompts_1.DATA_ANALYST_SYSTEM_PROMPT },
                    ...history,
                    {
                        role: 'user',
...
أنشئ التقرير النهائي بالعربية بناءً على هذه البيانات فقط.`,
        ...
        utils_1.logger.warn({ error, reportType }, 'Data analyst AI formatting failed');
        return null;
    }
}
class DataAnalystChatbotService {
    static reportTypeConstraintReady = false;
    /** Ensures report_type accepts 'exam' (migration 1773100000000). */
    static async ensureReportTypeConstraint() {
        if (this.reportTypeConstraintReady)
            return;
        const check = await pool_1.default.query(`SELECT pg_get_constraintdef(oid) AS definition
       FROM pg_constraint
    ...
    static async saveMessage(teacherId, role, message, reportType = null, context = {}) {
        await this.ensureReportTypeConstraint();
        const result = await pool_1.default.query(`INSERT INTO teacher_data_analyst_messages (teacher_id, role, message, report_type, context)
       VALUES ($1, $2, $3, $4, $5::jsonb)
       RETURNING id, teacher_id, role, message, report_type, context, created_at`, [teacherId, role, message, reportType, JSON.stringify(context)]);
    ...
    static async getHistory(teacherId, limit = 30, offset = 0) {
        ...
        const [rows, count] = await Promise.all([
            pool_1.default.query(`SELECT id, teacher_id, role, message, report_type, context, created_at
         FROM teacher_data_analyst_messages
         WHERE teacher_id = $1
         ORDER BY created_at DESC
         LIMIT $2 OFFSET $3`, [teacherId, safeLimit, safeOffset]),
            pool_1.default.query(`SELECT COUNT(*)::text AS total FROM teacher_data_analyst_messages WHERE teacher_id = $1`, [teacherId]),
        ]);
        return {
    ...
    static async handleMessage(teacherId, tenantId, text, recentMessages = []) {
        const trimmed = text.trim();
        if (!trimmed) {
            return {
                reply: dataAnalyst_prompts_1.DATA_ANALYST_WELCOME_MESSAGE,
                report_type: 'other',
            };
        }
        ...
        if ((0, dataAnalystExamReports_1.isExamAnalysisRequest)(trimmed)) {
        ...
        if (studentReq) {
            ...
            try {
                const result = await dataAnalystReports_1.DataAnalystReportsService.getStudentReportByName(teacherId, studentReq.value);
        ...
        if (courseReq || containsAny(trimmed, COURSE_REPORT_PREFIXES)) {
            ...
            if (courseReq?.type === 'name') {
                try {
                    const result = await dataAnalystReports_1.DataAnalystReportsService.getCourseReportByName(teacherId, courseReq.value, tenantId);
                    if ('matches' in result) {
                        const list = result.matches
                    ...
                    const aiReply = await generateReportWithAi('course', trimmed, result.report, recentMessages);
                    return {
            ...
            const courses = await dataAnalystReports_1.DataAnalystReportsService.getTeacherCourses(teacherId);
            if (!courses.length) {
        ...
        if (isGeneralReportRequest(trimmed)) {
            try {
                const data = await dataAnalystReports_1.DataAnalystReportsService.getGeneralReport(teacherId, tenantId);
                const aiReply = await generateReportWithAi('general', trimmed, data, recentMessages);
                return {
...
- تحليل امتحان نصف العام`,
    ...
    static async handleExamAnalysisRequest(teacherId, userRequest, recentMessages) {
        const kind = (0, dataAnalystExamReports_1.classifyExamReportKind)(userRequest);
        if (!kind) {
            return {
                reply: 'حدّد نوع التقرير:\n• **واجب/محاضرة** — مثال: `تقرير واجب المحاضرة الأولى`\n• **امتحان كورس** — مثال: `تحليل امتحان نصف العام`',
                report_type: 'exam',
            };
        }
        const searchQuery = (0, dataAnalystExamReports_1.extractExamSearchQuery)(userRequest);
        ...
        const matches = kind === 'lecture'
            ? await dataAnalystExamReports_1.DataAnalystExamReportsService.findLectureExams(teacherId, searchQuery)
            : await dataAnalystExamReports_1.DataAnalystExamReportsService.findCourseExams(teacherId, searchQuery);
        if (matches.length === 1) {
    ...
    static async buildExamReportById(teacherId, userRequest, examId, recentMessages, preferredKind) {
        const attempts = [];
        if (!preferredKind || preferredKind === 'lecture') {
            attempts.push({
                kind: 'lecture',
                loader: async () => (await dataAnalystExamReports_1.DataAnalystExamReportsService.getLectureExamAnalysis(teacherId, examId)),
        ...
        if (!preferredKind || preferredKind === 'course') {
            attempts.push({
                kind: 'course',
                loader: async () => (await dataAnalystExamReports_1.DataAnalystExamReportsService.getCourseExamAnalysis(teacherId, examId)),
        ...
        for (const attempt of attempts) {
            try {
                ...
                return {
                    reply: aiReply ??
                        (attempt.kind === 'course'
                            ? formatCourseExamReportFallback(rawData)
                            : formatExamReportFallback(rawData)),
                    report_type: 'exam',
                    context: { exam_id: examId, exam_kind: attempt.kind },
                };
            }
            catch (error) {
                if (error instanceof utils_1.HttpError && error.status === 404) {
                    continue;
                }
                const message = error instanceof Error ? error.message : 'تعذر جلب التقرير';
                return {
    ...
    static async buildCourseReport(teacherId, tenantId, userRequest, courseId, recentMessages) {
        try {
            const data = await dataAnalystReports_1.DataAnalystReportsService.getCourseReport(teacherId, courseId, tenantId);
            const aiReply = await generateReportWithAi('course', userRequest, data, recentMessages);
            return {
    ...
    static async buildStudentReport(teacherId, tenantId, userRequest, studentId, recentMessages) {
        try {
            const report = await dataAnalystReports_1.DataAnalystReportsService.getStudentReport(teacherId, studentId);
    ...
    static getBotInfo() {
        return {
            name: dataAnalyst_prompts_1.DATA_ANALYST_BOT_NAME,
            welcome_message: dataAnalyst_prompts_1.DATA_ANALYST_WELCOME_MESSAGE,
            capabilities: [
...
exports.DataAnalystChatbotService = DataAnalystChatbotService;
