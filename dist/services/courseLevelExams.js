...
exports.CourseLevelExamsService = void 0;
const pool_1 = __importDefault(require("../db/pool"));
const utils_1 = require("../utils");
class CourseLevelExamsService {
    static async createExam(requester, input) {
        const courseRes = await pool_1.default.query('SELECT id, teacher_id FROM courses WHERE id = $1', [
            input.courseId,
        ]);
        if (!courseRes.rowCount) {
            throw new utils_1.HttpError(404, 'Course not found');
        }
        const course = courseRes.rows[0];
        if (requester.role === 'teacher' && course.teacher_id !== requester.id) {
            throw new utils_1.HttpError(403, 'You are not allowed to create exams for this course');
        }
        // Validate attemptLimit if provided
        if (input.attemptLimit !== undefined &&
            input.attemptLimit !== null &&
            input.attemptLimit <= 0) {
            throw new utils_1.HttpError(400, 'attemptLimit must be greater than 0 if provided');
        }
        const result = await pool_1.default.query(`INSERT INTO course_level_exams (
        course_id,
        title,
        duration_minutes,
        questions_count,
    ...
    static async getExamsByCourse(courseId, requester) {
        ...
        const course = courseRes.rows[0];
        if (requester.role === 'teacher' && course.teacher_id !== requester.id) {
            throw new utils_1.HttpError(403, 'You are not allowed to view exams for this course');
        }
        const result = await pool_1.default.query(`SELECT * FROM course_level_exams 
       WHERE course_id = $1 
    ...
    static serializeTeacherCourseExam(row) {
        return {
            id: row.id,
            title: row.title,
            ...
            examKind: 'course_level',
        };
    }
    static async getExamById(examId, requester) {
        const examRes = await pool_1.default.query(`SELECT e.*, c.teacher_id, c.title as course_title 
       FROM course_level_exams e
       ...
       WHERE e.id = $1`, [examId]);
        ...
        if (requester.role === 'teacher' && exam.teacher_id !== requester.id) {
            throw new utils_1.HttpError(403, 'You are not allowed to view this exam');
        }
        return exam;
    }
    static async updateExam(examId, requester, input) {
       ...
       WHERE e.id = $1`, [examId]);
        ...
        const exam = examRes.rows[0];
        if (requester.role === 'teacher' && exam.teacher_id !== requester.id) {
            throw new utils_1.HttpError(403, 'You are not allowed to update this exam');
        }
        // Validate conditional fields
        const isVisibleToStudents = input.isVisibleToStudents !== undefined
            ? input.isVisibleToStudents
            : exam.is_visible_to_students;
        const visibilityEndDate = input.visibilityEndDate !== undefined ? input.visibilityEndDate : exam.visibility_end_date;
        if (!isVisibleToStudents && !visibilityEndDate) {
            throw new utils_1.HttpError(400, 'visibilityEndDate is required when isVisibleToStudents is false');
        }
        const showAnswersImmediately = input.showAnswersImmediately !== undefined
            ? input.showAnswersImmediately
            : exam.show_answers_immediately;
        const answersVisibleAt = input.answersVisibleAt !== undefined ? input.answersVisibleAt : exam.answers_visible_at;
        if (!showAnswersImmediately && !answersVisibleAt) {
            throw new utils_1.HttpError(400, 'answersVisibleAt is required when showAnswersImmediately is false');
        }
        // Validate positive numbers if provided
        if (input.durationMinutes !== undefined && input.durationMinutes <= 0) {
            throw new utils_1.HttpError(400, 'durationMinutes must be greater than 0');
        }
        if (input.questionsCount !== undefined && input.questionsCount <= 0) {
            throw new utils_1.HttpError(400, 'questionsCount must be greater than 0');
        }
        if (input.attemptLimit !== undefined &&
            input.attemptLimit !== null &&
            input.attemptLimit <= 0) {
    ...
    static async deleteExam(examId, requester) {
        // Verify exam exists and requester owns it
        const examRes = await pool_1.default.query(`SELECT e.*, c.teacher_id 
       FROM course_level_exams e
       JOIN courses c ON e.course_id = c.id
       WHERE e.id = $1`, [examId]);
        ...
        const exam = examRes.rows[0];
        if (requester.role === 'teacher' && exam.teacher_id !== requester.id) {
            throw new utils_1.HttpError(403, 'You are not allowed to delete this exam');
        }
        await pool_1.default.query('DELETE FROM course_level_exams WHERE id = $1', [examId]);
     ...
     */
    static async getVisibleExamsForStudent(courseId, studentId) {
        ...
        if (!enrollmentCheck.rowCount) {
            throw new utils_1.HttpError(403, 'You are not enrolled in this course');
        }
        // Get visible exams
        const now = new Date();
        const result = await pool_1.default.query(`SELECT e.*, c.title as course_title
       FROM course_level_exams e
       JOIN courses c ON e.course_id = c.id
       WHERE e.course_id = $1
       ...
       ORDER BY e.created_at DESC`, [courseId, now]);
        // Get attempt counts for each exam
        const examsWithAttempts = await Promise.all(result.rows.map(async (exam) => {
            const attemptsRes = await pool_1.default.query(`SELECT COUNT(*) as attempts_count, 
           ...
           WHERE exam_id = $1 AND student_id = $2`, [exam.id, studentId]);
            ...
            return {
                ...
                attempts_remaining: exam.attempt_limit === null
                    ? null
     ...
     */
    static async startExamAttempt(examId, studentId) {
       ...
       WHERE e.id = $1`, [examId]);
        ...
        const enrollmentCheck = await pool_1.default.query('SELECT id FROM enrollments WHERE course_id = $1 AND user_id = $2', [exam.course_id, studentId]);
        if (!enrollmentCheck.rowCount) {
            throw new utils_1.HttpError(403, 'You are not enrolled in this course');
        }
        // Check if exam is visible and active
        if (!exam.is_active) {
            throw new utils_1.HttpError(403, 'This exam is not active');
        }
        if (!exam.is_visible_to_students) {
            throw new utils_1.HttpError(403, 'This exam is not visible to students');
        }
        ...
        if (exam.visibility_end_date) {
            ...
            if (now > endDate) {
                throw new utils_1.HttpError(403, 'This exam is no longer available');
            }
        }
        // Check attempt limit and get previous attempts
        const attemptsRes = await pool_1.default.query(`SELECT COUNT(*) as attempts_count, MAX(attempt_number) as last_attempt_number
       FROM course_level_exam_attempts
       WHERE exam_id = $1 AND student_id = $2`, [examId, studentId]);
        ...
        if (exam.attempt_limit === 1 && hasCompletedAttempts) {
            ...
            const lastAttemptRes = await pool_1.default.query(`SELECT * FROM course_level_exam_attempts
         ...
         LIMIT 1`, [examId, studentId]);
            if (lastAttemptRes.rowCount && lastAttemptRes.rowCount > 0) {
                ...
                let wrongQuestions = [];
                if (showAnswers) {
                    const wrongAnswersRes = await pool_1.default.query(`SELECT a.*, q.*
             ...
             ORDER BY q.created_at ASC`, [lastAttempt.id]);
                ...
                const error = new utils_1.HttpError(403, 'You have already completed this exam. Only one attempt is allowed.');
                error.details = {
                    previousAttempt: {
        ...
        if (exam.attempt_limit !== null && attemptsCount >= exam.attempt_limit) {
            throw new utils_1.HttpError(403, 'You have used all allowed attempts for this exam');
        }
        // Check if there's an active attempt
        const activeAttemptRes = await pool_1.default.query(`SELECT * FROM course_level_exam_attempts
       ...
       LIMIT 1`, [examId, studentId]);
        if (activeAttemptRes.rowCount && activeAttemptRes.rowCount > 0) {
            ...
            const questionsRes = await pool_1.default.query(`SELECT * FROM course_level_exam_questions
         WHERE exam_id = $1
        ...
        const attemptResult = await pool_1.default.query(`INSERT INTO course_level_exam_attempts (
        exam_id, student_id, attempt_number, status, started_at
      ) VALUES ($1, $2, $3, 'in_progress', NOW())
      RETURNING *`, [examId, studentId, nextAttemptNumber]);
     ...
     */
    static async submitExamAttempt(examId, studentId, attemptId, answers) {
        // Get attempt
        const attemptRes = await pool_1.default.query(`SELECT a.*, e.*, c.id as course_id
       FROM course_level_exam_attempts a
       JOIN course_level_exams e ON a.exam_id = e.id
       JOIN courses c ON e.course_id = c.id
       WHERE a.id = $1 AND a.exam_id = $2 AND a.student_id = $3`, [attemptId, examId, studentId]);
        if (!attemptRes.rowCount) {
            throw new utils_1.HttpError(404, 'Attempt not found');
        }
        const attempt = attemptRes.rows[0];
        const exam = attemptRes.rows[0];
        if (attempt.status !== 'in_progress') {
            throw new utils_1.HttpError(400, 'This attempt has already been submitted');
        }
        // Verify student is enrolled
        const enrollmentCheck = await pool_1.default.query('SELECT id FROM enrollments WHERE course_id = $1 AND user_id = $2', [exam.course_id, studentId]);
        if (!enrollmentCheck.rowCount) {
       ...
       ORDER BY created_at ASC`, [examId]);
        ...
        for (const answer of answers) {
            ...
            if (question) {
                const isCorrect = question.correct_answer === answer.selectedAnswer;
                await pool_1.default.query(`INSERT INTO course_level_exam_answers (
            attempt_id, question_id, selected_answer, is_correct
          ) VALUES ($1, $2, $3, $4)`, [attemptId, answer.questionId, answer.selectedAnswer, isCorrect]);
            }
        }
        // Update attempt
        await pool_1.default.query(`UPDATE course_level_exam_attempts
       SET status = 'submitted',
           submitted_at = NOW(),
           total_grade = $1,
           obtained_grade = $2
       WHERE id = $3`, [questions.length, totalGrade, attemptId]);
        // Check if answers should be shown
        const now = new Date();
        let showAnswers = false;
        let releaseReason = '';
        ...
        const submittedAtRes = await pool_1.default.query(`SELECT started_at, submitted_at FROM course_level_exam_attempts WHERE id = $1`, [attemptId]);
     ...
     */
    static async getWrongQuestions(examId, studentId) {
        // Get exam with course info
        const examRes = await pool_1.default.query(`SELECT e.*, c.id as course_id
       FROM course_level_exams e
       JOIN courses c ON e.course_id = c.id
       WHERE e.id = $1`, [examId]);
        if (!examRes.rowCount) {
        ...
        // Get the last submitted attempt
        const attemptRes = await pool_1.default.query(`SELECT * FROM course_level_exam_attempts
       WHERE exam_id = $1 AND student_id = $2 AND status = 'submitted'
       ORDER BY submitted_at DESC
       LIMIT 1`, [examId, studentId]);
        if (!attemptRes.rowCount) {
            throw new utils_1.HttpError(404, 'No completed attempt found for this exam');
        }
        const attempt = attemptRes.rows[0];
        // Check if answers should be shown
        const now = new Date();
        ...
        if (exam.show_answers_immediately) {
            ...
            throw new utils_1.HttpError(403, 'Answers are not configured to be shown for this exam');
        }
        if (!showAnswers) {
            return {
                showAnswers: false,
       ...
       JOIN course_level_exam_questions q ON a.question_id = q.id
       WHERE a.attempt_id = $1 AND a.is_correct = FALSE
       ORDER BY q.created_at ASC`, [attempt.id]);
     ...
     */
    static async getMyCourseReport(examId, studentId) {
        const examRes = await pool_1.default.query(`SELECT e.*, c.id as course_id
       FROM course_level_exams e
       JOIN courses c ON e.course_id = c.id
       WHERE e.id = $1`, [examId]);
       ...
       LIMIT 1`, [examId, studentId]);
        if (!attemptRes.rowCount) {
            throw new utils_1.HttpError(404, 'لا توجد محاولة مُسلَّمة لهذا الامتحان');
        }
        const attempt = attemptRes.rows[0];
        const now = new Date();
        let showAnswers = false;
        if (exam.show_answers_immediately) {
            ...
            throw new utils_1.HttpError(403, 'لا يمكن عرض تقرير الإجابات حالياً (سيتم إظهارها في وقت لاحق أو غير مفعّل)');
        }
        const allAnswersRes = await pool_1.default.query(`SELECT a.question_id, a.selected_answer, a.is_correct,
     ...
     */
    static async getExamGrades(examId, requester) {
        // Verify exam exists and teacher owns it
        const examRes = await pool_1.default.query(`SELECT e.id, e.title, e.course_id, c.teacher_id, c.title as course_title
       FROM course_level_exams e
       JOIN courses c ON e.course_id = c.id
       WHERE e.id = $1`, [examId]);
        if (!examRes.rowCount) {
        ...
        if (requester.role === 'teacher' && exam.teacher_id !== requester.id) {
            throw new utils_1.HttpError(403, 'You are not allowed to view grades for this exam');
        }
        // Get all submitted attempts with student info
        const attemptsRes = await pool_1.default.query(`SELECT 
         a.id as attempt_id,
         a.student_id,
         a.attempt_number,
         a.status,
       ...
       ORDER BY a.submitted_at DESC`, [examId]);
        ...
        const maxGrade = attempts.length > 0 ? Math.max(...attempts.map((a) => a.obtained_grade || 0)) : 0;
        const minGrade = attempts.length > 0 ? Math.min(...attempts.map((a) => a.obtained_grade || 0)) : 0;
        return {
            exam: {
                id: exam.id,
                title: exam.title,
            ...
            students: attempts.map((a) => ({
                ...
                percentage: a.total_grade > 0 ? Math.round((a.obtained_grade / a.total_grade) * 100 * 100) / 100 : 0,
     ...
     */
    static async getExamReport(examId, requester) {
        // Verify exam exists and teacher owns it
        const examRes = await pool_1.default.query(`SELECT e.id, e.title, e.course_id, e.questions_count, c.teacher_id, c.title as course_title
       FROM course_level_exams e
       JOIN courses c ON e.course_id = c.id
       WHERE e.id = $1`, [examId]);
        if (!examRes.rowCount) {
        ...
        // Verify teacher owns the course
        if (requester.role === 'teacher' && exam.teacher_id !== requester.id) {
            throw new utils_1.HttpError(403, 'You are not allowed to view report for this exam');
        }
        // Get all questions
        const questionsRes = await pool_1.default.query(`SELECT id, type, question_text, question_image, option_a, option_b, option_c, option_d, correct_answer
       FROM course_level_exam_questions
       WHERE exam_id = $1
       ORDER BY id ASC`, [examId]);
        const questions = questionsRes.rows;
        // Get all submitted attempts
        const attemptsRes = await pool_1.default.query(`SELECT id, student_id
       FROM course_level_exam_attempts
       ...
       ORDER BY a.question_id ASC`, [examId]);
            ...
            return {
                ...
                statistics: {
                    ...
                    correctPercentage: questionAnswers.length > 0
                        ...
                        : 0,
                    wrongPercentage: questionAnswers.length > 0
        ...
        const sortedQuestions = [...questionsWithStats].sort((a, b) => b.statistics.wrongAnswers - a.statistics.wrongAnswers);
        // Calculate overall statistics
        const totalQuestions = questions.length;
        const totalAnswers = answers.length;
        const totalCorrect = answers.filter((a) => a.is_correct).length;
        const totalWrong = answers.filter((a) => !a.is_correct).length;
        const overallCorrectPercentage = totalAnswers > 0 ? Math.round((totalCorrect / totalAnswers) * 100 * 100) / 100 : 0;
     ...
     */
    static courseExamBankLinkColumns = null;
    static async getCourseExamBankLinkColumns() {
        if (this.courseExamBankLinkColumns)
            return this.courseExamBankLinkColumns;
        const res = await pool_1.default.query(`SELECT column_name
    ...
    static async ensureCourseExamBankLinkColumns() {
        let cols = await this.getCourseExamBankLinkColumns();
        if (cols.question_id && cols.question_id_v2)
            return cols;
      ...
      `);
        ...
        if (!cols.question_id) {
            const legacyTable = await pool_1.default.query(`SELECT 1
    ...
    static async validateQuestionIdsInBank(questionIds) {
        if (!questionIds?.length) {
            return { missing: [] };
        }
        const uniqueIds = [...new Set(questionIds)];
        const v2Res = await pool_1.default.query(`SELECT id FROM questions_v2 WHERE id = ANY($1::int[])`, [uniqueIds]);
        const found = new Set(v2Res.rows.map((row) => row.id));
        const notInV2 = uniqueIds.filter((id) => !found.has(id));
        if (notInV2.length > 0) {
            const v1Res = await pool_1.default.query(`SELECT id FROM questions WHERE id = ANY($1::int[])`, [notInV2]);
     ...
     */
    static async addQuestionsFromBank(requester, examId, questionIds) {
        console.log(`[addQuestionsFromBank] Called with examId=${examId}, questionIds=${JSON.stringify(questionIds)}, teacherId=${requester.id}`);
        try {
            ...
            // Wrap other errors
            throw new utils_1.HttpError(500, `Failed to verify exam ownership: ${error?.message || 'Unknown error'}`);
        }
        ...
        try {
            ...
            if (bankCols.question_id)
                selectParts.push('question_id');
            if (bankCols.question_id_v2)
                selectParts.push('question_id_v2');
            const existingV1Ids = new Set();
            const existingV2Ids = new Set();
            if (selectParts.length > 0) {
                const existingQsRes = await pool_1.default.query(`SELECT ${selectParts.join(', ')} FROM course_level_exam_questions WHERE exam_id = $1`, [examId]);
                if (bankCols.question_id) {
                    for (const row of existingQsRes.rows) {
                        if (row.question_id != null)
                            existingV1Ids.add(row.question_id);
                    }
                }
                if (bankCols.question_id_v2) {
                    for (const row of existingQsRes.rows) {
                        if (row.question_id_v2 != null)
                            existingV2Ids.add(row.question_id_v2);
                    }
                }
            }
            ...
            let v2Questions;
            try {
                v2Questions = await pool_1.default.query(`SELECT q.id, q.question_text, q.correct_answer_index, qm.media_url
           FROM questions_v2 q
           LEFT JOIN question_media qm ON q.id = qm.question_id
           WHERE q.id = ANY($1::int[])`, [uniqueIds]);
            }
            catch (queryError) {
                console.error(`[addQuestionsFromBank] Error fetching V2 questions:`, queryError.message);
                throw new utils_1.HttpError(500, `Failed to fetch questions from bank: ${queryError.message}`);
            }
            ...
            if (v2Questions && v2Questions.rowCount > 0) {
                for (const question of v2Questions.rows) {
                    ...
                    let optionsRes;
                    try {
                        optionsRes = await pool_1.default.query(`SELECT option_index, option_type, text_content, image_url
               FROM question_options
               WHERE question_id = $1
               ORDER BY option_index ASC`, [question.id]);
                    }
                    catch (optionsError) {
                        console.error(`[addQuestionsFromBank] Error fetching options for V2 question ${question.id}:`, optionsError.message);
                        continue;
                    }
                    ...
                    const correctAnswer = String.fromCharCode(65 + (question.correct_answer_index ?? 0));
                    ...
                    try {
                        ...
                        const result = await pool_1.default.query(`INSERT INTO course_level_exam_questions (${insertCols.join(', ')})
               VALUES (${placeholders})
               RETURNING *`, insertVals);
                        ...
                        console.error(`[addQuestionsFromBank] Error inserting V2 question ${question.id}:`, insertError.message);
                    }
                }
            }
            // 2. Identify remaining IDs from V1
            const addedV2Ids = (v2Questions?.rows ?? []).map((r) => r.id);
            const remainingIds = uniqueIds.filter((id) => !addedV2Ids.includes(id));
            if (remainingIds.length > 0) {
                let v1Questions;
                try {
                    v1Questions = await pool_1.default.query(`SELECT q.id, q.text, q.image, q.correct_answer, q.points
             FROM questions q
             WHERE q.id = ANY($1::int[])`, [remainingIds]);
                }
                catch (queryError) {
                ...
                for (const question of v1Questions?.rows || []) {
                    let choicesRes;
                    try {
                        choicesRes = await pool_1.default.query(`SELECT choice_text, is_correct
               ...
               LIMIT 4`, [question.id]);
                        ...
                        console.error(`[addQuestionsFromBank] Error fetching choices for V1 question ${question.id}:`, choicesError.message);
                        continue;
                    }
               ...
               RETURNING *`, insertVals);
                        ...
                        console.error(`[addQuestionsFromBank] Error inserting V1 question ${question.id}:`, insertError.message);
                    }
                }
            }
            // 4. Update the exam's questions_count
            const countRes = await pool_1.default.query(`SELECT COUNT(*) as count FROM course_level_exam_questions WHERE exam_id = $1`, [examId]);
            const actualCount = parseInt(countRes.rows[0]?.count || '0', 10);
            try {
                await pool_1.default.query(`UPDATE course_level_exams SET questions_count = $1 WHERE id = $2`, [
                    actualCount,
                    examId,
                ]);
            }
            catch (updateError) {
                console.error(`[addQuestionsFromBank] Error updating questions_count:`, updateError.message);
            }
            ...
            throw new utils_1.HttpError(500, `Failed to add questions from bank: ${error?.message || 'Unknown error'} `);
        }
    }
    /**
     * Verify exam ownership
    ...
    static async verifyExamOwnership(examId, teacherId) {
        try {
            ...
            const examRes = await pool_1.default.query(`SELECT e.id, c.teacher_id
         FROM course_level_exams e
         JOIN courses c ON e.course_id = c.id
         WHERE e.id = $1`, [examId]);
            ...
            if (examRes.rows[0].teacher_id !== teacherId) {
                console.error(`[verifyExamOwnership] Teacher ${teacherId} does not own exam ${examId} (owner: ${examRes.rows[0].teacher_id})`);
...
exports.CourseLevelExamsService = CourseLevelExamsService;
