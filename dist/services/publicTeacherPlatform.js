...
exports.resolveTeacherPlatformBySubdomain = resolveTeacherPlatformBySubdomain;
exports.getPublicFreeLecturesBySubdomain = getPublicFreeLecturesBySubdomain;
exports.getPublicCoursesBySubdomain = getPublicCoursesBySubdomain;
...
async function resolveTeacherPlatformBySubdomain(subdomainRaw) {
    ...
    if (!tenant.owner_user_id)
        return null;
    const access = await teacherPlatformSubscriptions_1.TeacherPlatformSubscriptionsService.getPlatformAccessState(tenant.owner_user_id);
    if (!access.allowed)
        return null;
    if (!tenant.is_active) {
        if (access.phase === 'grace') {
            await pool_1.default.query(`UPDATE tenants SET is_active = true, updated_at = NOW() WHERE id = $1`, [tenant.id]);
    ...
    const teacherRes = await pool_1.default.query(`SELECT id, name, avatar, description, subject
     FROM users
...
async function getPublicFreeLecturesBySubdomain(subdomainRaw) {
    ...
    const lecturesRes = await pool_1.default.query(`SELECT
     ...
     ORDER BY l.created_at DESC`, [platform.teacher.id]);
    return {
        platform: {
...
async function getPublicCoursesBySubdomain(subdomainRaw, gradeId) {
    const platform = await resolveTeacherPlatformBySubdomain(subdomainRaw);
    if (!platform)
        return null;
    const params = [platform.teacher.id];
    let gradeFilter = '';
    if (gradeId != null) {
        if (!Number.isInteger(gradeId) || gradeId <= 0) {
            throw new utils_1.HttpError(400, 'grade_id غير صحيح');
        }
        params.push(gradeId);
