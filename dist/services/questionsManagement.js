...
class QuestionsManagementService {
    ...
    static async createBulkQuestions(bulkText) {
        ...
        for (const q of questions) {
            const result = await pool_1.default.query(`INSERT INTO questions_management (question_text, options) VALUES ($1, $2) RETURNING *`, [q.question_text, JSON.stringify(q.options)]);
    ...
    static async createBulkQuestionsForLectureExam(examId, bulkText) {
        // التحقق من وجود الامتحان وأنه يخص المدرس
        const examCheck = await pool_1.default.query(`SELECT e.*, l.course_id, c.teacher_id 
       FROM exams e 
       ...
       WHERE e.id = $1`, [examId]);
        ...
        for (const q of questions) {
            ...
            const examQuestionResult = await pool_1.default.query(`INSERT INTO exam_questions (exam_id, question_text, grade, question_id) VALUES ($1, $2, $3, $4) RETURNING id`, [examId, q.text, 1, questionId]);
     ...
     */
    static async createBulkQuestionsForLectureExamFromParsed(examId, teacherId, parsed) {
        const examCheck = await pool_1.default.query(`SELECT e.id, c.teacher_id FROM exams e
       JOIN lectures l ON e.lecture_id = l.id
       JOIN courses c ON l.course_id = c.id
       WHERE e.id = $1 AND e.type IN ('exam', 'assignment')`, [examId]);
        if (!examCheck.rowCount) {
            throw new Error('امتحان المحاضرة غير موجود');
        }
        ...
        for (const q of parsed) {
            ...
            const correct = (q.correctAnswer || 'A').toUpperCase();
            if (!questionText || !optionA || !optionB || !optionC || !optionD) {
                throw new Error('كل سؤال يحتاج: questionText و optionA, optionB, optionC, optionD');
            }
            const questionResult = await pool_1.default.query(`INSERT INTO questions (text, type) VALUES ($1, 'single_choice') RETURNING id`, [questionText]);
            const questionId = questionResult.rows[0].id;
            const choices = [
                { key: 'A', text: optionA },
                { key: 'B', text: optionB },
            ...
            for (const ch of choices) {
                await pool_1.default.query(`INSERT INTO question_choices (question_id, text, is_correct) VALUES ($1, $2, $3)`, [questionId, ch.text, ch.key === correct]);
            }
            const examQuestionResult = await pool_1.default.query(`INSERT INTO exam_questions (exam_id, question_text, grade, question_id) VALUES ($1, $2, 1, $3) RETURNING id`, [examId, questionText, questionId]);
            const examQuestionId = examQuestionResult.rows[0].id;
            inserted.push({
                id: examQuestionId,
    ...
    static async getLectureExamQuestions(examId) {
        // التحقق من وجود الامتحان
        const examCheck = await pool_1.default.query(`SELECT e.* FROM exams e WHERE e.id = $1 AND e.type = 'exam'`, [examId]);
        if (!examCheck.rowCount) {
            throw new Error('امتحان المحاضرة غير موجود');
        }
    ...
    static async getAllQuestions() {
        const result = await pool_1.default.query(`SELECT * FROM questions_management ORDER BY created_at DESC`);
    ...
    static async updateCorrectAnswer(questionId, correctOption) {
        if (!['A', 'B', 'C', 'D'].includes(correctOption.toUpperCase())) {
            throw new Error('correctOption يجب أن يكون A, B, C, أو D');
        }
        const result = await pool_1.default.query(`UPDATE questions_management SET correct_option = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2 RETURNING *`, [correctOption.toUpperCase(), questionId]);
        if (!result.rowCount) {
    ...
    static async deleteQuestion(questionId) {
        const result = await pool_1.default.query(`DELETE FROM questions_management WHERE id = $1 RETURNING *`, [
            questionId,
        ]);
        if (!result.rowCount) {
            throw new Error('السؤال غير موجود');
    ...
    // تحديث سؤال كامل
    static async updateQuestion(questionId, questionText, options) {
        ...
        const result = await pool_1.default.query(`UPDATE questions_management SET question_text = $1, options = $2, updated_at = CURRENT_TIMESTAMP WHERE id = $3 RETURNING *`, [questionText, JSON.stringify(options), questionId]);
        if (!result.rowCount) {
            throw new Error('السؤال غير موجود');
        }
        return {
    ...
    static async getQuestionById(questionId) {
        const result = await pool_1.default.query(`SELECT * FROM questions_management WHERE id = $1`, [
            questionId,
        ]);
        if (!result.rowCount) {
            throw new Error('السؤال غير موجود');
    ...
    static async updateLectureExamQuestion(questionId, questionText, grade, image) {
        if (!questionText && !image && grade === undefined) {
            throw new Error('يجب إرسال نص السؤال أو الصورة أو الدرجة للتعديل');
        }
        const examQuestionRes = await pool_1.default.query(`SELECT question_id FROM exam_questions WHERE id = $1`, [questionId]);
        ...
        const result = await pool_1.default.query(`UPDATE exam_questions SET ${fields.join(', ')} WHERE id = $${i} RETURNING *`, values);
        // إذا تم تحديث الصورة وكان السؤال مربوط بجدول questions، حدث الصورة هناك أيضاً
        if (image !== undefined && questionIdInQuestions) {
            await pool_1.default.query(`UPDATE questions SET image = $1 WHERE id = $2`, [
                image,
        ...
        if (questionText !== undefined && questionIdInQuestions) {
            await pool_1.default.query(`UPDATE questions SET text = $1 WHERE id = $2`, [
                questionText,
                questionIdInQuestions,
            ]);
        }
    ...
    static async setLectureExamQuestionCorrectAnswer(questionId, correctChoiceId) {
        const examQuestionRes = await pool_1.default.query(`SELECT question_id, question_id_v2 FROM exam_questions WHERE id = $1`, [questionId]);
        if (!examQuestionRes.rowCount) {
            throw new Error('السؤال غير موجود');
        }
        ...
        if (questionIdV2 != null && actualQuestionId == null) {
            ...
            if (correctChoiceId >= 0) {
                const optionRow = await pool_1.default.query(`SELECT option_index FROM question_options WHERE id = $1 AND question_id = $2`, [correctChoiceId, questionIdV2]);
            ...
            try {
                await pool_1.default.query(`UPDATE exam_questions SET correct_answer_index_override = $1 WHERE id = $2`, [optionIndex, questionId]);
    ...
    static async deleteLectureExamQuestion(questionId) {
        const examQuestionRes = await pool_1.default.query(`SELECT question_id, question_id_v2 FROM exam_questions WHERE id = $1`, [questionId]);
        if (!examQuestionRes.rowCount) {
            throw new Error('السؤال غير موجود');
        }
        ...
        const result = await pool_1.default.query(`DELETE FROM exam_questions WHERE id = $1 RETURNING *`, [
            questionId,
        ]);
        if (!result.rowCount) {
            throw new Error('السؤال غير موجود');
    ...
    static async addQuestionToLectureExam(examId, questionText, imageFile, grade = 1) {
        // التحقق من وجود الامتحان
        const examCheck = await pool_1.default.query(`SELECT id FROM exams WHERE id = $1 AND type = 'exam'`, [
            examId,
        ]);
        if (!examCheck.rowCount) {
            throw new Error('امتحان المحاضرة غير موجود');
        ...
        if (imageFile) {
            const uploaded = await (0, utils_1.uploadToCloudinary)(imageFile.path);
            imageUrl = uploaded.secure_url;
        }
        // إنشاء السؤال في جدول questions
        const questionResult = await pool_1.default.query(`INSERT INTO questions (text, type, image) VALUES ($1, 'single_choice', $2) RETURNING id`, [questionText || '', imageUrl]);
        const questionId = questionResult.rows[0].id;
        ...
        const examQuestionResult = await pool_1.default.query(`INSERT INTO exam_questions (exam_id, question_text, grade, question_id, image) 
       VALUES ($1, $2, $3, $4, $5) RETURNING *`, [examId, questionText || '', grade, questionId, imageUrl]);
    ...
    // إنشاء أسئلة بالصور
    static async createImageQuestions(files, examId) {
        const questions = [];
        for (const file of files) {
            ...
            if (examId) {
                ...
                await pool_1.default.query(`INSERT INTO exam_questions (exam_id, question_text, grade, question_id) VALUES ($1, $2, $3, $4)`, [examId, '', 1, questionId]);
            }
            // جلب الاختيارات المُنشأة
            const choicesResult = await pool_1.default.query(`SELECT id, text, is_correct FROM question_choices WHERE question_id = $1 ORDER BY id`, [questionId]);
            questions.push({
    ...
    static async submitLectureExam(examId, studentId, answers) {
