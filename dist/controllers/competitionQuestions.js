...
const competitionQuestions_1 = require("../services/competitionQuestions");
...
router.post('/', (0, authentication_1.authMiddleware)(['admin']), async (req, res) => {
    try {
        ...
        if (!questionData.competition_id ||
        ...
        const question = await competitionQuestions_1.CompetitionQuestionsService.create(questionData, req.user.id);
        res.status(201).json({
            success: true,
...
router.post('/bulk', (0, authentication_1.authMiddleware)(['admin']), async (req, res) => {
    try {
        ...
        const questions = await competitionQuestions_1.CompetitionQuestionsService.createBulk(bulkData, req.user.id);
        res.status(201).json({
            success: true,
...
router.post('/text', (0, authentication_1.authMiddleware)(['admin']), async (req, res) => {
    try {
        ...
        const result = await competitionQuestions_1.CompetitionQuestionsService.createFromText(competition_id, questions_text, req.user.id);
        res.status(201).json({
            success: true,
...
router.get('/competition/:competitionId', async (req, res) => {
    try {
        ...
        const questions = await competitionQuestions_1.CompetitionQuestionsService.getByCompetition(competitionId);
        res.json({
            success: true,
            data: questions,
        });
...
router.get('/competition/:competitionId/details', (0, authentication_1.authMiddleware)(['admin']), async (req, res) => {
    try {
        const competitionId = parseInt(req.params.competitionId);
        if (isNaN(competitionId)) {
            return res.status(400).json({
        ...
        const questions = await competitionQuestions_1.CompetitionQuestionsService.getByCompetitionWithDetails(competitionId);
        res.json({
            success: true,
            data: questions,
        });
...
router.get('/:id', async (req, res) => {
    try {
        ...
        const question = await competitionQuestions_1.CompetitionQuestionsService.getById(id);
        if (!question) {
...
router.put('/:id', (0, authentication_1.authMiddleware)(['admin']), async (req, res) => {
    try {
        ...
        const existingQuestion = await competitionQuestions_1.CompetitionQuestionsService.getById(id);
        if (!existingQuestion) {
            return res.status(404).json({
                success: false,
                message: 'السؤال غير موجود',
        ...
        const updatedQuestion = await competitionQuestions_1.CompetitionQuestionsService.update(id, updateData);
        res.json({
            success: true,
...
router.delete('/:id', (0, authentication_1.authMiddleware)(['admin']), async (req, res) => {
    try {
        ...
        // التحقق من وجود السؤال
        const exists = await competitionQuestions_1.CompetitionQuestionsService.exists(id);
        if (!exists) {
            return res.status(404).json({
                success: false,
                message: 'السؤال غير موجود',
        ...
        await competitionQuestions_1.CompetitionQuestionsService.delete(id);
        res.json({
            success: true,
...
router.patch('/:id/toggle-active', (0, authentication_1.authMiddleware)(['admin']), async (req, res) => {
    try {
        ...
        const updatedQuestion = await competitionQuestions_1.CompetitionQuestionsService.toggleActive(id);
        if (!updatedQuestion) {
            return res.status(404).json({
                success: false,
                message: 'السؤال غير موجود',
...
router.patch('/reorder/:competitionId', (0, authentication_1.authMiddleware)(['admin']), async (req, res) => {
    try {
        const competitionId = parseInt(req.params.competitionId);
        if (isNaN(competitionId)) {
            return res.status(400).json({
        ...
        await competitionQuestions_1.CompetitionQuestionsService.reorderQuestions(competitionId, questionOrders);
        res.json({
            success: true,
...
router.get('/stats/:competitionId', (0, authentication_1.authMiddleware)(['admin']), async (req, res) => {
    try {
        const competitionId = parseInt(req.params.competitionId);
        if (isNaN(competitionId)) {
            return res.status(400).json({
        ...
        const stats = await competitionQuestions_1.CompetitionQuestionsService.getCompetitionStats(competitionId);
        res.json({
            success: true,
            data: stats,
        });
...
router.patch('/:id/correct-answer', (0, authentication_1.authMiddleware)(['admin']), async (req, res) => {
    try {
        const id = parseInt(req.params.id);
        if (isNaN(id)) {
            return res.status(400).json({
        ...
        const updatedQuestion = await competitionQuestions_1.CompetitionQuestionsService.updateCorrectAnswer(id, correct_answer);
        res.json({
            success: true,
