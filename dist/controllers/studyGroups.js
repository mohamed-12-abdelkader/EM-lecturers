...
const studyGroups_1 = require("../services/studyGroups");
...
router.post('/', (0, authentication_1.authMiddleware)(['admin', 'teacher']), async (req, res) => {
    try {
        ...
        const group = await studyGroups_1.StudyGroupService.createStudyGroup(groupData);
        res.status(201).json({
        ...
        utils_1.logger.error('Error creating study group:', error);
        res.status(500).json({
...
router.put('/:id', (0, authentication_1.authMiddleware)(['admin', 'teacher']), async (req, res) => {
    try {
        const { id } = req.params;
        const { name, start_time, end_time, days, grade_id } = req.body;
        const teacherId = req.user.id;
        ...
        if (grade_id !== undefined)
            updateData.grade_id = grade_id ? parseInt(grade_id) : undefined;
        const group = await studyGroups_1.StudyGroupService.updateStudyGroup(parseInt(id), teacherId, updateData);
        if (!group) {
        ...
        utils_1.logger.error('Error updating study group:', error);
...
router.delete('/:id', (0, authentication_1.authMiddleware)(['admin', 'teacher']), async (req, res) => {
    try {
        const { id } = req.params;
        const teacherId = req.user.id;
        const group = await studyGroups_1.StudyGroupService.deleteStudyGroup(parseInt(id), teacherId);
        if (!group) {
        ...
        utils_1.logger.error('Error deleting study group:', error);
...
router.get('/all', async (req, res) => {
    try {
        const groups = await studyGroups_1.StudyGroupService.getAllGroups();
        res.json({ groups });
    }
    catch (error) {
        utils_1.logger.error('Error fetching all groups:', error);
        res.status(500).json({
...
router.get('/teacher/my-groups', (0, authentication_1.authMiddleware)(['admin', 'teacher']), async (req, res) => {
    try {
        const teacherId = req.user.id;
        const groups = await studyGroups_1.StudyGroupService.getTeacherGroups(teacherId);
        res.json({ groups });
    }
    catch (error) {
        utils_1.logger.error('Error fetching teacher groups:', error);
        res.status(500).json({
...
router.get('/teacher/my-students', (0, authentication_1.authMiddleware)(['admin', 'teacher']), async (req, res) => {
    try {
        const teacherId = req.user.id;
        ...
        const students = result.rows.map((row) => ({
            ...
            grades: Array.isArray(row.grade_ids) && row.grade_ids.length
                ? row.grade_ids.map((id, idx) => ({ id, name: row.grade_names[idx] }))
                : [],
        }));
        res.json({ students, total: students.length });
    }
    catch (error) {
        utils_1.logger.error('Error fetching teacher students:', error);
...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const group = await studyGroups_1.StudyGroupService.getStudyGroupById(parseInt(id));
        if (!group) {
            return res.status(404).json({ error: 'المجموعة غير موجودة' });
        }
        // جلب عدد الطلاب
        const studentsCount = await studyGroups_1.StudyGroupService.getGroupStudentsCount(parseInt(id));
        res.json({
            group: {
                ...group,
        ...
        utils_1.logger.error('Error fetching study group:', error);
        res.status(500).json({
...
router.post('/:groupId/students', (0, authentication_1.authMiddleware)(['admin', 'teacher']), async (req, res) => {
    try {
        const { groupId } = req.params;
        const { student_id, name, phone, parent_phone } = req.body;
        const teacherId = req.user.id;
        const tenantId = req.tenant.id;
        // التحقق من ملكية المجموعة
        const group = await studyGroups_1.StudyGroupService.getStudyGroupById(parseInt(groupId));
        if (!group) {
            return res.status(404).json({ error: 'المجموعة غير موجودة' });
        }
        ...
        if (student_id) {
            ...
            const existingStudent = await pool_1.default.query('SELECT id FROM users WHERE id = $1 AND role = $2 AND tenant_id = $3', [studentId, 'student', tenantId]);
            ...
            if (phone) {
                const duplicate = await studyGroups_1.StudyGroupService.findStudentByPhone(phone, tenantId);
                if (duplicate) {
                    studentId = duplicate.id;
                }
            }
            if (!studentId) {
                const newStudent = await studyGroups_1.StudyGroupService.createStudentMinimal(name, phone, parent_phone, tenantId);
                studentId = newStudent.id;
            }
        }
        // التحقق من عدم وجود الطالب في المجموعة
        const isStudentInGroup = await studyGroups_1.StudyGroupService.isStudentInGroup(parseInt(groupId), studentId);
        if (isStudentInGroup) {
            return res.status(400).json({ error: 'الطالب موجود بالفعل في المجموعة' });
        }
        const student = await studyGroups_1.StudyGroupService.addStudentToGroup(parseInt(groupId), studentId);
        res.status(201).json({
        ...
        utils_1.logger.error('Error adding student to group:', error);
        res.status(500).json({
...
router.delete('/:groupId/students/:studentId', (0, authentication_1.authMiddleware)(['admin', 'teacher']), async (req, res) => {
    try {
        ...
        const student = await pool_1.default.query(`DELETE FROM group_students
         WHERE group_id = $1
           AND (student_id = $2 OR id = $2)
         RETURNING *`, [groupIdNum, studentIdentifier]);
        ...
        utils_1.logger.error('Error removing student from group:', error);
        res.status(500).json({
...
router.get('/:groupId/students', async (req, res) => {
    try {
        ...
        const group = await studyGroups_1.StudyGroupService.getStudyGroupById(groupIdNum);
        if (!group) {
            return res.status(404).json({ error: 'المجموعة غير موجودة' });
        }
        const students = await studyGroups_1.StudyGroupService.getGroupStudents(groupIdNum, date);
        ...
        const studentsWithQr = await Promise.all(students.map(async (student) => {
            try {
                ...
                utils_1.logger.error('Error generating QR for student', { studentId: student.student_id, err: qrErr });
        ...
        utils_1.logger.error('Error fetching group students:', error);
        res.status(500).json({ error: 'خطأ في جلب طلاب المجموعة' });
    }
});
// 10. تعديل بيانات الطالب في المجموعة
router.put('/:groupId/students/:studentId', (0, authentication_1.authMiddleware)(['admin', 'teacher']), async (req, res) => {
    try {
        const { groupId, studentId } = req.params;
        const { name, phone, parent_phone, payment_status, payment_amount } = req.body;
        const teacherId = req.user.id;
        // التحقق من ملكية المجموعة
        const group = await studyGroups_1.StudyGroupService.getStudyGroupById(parseInt(groupId));
        if (!group) {
        ...
        const isStudentInGroup = await studyGroups_1.StudyGroupService.isStudentInGroup(parseInt(groupId), parseInt(studentId));
        if (!isStudentInGroup) {
            return res.status(404).json({ error: 'الطالب غير موجود في هذه المجموعة' });
        }
        // تحديث بيانات الطالب
        ...
        const result = await pool_1.default.query(`UPDATE users 
         ...
         RETURNING id, name, phone, parent_phone, payment_status, payment_amount, payment_date`, values);
        ...
        utils_1.logger.error('Error updating student:', error);
        res.status(500).json({
...
router.post('/:groupId/attendance', (0, authentication_1.authMiddleware)(['admin', 'teacher']), async (req, res) => {
    try {
        const { groupId } = req.params;
        ...
        for (const entry of attendance) {
            if (!entry.student_id || !['present', 'absent'].includes(entry.status))
                continue;
            await pool_1.default.query(`INSERT INTO group_attendance (group_id, student_id, date, status)
           VALUES ($1, $2, $3, $4)
        ...
        utils_1.logger.error('Error recording attendance:', error);
        res.status(500).json({ error: 'خطأ في تسجيل الحضور' });
    }
});
// 12. جلب حضور الطلاب في مجموعة ليوم معين
router.get('/:groupId/attendance', (0, authentication_1.authMiddleware)(['admin', 'teacher']), async (req, res) => {
    try {
        const { groupId } = req.params;
        const { date } = req.query;
        const teacherId = req.user.id;
        ...
        // جلب الحضور
        const result = await pool_1.default.query(`SELECT ga.student_id, u.name, ga.status
         FROM group_attendance ga
         JOIN users u ON ga.student_id = u.id
         WHERE ga.group_id = $1 AND ga.date = $2`, [groupId, date]);
        res.json({ attendance: result.rows });
    }
    catch (error) {
        utils_1.logger.error('Error fetching attendance:', error);
        res.status(500).json({ error: 'خطأ في جلب الحضور' });
    }
});
// 12.b جلب سجل الحضور والغياب لمدى زمني (آخر أسبوع/شهر...)
router.get('/:groupId/attendance-range', (0, authentication_1.authMiddleware)(['admin', 'teacher']), async (req, res) => {
    try {
        const { groupId } = req.params;
        const { period, days, start_date, end_date } = req.query;
        const teacherId = req.user.id;
        // التحقق من ملكية المجموعة
        const group = await studyGroups_1.StudyGroupService.getStudyGroupById(parseInt(groupId));
        if (!group) {
        ...
        // جلب السجل التفصيلي
        const details = await pool_1.default.query(`SELECT 
           ga.student_id,
           u.name AS student_name,
           ga.date,
           ga.status
         ...
         ORDER BY u.name ASC, ga.date ASC`, [groupId, from, to]);
        // جلب ملخص حسب الطالب
        const summary = await pool_1.default.query(`SELECT 
           ga.student_id,
           u.name AS student_name,
           COUNT(*) AS total_days,
         ...
         ORDER BY u.name ASC`, [groupId, from, to]);
        res.json({
            range: {
                from: from,
                to: to,
        ...
        utils_1.logger.error('Error fetching attendance range:', error);
        res.status(500).json({ error: 'خطأ في جلب سجل الحضور للمدى الزمني' });
    }
});
// 13. جلب تفاصيل حضور طالب معين في مجموعة
router.get('/:groupId/students/:studentId/attendance-details', (0, authentication_1.authMiddleware)(['admin', 'teacher']), async (req, res) => {
    try {
        const { groupId, studentId } = req.params;
        const { month, year, all_time, start_date, end_date } = req.query;
        const teacherId = req.user.id;
        utils_1.logger.info(`Fetching attendance details for group ${groupId}, student ${studentId}`, { query: req.query });
        ...
        // التحقق من ملكية المجموعة
        const group = await studyGroups_1.StudyGroupService.getStudyGroupById(parsedGroupId);
        if (!group) {
            return res.status(404).json({ error: 'المجموعة غير موجودة' });
        }
        if (group.teacher_id !== teacherId) {
        ...
        const studentCheck = await pool_1.default.query(`SELECT 1 FROM (
        ...
        ) as student_check`, [parsedGroupId, parsedStudentId]);
        if (studentCheck.rows.length === 0) {
            utils_1.logger.warn(`Student ${parsedStudentId} not found in group ${parsedGroupId}`);
        ...
        const queryParams = [parsedGroupId, parsedStudentId];
        let paramIndex = 3;
        let rangeInfo = {};
        if (all_time === 'true') {
            // جلب كل السجل
        ...
        // جلب تفاصيل الحضور
        const attendanceResult = await pool_1.default.query(`SELECT 
           ga.date,
           ga.status,
         ...
         WHERE ga.group_id = $1 
           AND ga.student_id = $2
           ${dateFilterClause}
         ORDER BY ga.date ASC`, queryParams);
        // حساب الإحصائيات
        ...
        // جلب معلومات الطالب
        const studentInfo = await pool_1.default.query('SELECT id, name, phone, parent_phone FROM users WHERE id = $1', [studentId]);
        ...
        utils_1.logger.error('Error fetching student attendance details:', error);
        res.status(500).json({
...
router.get('/:groupId/attendance-summary', (0, authentication_1.authMiddleware)(['admin', 'teacher']), async (req, res) => {
    try {
        const { groupId } = req.params;
        ...
        const studentsAttendanceResult = await pool_1.default.query(`SELECT 
           u.id as student_id,
           u.name as student_name,
         ...
         GROUP BY u.id, u.name, u.phone, u.parent_phone
         ORDER BY attendance_rate DESC, u.name ASC`, [groupId, targetMonth, targetYear]);
        // جلب إحصائيات المجموعة ككل
        const groupStatsResult = await pool_1.default.query(`SELECT 
        ...
        utils_1.logger.error('Error fetching group attendance summary:', error);
        res.status(500).json({
...
router.post('/:groupId/students/:studentId/exam-grades', (0, authentication_1.authMiddleware)(['admin', 'teacher']), async (req, res) => {
    try {
        const { groupId, studentId } = req.params;
        ...
        const studentCheck = await pool_1.default.query(`SELECT 1 FROM (
          ...
          UNION
          SELECT student_id FROM group_attendance WHERE group_id = $1 AND student_id = $2
        ) as student_check`, [groupId, studentId]);
        if (studentCheck.rows.length === 0) {
            return res.status(404).json({ error: 'الطالب غير موجود في هذه المجموعة' });
        }
        ...
        let examId;
        const existingExam = await pool_1.default.query('SELECT id FROM group_exams WHERE group_id = $1 AND name = $2', [groupId, exam_name]);
        if (existingExam.rows.length > 0) {
            examId = existingExam.rows[0].id;
        }
        else {
            // إنشاء امتحان جديد
            const newExam = await pool_1.default.query(`INSERT INTO group_exams (group_id, name, total_grade, exam_date, created_at, updated_at)
           VALUES ($1, $2, $3, $4, NOW(), NOW())
           RETURNING id`, [groupId, exam_name, total_grade || 100, exam_date || new Date()]);
            examId = newExam.rows[0].id;
        }
        // إضافة أو تحديث درجة الطالب
        const gradeResult = await pool_1.default.query(`INSERT INTO group_exam_grades (exam_id, student_id, grade, notes, created_at, updated_at)
         VALUES ($1, $2, $3, $4, NOW(), NOW())
         ...
         DO UPDATE SET 
           ...
           updated_at = NOW()
         RETURNING id, grade, notes, created_at, updated_at`, [examId, studentId, grade, notes || null]);
        // جلب معلومات الطالب
        const studentInfo = await pool_1.default.query('SELECT id, name FROM users WHERE id = $1', [studentId]);
        res.json({
        ...
        utils_1.logger.error('Error adding exam grade:', error);
        res.status(500).json({
...
router.get('/:groupId/students/:studentId/exam-grades', (0, authentication_1.authMiddleware)(['admin', 'teacher']), async (req, res) => {
    try {
        const { groupId, studentId } = req.params;
        const teacherId = req.user.id;
        // التحقق من ملكية المجموعة
        ...
        const gradesResult = await pool_1.default.query(`SELECT 
           ge.id as exam_id,
         ...
         WHERE ge.group_id = $1
         ORDER BY ge.exam_date DESC, ge.created_at DESC`, [groupId, studentId]);
        // جلب معلومات الطالب
        const studentInfo = await pool_1.default.query('SELECT id, name, phone, parent_phone FROM users WHERE id = $1', [studentId]);
        // حساب الإحصائيات
        ...
        utils_1.logger.error('Error fetching student exam grades:', error);
        res.status(500).json({
...
router.post('/:groupId/scan-qr', (0, authentication_1.authMiddleware)(['admin', 'teacher']), async (req, res) => {
    try {
        const { groupId } = req.params;
        const { date, qr_data, status } = req.body;
        const teacherId = req.user.id;
        if (!date || !qr_data) {
        ...
        try {
            ...
            if (params.has('student_id')) {
                ...
                if (maybeJson && maybeJson.student_id)
                    studentId = parseInt(maybeJson.student_id, 10);
            }
        }
        catch (parseErr) {
            utils_1.logger.error('QR parse error', { qr_data, err: parseErr });
        }
        ...
        // تحقق من وجود الطالب
        const studentRes = await pool_1.default.query('SELECT id FROM users WHERE id = $1 AND role = $2', [
            studentId,
            'student',
        ]);
        ...
        await pool_1.default.query(`INSERT INTO group_attendance (group_id, student_id, date, status)
         VALUES ($1, $2, $3, $4)
         ON CONFLICT (group_id, student_id, date)
         DO UPDATE SET status = EXCLUDED.status, updated_at = NOW()`, [groupId, studentId, date, attendanceStatus]);
        res.json({
        ...
        utils_1.logger.error('Error scanning QR attendance:', error);
