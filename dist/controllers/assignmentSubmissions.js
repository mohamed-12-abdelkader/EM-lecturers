...
const assignmentSubmissions_1 = require("../services/assignmentSubmissions");
...
const SubmitAssignmentSchema = zod_1.z.object({
    answers: zod_1.z
        ...
        question_id: zod_1.z.union([
            zod_1.z.number().int().positive('question_id يجب أن يكون رقماً صحيحاً موجباً'),
        ...
        option_id: zod_1.z.union([
            zod_1.z.number().int().positive('option_id يجب أن يكون رقماً صحيحاً موجباً'),
            zod_1.z.string().transform((val) => {
                const num = parseInt(val, 10);
                if (isNaN(num) || num <= 0) {
...
async function checkStudentPackageAccess(assignmentId, studentId) {
    try {
        ...
        const isActivated = await packageActivationCodes_1.PackageActivationCodeService.isActivated(packageId, studentId);
        if (!isActivated)
            return false;
        ...
        if (!studentGroupId)
            return false;
        if (lesson.group_id !== studentGroupId)
            return false;
...
router.get('/assignments/:assignmentId/questions', 
...
(0, authentication_1.authMiddleware)(['admin', 'teacher', 'student']), (0, utils_1.asyncWrapper)(async (req, res, next) => {
    try {
        const user = req.user;
        ...
        const hasSubmitted = await assignmentSubmissions_1.AssignmentSubmissionsService.hasStudentSubmitted(assignmentId, user.id);
        // جلب الأسئلة
...
router.get('/me/assignments/:assignmentId/questions', (0, authentication_1.authMiddleware)(['student']), (0, utils_1.asyncWrapper)(async (req, res) => {
    try {
        const assignmentId = parseInt(req.params.assignmentId);
        if (isNaN(assignmentId)) {
            return res.status(400).json({ error: 'Invalid assignment ID' });
...
// 2. تسليم الواجب
router.post('/assignments/:assignmentId/submit', (0, authentication_1.authMiddleware)(['student']), (0, utils_1.asyncWrapper)(async (req, res) => {
    try {
        const assignmentId = parseInt(req.params.assignmentId);
        if (isNaN(assignmentId)) {
            return res.status(400).json({ error: 'Invalid assignment ID' });
        ...
        // تسليم الواجب
        const submission = await assignmentSubmissions_1.AssignmentSubmissionsService.submitAssignment(assignmentId, user.id, parse.data);
        res.status(201).json({
            success: true,
...
router.get('/assignments/:assignmentId/submission', (0, authentication_1.authMiddleware)(['student']), (0, utils_1.asyncWrapper)(async (req, res) => {
    try {
        const assignmentId = parseInt(req.params.assignmentId);
        if (isNaN(assignmentId)) {
            return res.status(400).json({ error: 'Invalid assignment ID' });
        ...
        // جلب تسليم الطالب
        const submission = await assignmentSubmissions_1.AssignmentSubmissionsService.getStudentSubmission(assignmentId, user.id);
        if (!submission) {
            return res.status(404).json({
