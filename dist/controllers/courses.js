...
const courseAccess_1 = require("../services/courseAccess");
const zod_1 = require("zod");
const notifications_1 = require("../services/notifications");
const teacherActivities_1 = require("../services/teacherActivities");
const lectureExam_1 = require("../services/lectureExam");
...
const CreateActivationCodeSchema = zod_1.z.object({
    ...
    count: zod_1.z.number().min(1).max(100).default(1), // عدد الأكواد المطلوبة
});
...
exports.router.post('/', (0, authentication_1.authMiddleware)(['teacher']), uploadCourseImage.single('avatar'), (0, utils_1.asyncWrapper)(async (req, res) => {
    try {
...
exports.router.post('/activation-code', (0, authentication_1.authMiddleware)(['teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    ...
    const courseCheck = await pool_1.default.query('SELECT id FROM courses WHERE id = $1 AND teacher_id = $2', [course_id, teacher_id]);
    ...
    for (let i = 0; i < count; i++) {
        ...
        // إدخال كود التفعيل
        const result = await pool_1.default.query(`INSERT INTO teacher_invite_codes (code, course_id, teacher_id, max_uses, expires_at)
         VALUES ($1, $2, $3, $4, $5)
         RETURNING id, code, max_uses, expires_at, created_at`, [code, course_id, teacher_id, 1, null]);
...
exports.router.get('/my-activation-codes', (0, authentication_1.authMiddleware)(['teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const teacher_id = req.user.id;
    ...
    const activationCodesWithQR = await Promise.all(result.rows.map(async (row) => {
        try {
            const qrCodeData = {
...
exports.router.post('/admin/activate-by-code', (0, authentication_1.authMiddleware)(['admin']), (0, utils_1.asyncWrapper)(async (req, res) => {
    ...
    const codeCheck = await pool_1.default.query(`SELECT tic.id, tic.code, tic.course_id, tic.max_uses, tic.uses, tic.expires_at,
              c.title as course_title
       FROM teacher_invite_codes tic
       JOIN courses c ON tic.course_id = c.id
       WHERE tic.code = $1`, [code]);
    ...
    const studentCheck = await pool_1.default.query('SELECT id, name, email FROM users WHERE id = $1 AND role = $2', [studentId, 'student']);
    if (!studentCheck.rowCount) {
        return res.status(404).json({ message: 'الطالب غير موجود أو ليس حساب طالب' });
    }
    const usageCheck = await pool_1.default.query('SELECT id FROM invite_code_usages WHERE user_id = $1 AND code_id = $2', [studentId, row.id]);
    ...
    await pool_1.default.query('UPDATE teacher_invite_codes SET uses = uses + 1 WHERE id = $1', [row.id]);
    await pool_1.default.query('INSERT INTO enrollments (user_id, course_id) VALUES ($1, $2) ON CONFLICT (user_id, course_id) DO NOTHING', [studentId, row.course_id]);
    try {
        const gradeRes = await pool_1.default.query('SELECT grade_id, teacher_id FROM courses WHERE id = $1', [row.course_id]);
        if (gradeRes.rowCount) {
            const gradeId = gradeRes.rows[0].grade_id;
            const teacherId = gradeRes.rows[0].teacher_id;
            const group = await chat_1.ChatService.getOrCreateTeacherGradeGroup(gradeId, teacherId);
            await chat_1.ChatService.addMember(group.id, studentId, 'student');
        }
    }
    catch (err) {
    ...
    try {
        await pool_1.default.query(`INSERT INTO notifications (user_id, title, message, type, course_id) VALUES ($1, $2, $3, $4, $5)`, [studentId, 'كورس جديد متاح', `تم تفعيل كورس "${row.course_title}" لك`, 'course_opened', row.course_id]);
...
exports.router.get('/admin/activation-code/:code', (0, authentication_1.authMiddleware)(['admin']), (0, utils_1.asyncWrapper)(async (req, res) => {
...
exports.router.post('/activate', (0, authentication_1.authMiddleware)(['student']), (0, utils_1.asyncWrapper)(async (req, res) => {
    ...
    const codeCheck = await pool_1.default.query(`SELECT 
       ...
       FROM teacher_invite_codes tic
       JOIN courses c ON tic.course_id = c.id
       WHERE tic.code = $1 AND tic.course_id = $2`, [code, course_id]);
    if (!codeCheck.rowCount) {
    ...
    const usageCheck = await pool_1.default.query('SELECT id FROM invite_code_usages WHERE user_id = $1 AND code_id = $2', [student_id, activationCode.id]);
    ...
    await pool_1.default.query('UPDATE teacher_invite_codes SET uses = uses + 1 WHERE id = $1', [
        activationCode.id,
    ]);
    // إضافة الطالب للكورس (enrollment)
    await pool_1.default.query('INSERT INTO enrollments (user_id, course_id) VALUES ($1, $2) ON CONFLICT (user_id, course_id) DO NOTHING', [student_id, activationCode.course_id]);
    // إضافة الطالب تلقائياً لمجموعة دردشة المدرس/الصف المرتبط بالكورس
    try {
        const gradeRes = await pool_1.default.query('SELECT grade_id, teacher_id FROM courses WHERE id = $1', [
        ...
        if (gradeRes.rowCount) {
            ...
            const teacherId = gradeRes.rows[0].teacher_id;
            const group = await chat_1.ChatService.getOrCreateTeacherGradeGroup(gradeId, teacherId);
            await chat_1.ChatService.addMember(group.id, student_id, 'student');
        }
    }
    catch (err) {
...
exports.router.post('/activate-free', (0, authentication_1.authMiddleware)(['student']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const courseId = Number((0, requestParsers_1.pickBodyValue)(req.body, 'course_id'));
    ...
    const existingEnrollment = await pool_1.default.query('SELECT id FROM enrollments WHERE user_id = $1 AND course_id = $2', [studentId, courseId]);
    ...
    await pool_1.default.query('INSERT INTO enrollments (user_id, course_id) VALUES ($1, $2) ON CONFLICT (user_id, course_id) DO NOTHING', [studentId, courseId]);
    try {
        const group = await chat_1.ChatService.getOrCreateTeacherGradeGroup(course.grade_id, course.teacher_id);
        await chat_1.ChatService.addMember(group.id, studentId, 'student');
    }
    catch (err) {
    ...
    try {
        await pool_1.default.query(`INSERT INTO notifications (user_id, title, message, type, course_id) VALUES ($1, $2, $3, $4, $5)`, [studentId, 'كورس جديد متاح', `تم تفعيل كورس "${course.title}" لك`, 'course_opened', courseId]);
        ExpoPushService.sendPushNotification(studentId, 'كورس جديد متاح', `تم تفعيل كورس "${course.title}" لك`, { type: 'course_opened', course_id: courseId }).catch((e) => console.error('Expo push error:', e));
    }
    catch (_) {
        // ...
    }
...
exports.router.post('/scan-qr-activate', (0, authentication_1.authMiddleware)(['student']), (0, utils_1.asyncWrapper)(async (req, res) => {
    ...
    try {
        ...
        const codeCheck = await pool_1.default.query(`SELECT 
         ...
         FROM teacher_invite_codes tic
         JOIN courses c ON tic.course_id = c.id
         WHERE tic.code = $1 AND tic.course_id = $2`, [activation_code, course_id]);
        if (!codeCheck.rowCount) {
        ...
        // تسجيل استخدام الكود
        await pool_1.default.query('INSERT INTO invite_code_usages (user_id, code_id, used_at) VALUES ($1, $2, NOW())', [student_id, activationCode.id]);
        // تحديث عدد مرات الاستخدام
        await pool_1.default.query('UPDATE teacher_invite_codes SET uses = uses + 1 WHERE id = $1', [
            activationCode.id,
        ]);
        // إضافة الطالب للكورس
        const enrollmentCheck = await pool_1.default.query('SELECT id FROM enrollments WHERE user_id = $1 AND course_id = $2', [student_id, course_id]);
        if (!enrollmentCheck.rowCount) {
            await pool_1.default.query('INSERT INTO enrollments (user_id, course_id, enrolled_at) VALUES ($1, $2, NOW())', [student_id, course_id]);
        }
        res.json({
            success: true,
            message: 'Course activated successfully',
...
// تعديل كورس (مدرس فقط)
exports.router.put('/:id', (0, authentication_1.authMiddleware)(['teacher']), uploadCourseImage.single('avatar'), (0, utils_1.asyncWrapper)(async (req, res) => {
    try {
        const courseId = req.params.id;
        const teacher_id = req.user.id;
        // تحقق أن الكورس يخص المدرس
        const courseCheck = await pool_1.default.query('SELECT * FROM courses WHERE id = $1 AND teacher_id = $2', [courseId, teacher_id]);
        if (!courseCheck.rowCount)
            return res.status(404).json({ message: 'Course not found or not yours' });
        // تحقق من البيانات
...
// حذف كورس (مدرس فقط)
exports.router.delete('/:id', (0, authentication_1.authMiddleware)(['teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const courseId = req.params.id;
    const teacher_id = req.user.id;
...
exports.router.get('/teacher/students', (0, authentication_1.authMiddleware)(['teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const teacherId = req.user.id;
    try {
        const students = await teacherReports_1.TeacherReportsService.getTeacherStudents(teacherId);
        res.json({
            students,
...
exports.router.get('/teacher/students/report-by-name', (0, authentication_1.authMiddleware)(['teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const teacherId = req.user.id;
    ...
    try {
        const result = await teacherReports_1.TeacherReportsService.getStudentReportByName(teacherId, name.trim());
        if ('report' in result) {
...
// يجب أن يكون قبل /teacher/:teacherId لتجنب التداخل
exports.router.get('/teacher/students/:studentId/report', (0, authentication_1.authMiddleware)(['teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const teacherId = req.user.id;
...
exports.router.get('/teacher/:teacherId', (0, authentication_1.authMiddleware)(['student']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const teacherId = Number(req.params.teacherId);
    const studentId = req.user.id;
    // احصل على الصفوف الدراسية للطالب
    const gradesRes = await pool_1.default.query('SELECT grade_id FROM user_grades WHERE user_id = $1', [
        studentId,
    ]);
    ...
    const coursesRes = await pool_1.default.query(`SELECT 
       ...
       ORDER BY c.created_at DESC`, [studentId, teacherId, gradeIds]);
    res.json({
...
exports.router.get('/my-courses', (0, authentication_1.authMiddleware)(['teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const teacher_id = req.user.id;
...
exports.router.get('/my-enrollments', (0, authentication_1.authMiddleware)(['student']), (0, utils_1.asyncWrapper)(async (req, res) => {
    try {
        const studentId = req.user.id;
        // جلب الكورسات المشترك فيها الطالب
        const coursesResult = await pool_1.default.query(`SELECT c.id, c.title, c.price, c.description, c.teacher_id, c.avatar, c.created_at, c.grade_id
         FROM enrollments e
         JOIN courses c ON e.course_id = c.id
         WHERE e.user_id = $1 AND c.is_visible = true
         ORDER BY c.created_at DESC`, [studentId]);
        // جلب الكورسات العامة المشترك فيها الطالب
        const generalCoursesResult = await pool_1.default.query(`SELECT gc.id, gc.title, gc.price, gc.description, gc.image, gc.category, gc.created_at,
         ...
         ORDER BY gce.enrolled_at DESC`, [studentId]);
        // جلب الباقات المشترك فيها الطالب
        const packagesResult = await pool_1.default.query(`SELECT p.id, p.name as title, p.price, p.image as avatar, p.grade_id, p.created_at
...
exports.router.post('/:courseId/lectures', (0, authentication_1.authMiddleware)(['teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    ...
    // تحقق أن الكورس يخص المدرس
    const courseCheck = await pool_1.default.query('SELECT id, title FROM courses WHERE id = $1 AND teacher_id = $2', [courseId, teacherId]);
    if (!courseCheck.rowCount) {
        return res.status(404).json({ message: 'Course not found or not yours' });
    }
    ...
    let pos = position;
    if (!pos) {
        const posRes = await pool_1.default.query('SELECT COALESCE(MAX(position), 0) + 1 AS next_pos FROM lectures WHERE course_id = $1', [courseId]);
        pos = posRes.rows[0].next_pos;
    }
    const result = await pool_1.default.query(`INSERT INTO lectures (course_id, title, description, position, is_visible) VALUES ($1, $2, $3, $4, $5) RETURNING *`, [courseId, title, description, pos, true]);
    ...
    if (lecture.is_visible !== false) {
        await notifications_1.NotificationService.notifyLectureAdded(courseId, lecture.id, title, courseTitle);
    }
    res.status(201).json({ lecture: result.rows[0] });
}));
// إضافة فيديو لمحاضرة (للأستاذ فقط)
const LectureVideoSchema = zod_1.z.object({
    video_url: zod_1.z.string().url(),
    title: zod_1.z.string().optional(),
    position: zod_1.z.number().optional(),
});
exports.router.post('/lecture/:lectureId/videos', (0, authentication_1.authMiddleware)(['teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    ...
    const lectureCheck = await pool_1.default.query(`SELECT l.id, l.title as lecture_title, c.id as course_id, c.title as course_title 
       FROM lectures l JOIN courses c ON l.course_id = c.id WHERE l.id = $1 AND c.teacher_id = $2`, [lectureId, teacherId]);
    ...
    let pos = position;
    if (!pos) {
        const posRes = await pool_1.default.query('SELECT COALESCE(MAX(position), 0) + 1 AS next_pos FROM lecture_videos WHERE lecture_id = $1', [lectureId]);
        pos = posRes.rows[0].next_pos;
    }
    const result = await pool_1.default.query(`INSERT INTO lecture_videos (lecture_id, video_url, title, position) VALUES ($1, $2, $3, $4) RETURNING *`, [lectureId, video_url, title, pos]);
    // إرسال إشعار للطلاب المشتركين في الكورس
    const lectureData = lectureCheck.rows[0];
    const videoTitle = title || 'فيديو جديد';
    await notifications_1.NotificationService.notifyVideoAdded(lectureData.course_id, lectureId, videoTitle, lectureData.lecture_title, lectureData.course_title);
    res.status(201).json({ video: result.rows[0] });
}));
// تحديث فيديو محاضرة (للأستاذ فقط)
exports.router.put('/lecture-video/:videoId', (0, authentication_1.authMiddleware)(['teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    ...
    const videoCheck = await pool_1.default.query(`SELECT lv.id, lv.title as video_title, l.id as lecture_id, l.title as lecture_title, c.id as course_id, c.title as course_title 
       FROM lecture_videos lv 
       JOIN lectures l ON lv.lecture_id = l.id 
       JOIN courses c ON l.course_id = c.id 
       WHERE lv.id = $1 AND c.teacher_id = $2`, [videoId, teacherId]);
    ...
    values.push(videoId);
    const result = await pool_1.default.query(`UPDATE lecture_videos SET ${updateFields.join(', ')} WHERE id = $${i} RETURNING *`, values);
    res.json({ video: result.rows[0] });
}));
// حذف فيديو محاضرة (للأستاذ فقط)
exports.router.delete('/lecture-video/:videoId', (0, authentication_1.authMiddleware)(['teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const videoId = Number(req.params.videoId);
    const teacherId = req.user.id;
    // تحقق أن الفيديو يخص محاضرة في كورس يملكه المدرس
    const videoCheck = await pool_1.default.query(`SELECT lv.id, lv.title as video_title, l.id as lecture_id, l.title as lecture_title, c.id as course_id, c.title as course_title 
    ...
    await pool_1.default.query('DELETE FROM lecture_videos WHERE id = $1', [videoId]);
    res.json({ message: 'Lecture video deleted successfully' });
}));
// إضافة ملف PDF لمحاضرة (للأستاذ فقط)
const LectureFileSchema = zod_1.z.object({
    file_url: zod_1.z.string().url(),
    filename: zod_1.z.string(),
});
exports.router.post('/lecture/:lectureId/files', (0, authentication_1.authMiddleware)(['teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const lectureId = Number(req.params.lectureId);
    const teacherId = req.user.id;
    ...
    const result = await pool_1.default.query(`INSERT INTO lecture_files (lecture_id, file_url, filename) VALUES ($1, $2, $3) RETURNING *`, [lectureId, file_url, filename]);
    // إرسال إشعار للطلاب المشتركين في الكورس
    const lectureData = lectureCheck.rows[0];
    await notifications_1.NotificationService.notifyFileAdded(lectureData.course_id, lectureId, filename, lectureData.lecture_title, lectureData.course_title);
    res.status(201).json({ file: result.rows[0] });
}));
// تغيير حالة ظهور الكورس (إظهار/إخفاء للطلاب)
exports.router.patch('/:courseId/visibility', (0, authentication_1.authMiddleware)(['teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const courseId = Number(req.params.courseId);
    ...
    // تحقق أن الكورس يخص المدرس
    const courseCheck = await pool_1.default.query('SELECT id, title FROM courses WHERE id = $1 AND teacher_id = $2', [courseId, req.user.id]);
    if (!courseCheck.rowCount) {
        return res.status(404).json({ message: 'Course not found or not yours' });
    }
    try {
        // محاولة تحديث العمود is_visible
        const result = await pool_1.default.query('UPDATE courses SET is_visible = $1 WHERE id = $2 RETURNING id, title, is_visible, updated_at', [is_visible, courseId]);
        res.json({
        ...
        if (error.message && error.message.includes('column "is_visible" does not exist')) {
            ...
            try {
                // إضافة العمود
                await pool_1.default.query('ALTER TABLE courses ADD COLUMN is_visible BOOLEAN DEFAULT TRUE');
                // تحديث الكورس المحدد
                const result = await pool_1.default.query('UPDATE courses SET is_visible = $1 WHERE id = $2 RETURNING id, title, is_visible, updated_at', [is_visible, courseId]);
                res.json({
                    message: is_visible ? 'تم إظهار الكورس للطلاب' : 'تم إخفاء الكورس عن الطلاب',
...
exports.router.get('/:courseId/details', (0, authentication_1.authMiddleware)(), (0, utils_1.asyncWrapper)(async (req, res) => {
    const courseId = Number(req.params.courseId);
    const user = req.user;
    ...
    if (user.role === 'student') {
        const accessCheck = await courseAccess_1.CourseAccessService.checkStudentAccess(user.id, courseId);
    ...
    if (courseInfo.type === 'regular') {
        ...
        const lecturesRes = await pool_1.default.query(`SELECT * FROM lectures WHERE course_id = $1 ORDER BY position, created_at`, [courseId]);
        lectures = lecturesRes.rows;
    }
    else {
        // للكورسات في المواد الدراسية، استخدم جدول course_lectures
        const lecturesRes = await pool_1.default.query(`SELECT * FROM course_lectures WHERE course_id = $1 ORDER BY order_index, created_at`, [courseId]);
        lectures = lecturesRes.rows;
    }
    ...
    if (lectureIds.length) {
        const videosRes = await pool_1.default.query(`SELECT * FROM lecture_videos WHERE lecture_id = ANY($1::int[]) ORDER BY position, id`, [lectureIds]);
        videos = videosRes.rows;
        const filesRes = await pool_1.default.query(`SELECT * FROM lecture_files WHERE lecture_id = ANY($1::int[]) ORDER BY uploaded_at, id`, [lectureIds]);
        files = filesRes.rows;
    }
    // جلب الامتحانات لكل محاضرة
    let exams = [];
    if (lectureIds.length) {
        const examsRes = await pool_1.default.query(`SELECT * FROM exams WHERE lecture_id = ANY($1::int[]) AND type = 'exam'`, [lectureIds]);
        exams = examsRes.rows;
    }
...
exports.router.get('/:courseId/course-exams', (0, authentication_1.authMiddleware)(['teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const courseId = Number(req.params.courseId);
...
exports.router.get('/course-exam/:examId/questions', (0, authentication_1.authMiddleware)(['teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    ...
    try {
        ...
        const examRes = await pool_1.default.query(`SELECT e.*, c.id as course_id, c.teacher_id
        ...
        const questionsRes = await pool_1.default.query(`SELECT id, type, question_text, question_image, option_a, option_b, option_c, option_d, correct_answer, exam_id
         FROM course_level_exam_questions
         WHERE exam_id = $1::integer
         ORDER BY id ASC`, [examId]);
        ...
        if (questionsRes.rowCount === 0) {
            const debugRes = await pool_1.default.query(`SELECT COUNT(*) as count FROM course_level_exam_questions WHERE exam_id = $1`, [examId]);
            ...
            if (dbCount > 0) {
                ...
                const sampleRes = await pool_1.default.query(`SELECT id, exam_id FROM course_level_exam_questions WHERE exam_id = $1 LIMIT 1`, [examId]);
...
exports.router.delete('/course-exam/question/:questionId', (0, authentication_1.authMiddleware)(['teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    ...
    try {
        ...
        let questionCheck = await pool_1.default.query(`SELECT q.id as row_id, e.id as exam_id, c.teacher_id, 'course_level' as exam_type
        ...
        if (!questionCheck.rowCount) {
            ...
            try {
                await pool_1.default.query(`ALTER TABLE course_level_exam_questions
            ...
            const byBankId = await pool_1.default.query(`SELECT q.id as row_id, e.id as exam_id, c.teacher_id, 'course_level' as exam_type
           FROM course_level_exam_questions q
           JOIN course_level_exams e ON q.exam_id = e.id
           JOIN courses c ON e.course_id = c.id
           WHERE q.question_id_v2 = $1 OR q.question_id = $1`, [questionId]);
            if (byBankId.rowCount)
                questionCheck = byBankId;
            // إن لم يُوجَد (الأسئلة القديمة قد تكون question_id_v2 = NULL): ابحث بمطابقة المحتوى من البنك
            if (!questionCheck.rowCount) {
                let matchImage = null;
                let matchText = null;
                let fromV2 = false;
                const v2Row = await pool_1.default.query(`SELECT q.question_text, qm.media_url
             FROM questions_v2 q
             ...
             WHERE q.id = $1`, [questionId]);
                if (v2Row.rowCount) {
                    ...
                    const v1Row = await pool_1.default.query(`SELECT text as question_text, image FROM questions WHERE id = $1`, [questionId]);
                    if (v1Row.rowCount) {
                ...
                if (matchImage != null || matchText != null) {
                    const byContent = await pool_1.default.query(`SELECT q.id as row_id, e.id as exam_id, c.teacher_id, 'course_level' as exam_type
               FROM course_level_exam_questions q
               JOIN course_level_exams e ON q.exam_id = e.id
               JOIN courses c ON e.course_id = c.id
                 ...
                 AND (
                   ...
                   OR ($4::text IS NOT NULL AND q.question_text = $4)
                    ...
                    if (byContent.rowCount === 1) {
                        ...
                        if (fromV2) {
                            await pool_1.default.query(`UPDATE course_level_exam_questions SET question_id_v2 = $1 WHERE id = $2`, [questionId, rowId]).catch(() => { });
                        }
                        else {
                            await pool_1.default.query(`UPDATE course_level_exam_questions SET question_id = $1 WHERE id = $2`, [questionId, rowId]).catch(() => { });
                        }
                    }
                }
            }
        ...
        if (!questionCheck.rowCount) {
            questionCheck = await pool_1.default.query(`SELECT ceq.id as row_id, ce.id as exam_id, c.teacher_id, 'course_exam' as exam_type
           FROM questions q
        ...
        if (!questionCheck.rowCount) {
            const lectureExamCheck = await pool_1.default.query(`SELECT eq.id as row_id, e.id as exam_id, c.teacher_id
           FROM exam_questions eq
           JOIN exams e ON eq.exam_id = e.id
           JOIN lectures l ON e.lecture_id = l.id
           JOIN courses c ON l.course_id = c.id
           WHERE eq.id = $1 AND c.teacher_id = $2`, [questionId, req.user.id]);
            if (lectureExamCheck.rowCount) {
                await pool_1.default.query('DELETE FROM exam_questions WHERE id = $1', [questionId]);
        ...
        if (question.exam_type === 'course_level') {
            ...
            await pool_1.default.query('DELETE FROM course_level_exam_questions WHERE id = $1', [idToDelete]);
            return res.json({ message: 'تم حذف السؤال بنجاح' });
        }
        else {
            // حذف من course_exam_questions
            const result = await exams_1.ExamsService.deleteCourseExamQuestion(questionId);
...
exports.router.put('/course-exam/question/:questionId', (0, authentication_1.authMiddleware)(['teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const questionId = Number(req.params.questionId);
    if (Number.isNaN(questionId)) {
        return res.status(400).json({ message: 'Invalid question id' });
    }
    try {
        ...
        let questionCheck = await pool_1.default.query(`SELECT e.id as exam_id, c.teacher_id, 'course_level' as exam_type,
        ...
        if (!questionCheck.rowCount) {
            questionCheck = await pool_1.default.query(`SELECT ce.id as exam_id, c.teacher_id, 'course_exam' as exam_type
           FROM questions q
           JOIN course_exam_questions ceq ON q.id = ceq.question_id
           JOIN course_exams ce ON ceq.course_exam_id = ce.id
           JOIN courses c ON ce.course_id = c.id
        ...
        const { text, choices, image, questionText, questionImage, optionA, optionB, optionC, optionD, correctAnswer, } = req.body;
        ...
        if (question.exam_type === 'course_level') {
            ...
            if (choices) {
                ...
                const correctCount = processedChoices.filter((c) => c.is_correct === true || c.is_correct === 'true').length;
                if (correctCount !== 1) {
                    return res.status(400).json({ message: 'يجب أن يكون هناك اختيار صحيح واحد فقط' });
                }
            }
            // تحديث السؤال
            const result = await exams_1.ExamsService.updateCourseExamQuestion(questionId, finalText, undefined, // grade - سيتم تعيينه تلقائياً إلى 1
            processedChoices, finalImage);
            return res.json(result);
        }
    }
    catch (error) {
...
exports.router.patch('/course-exam/question/:questionId/image', (0, authentication_1.authMiddleware)(['teacher']), uploadQuestionImage.single('questionImage'), (0, utils_1.asyncWrapper)(async (req, res) => {
    const questionId = Number(req.params.questionId);
    if (Number.isNaN(questionId)) {
        return res.status(400).json({ message: 'Invalid question id' });
    }
    ...
    try {
        ...
        if (row.exam_type === 'course_level') {
            await pool_1.default.query(`UPDATE course_level_exam_questions SET question_image = $1, updated_at = NOW() WHERE id = $2`, [uploadedUrl, row.row_id]);
            const updated = await pool_1.default.query(`SELECT * FROM course_level_exam_questions WHERE id = $1`, [row.row_id]);
            return res.json({
        ...
        await pool_1.default.query(`UPDATE questions SET image = $1 WHERE id = $2`, [uploadedUrl, questionId]);
        return res.json({
            message: 'تمت إضافة صورة السؤال بنجاح',
            questionImage: uploadedUrl,
        });
...
exports.router.patch('/course-exam/question/:questionId/correct-answer', (0, authentication_1.authMiddleware)(['teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const questionId = Number(req.params.questionId);
    if (Number.isNaN(questionId)) {
        return res.status(400).json({ message: 'Invalid question id' });
    }
    ...
    try {
        ...
        if (question.exam_type === 'course_level') {
           ...
           WHERE id = $2
           RETURNING *`, [finalCorrectAnswer, questionId]);
            if (!result.rowCount) {
                return res.status(404).json({ message: 'Question not found' });
            }
            ...
            const choicesRes = await pool_1.default.query(`SELECT id FROM question_choices WHERE question_id = $1 ORDER BY id ASC`, [questionId]);
            ...
            const result = await exams_1.ExamsService.setCourseExamQuestionCorrectAnswer(questionId, correctChoiceId);
            return res.json(result);
        }
    }
    catch (error) {
...
exports.router.get('/:courseId/enrollments', (0, authentication_1.authMiddleware)(['teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const courseId = Number(req.params.courseId);
    const teacherId = req.user.id;
    // تحقق أن الكورس يخص المدرس
    const courseCheck = await pool_1.default.query('SELECT id FROM courses WHERE id = $1 AND teacher_id = $2', [courseId, teacherId]);
    if (!courseCheck.rowCount) {
        return res.status(404).json({ message: 'Course not found or not yours' });
    }
    // جلب الطلاب المشتركين مع كود التفعيل المستخدم
    const result = await pool_1.default.query(`SELECT u.id as student_id, u.name, u.email, u.phone, u.avatar, e.enrolled_at, tic.code as activation_code,
...
exports.router.patch('/lecture/:lectureId/visibility', (0, authentication_1.authMiddleware)(['teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const lectureId = Number(req.params.lectureId);
    const { is_visible } = req.body;
    if (typeof is_visible !== 'boolean') {
        return res.status(400).json({ message: 'is_visible (boolean) is required' });
    ...
    const lecCheck = await pool_1.default.query(`SELECT l.* FROM lectures l JOIN courses c ON l.course_id = c.id WHERE l.id = $1 AND c.teacher_id = $2`, [lectureId, req.user.id]);
    if (!lecCheck.rowCount) {
        return res.status(404).json({ message: 'Lecture not found or not yours' });
    }
    const result = await pool_1.default.query(`UPDATE lectures SET is_visible = $1 WHERE id = $2 RETURNING *`, [is_visible, lectureId]);
    // إرسال إشعار للطلاب عند جعل المحاضرة ظاهرة
    if (is_visible && result.rowCount && result.rowCount > 0) {
        try {
            const lecture = result.rows[0];
            const courseInfo = await pool_1.default.query(`SELECT id, title FROM courses WHERE id = $1`, [lecture.course_id]);
            if (courseInfo.rowCount && courseInfo.rowCount > 0) {
                await notifications_1.NotificationService.notifyLectureAdded(lecture.course_id, lecture.id, lecture.title, courseInfo.rows[0].title);
            }
        }
        catch (notifError) {
...
exports.router.post('/lecture/:lectureId/exam', (0, authentication_1.authMiddleware)(['teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    ...
    const lectureCheck = await pool_1.default.query(`SELECT l.*, c.teacher_id FROM lectures l 
       JOIN courses c ON l.course_id = c.id 
       WHERE l.id = $1 AND c.teacher_id = $2`, [lectureId, req.user.id]);
    if (!lectureCheck.rowCount) {
        return res.status(404).json({ message: 'Lecture not found or not yours' });
    }
    // معالجة البيانات
    ...
    if (visibility) {
        try {
            const courseInfo = await pool_1.default.query(`SELECT c.id as course_id, c.title as course_title, l.title as lecture_title
           FROM lectures l
           JOIN courses c ON l.course_id = c.id
           WHERE l.id = $1`, [lectureId]);
            if (courseInfo.rowCount) {
                const result = await notifications_1.NotificationService.notifyExamAdded(courseInfo.rows[0].course_id, lectureId, exam.rows[0].id, examTitle, courseInfo.rows[0].lecture_title, courseInfo.rows[0].course_title);
...
exports.router.patch('/lecture/exam/:examId/visibility', (0, authentication_1.authMiddleware)(['teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const examId = Number(req.params.examId);
    const { is_visible } = req.body;
    if (typeof is_visible !== 'boolean') {
        return res.status(400).json({ message: 'is_visible (boolean) is required' });
    ...
    const examCheck = await pool_1.default.query(`SELECT e.* FROM exams e 
       JOIN lectures l ON e.lecture_id = l.id 
       JOIN courses c ON l.course_id = c.id 
       WHERE e.id = $1 AND c.teacher_id = $2`, [examId, req.user.id]);
    if (!examCheck.rowCount) {
        // تحقق إذا كان الامتحان موجود أصلاً
        const examExists = await pool_1.default.query('SELECT id FROM exams WHERE id = $1', [examId]);
        ...
        const examDetails = await pool_1.default.query(`SELECT e.id, e.lecture_id, e.type, l.course_id, c.teacher_id 
         FROM exams e 
    ...
    const result = await pool_1.default.query(`UPDATE exams SET is_visible = $1 WHERE id = $2 RETURNING *`, [
        is_visible,
        examId,
    ]);
    ...
    if (is_visible && result.rowCount && result.rowCount > 0) {
        try {
            const examInfo = await pool_1.default.query(`SELECT c.id as course_id, c.title as course_title, l.id as lecture_id, l.title as lecture_title, e.title as exam_title
           FROM exams e
           JOIN lectures l ON e.lecture_id = l.id
           JOIN courses c ON l.course_id = c.id
           WHERE e.id = $1`, [examId]);
            if (examInfo.rowCount) {
                ...
                await notifications_1.NotificationService.notifyExamAdded(info.course_id, info.lecture_id, examId, info.exam_title || 'امتحان', info.lecture_title, info.course_title);
...
exports.router.get('/lecture/:lectureId/exam', (0, authentication_1.authMiddleware)(['teacher', 'student']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const lectureId = Number(req.params.lectureId);
    const user = req.user;
    if (isNaN(lectureId)) {
        return res.status(400).json({ message: 'Invalid lecture ID' });
    ...
    if (user.role === 'teacher') {
        const lectureCheck = await pool_1.default.query(`SELECT l.* FROM lectures l 
         JOIN courses c ON l.course_id = c.id 
         WHERE l.id = $1 AND c.teacher_id = $2`, [lectureId, user.id]);
        if (lectureCheck.rowCount)
            isAllowed = true;
    }
    else if (user.role === 'student') {
        const enrollCheck = await pool_1.default.query(`SELECT 1 FROM enrollments e 
         JOIN lectures l ON e.course_id = l.course_id 
         WHERE l.id = $1 AND e.user_id = $2`, [lectureId, user.id]);
        if (enrollCheck.rowCount)
            isAllowed = true;
    }
    if (!isAllowed) {
    ...
    const examRes = await pool_1.default.query(examQuery, examParams);
    if (!examRes.rowCount) {
...
// تحديث امتحان محاضرة
exports.router.patch('/lecture/exam/:examId', (0, authentication_1.authMiddleware)(['teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const examId = Number(req.params.examId);
    ...
    const examCheck = await pool_1.default.query(`SELECT e.*, l.title as lecture_title, c.id as course_id, c.title as course_title 
       FROM exams e 
       JOIN lectures l ON e.lecture_id = l.id 
       JOIN courses c ON l.course_id = c.id 
       WHERE e.id = $1 AND c.teacher_id = $2`, [examId, req.user.id]);
    ...
    values.push(examId);
    const result = await pool_1.default.query(`UPDATE exams SET ${updateFields.join(', ')} WHERE id = $${i} RETURNING *`, values);
    res.json({ exam: result.rows[0] });
}));
// حذف امتحان محاضرة
exports.router.delete('/lecture/exam/:examId', (0, authentication_1.authMiddleware)(['teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const examId = Number(req.params.examId);
    if (isNaN(examId)) {
        return res.status(400).json({ message: 'Invalid exam ID' });
    }
    ...
    await pool_1.default.query('DELETE FROM exams WHERE id = $1', [examId]);
    res.json({ message: 'Lecture exam deleted successfully' });
}));
// Debug endpoint لفحص حالة الامتحانات المانعة للوصول
exports.router.get('/lecture/:lectureId/debug-access', (0, authentication_1.authMiddleware)(['student', 'teacher', 'admin']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const lectureId = Number(req.params.lectureId);
    ...
    const enrollmentCheck = await pool_1.default.query(`SELECT 1 FROM enrollments e 
       JOIN lectures l ON e.course_id = l.course_id 
       WHERE l.id = $1 AND e.user_id = $2`, [lectureId, userId]);
    if (!enrollmentCheck.rowCount) {
        return res.status(403).json({ message: 'Not enrolled in this course' });
    }
    const debugInfo = await lectureExam_1.LectureExamService.debugBlockingExams(lectureId, userId);
    res.json(debugInfo);
}));
// التحقق من إمكانية الوصول لمحاضرة معينة للطالب
exports.router.get('/lecture/:lectureId/access-check', (0, authentication_1.authMiddleware)(['student']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const lectureId = Number(req.params.lectureId);
    const studentId = req.user.id;
    if (isNaN(lectureId)) {
        return res.status(400).json({ message: 'Invalid lecture ID' });
    ...
    const lectureCheck = await pool_1.default.query('SELECT id FROM lectures WHERE id = $1', [lectureId]);
    if (lectureCheck.rowCount) {
        canAccess = await lectureExam_1.LectureExamService.canStudentAccessLecture(lectureId, studentId);
        blockingExams = await lectureExam_1.LectureExamService.getBlockingExamsForLecture(lectureId, studentId);
    }
    res.json({
...
exports.router.patch('/lecture/exam/:examId/advanced-settings', (0, authentication_1.authMiddleware)(['teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const examId = Number(req.params.examId);
    const { show_at, hide_at, lock_next_lectures, show_answers_immediately, show_answers_after_hours, } = req.body;
    if (isNaN(examId)) {
        return res.status(400).json({ message: 'Invalid exam ID' });
    }
    // تحقق من أن الامتحان يخص محاضرة في كورس يملكه المدرس
...
exports.router.post('/activate-student', (0, authentication_1.authMiddleware)(['teacher', 'admin']), (0, utils_1.asyncWrapper)(async (req, res) => {
    ...
    const courseCheck = await pool_1.default.query(isAdmin
        ? 'SELECT id, title, teacher_id FROM courses WHERE id = $1'
        : 'SELECT id, title, teacher_id FROM courses WHERE id = $1 AND teacher_id = $2', isAdmin ? [courseId] : [courseId, teacherId]);
    if (!courseCheck.rowCount) {
        return res.status(404).json({
    ...
    try {
        await pool_1.default.query('INSERT INTO enrollments (user_id, course_id) VALUES ($1, $2)', [
            studentId,
            courseId,
        ]);
        try {
            const gradeRes = await pool_1.default.query('SELECT grade_id, teacher_id FROM courses WHERE id = $1', [courseId]);
            if (gradeRes.rowCount) {
                const gradeId = gradeRes.rows[0].grade_id;
                const courseTeacherId = gradeRes.rows[0].teacher_id;
                const group = await chat_1.ChatService.getOrCreateTeacherGradeGroup(gradeId, courseTeacherId);
                await chat_1.ChatService.addMember(group.id, studentId, 'student');
            }
        }
        catch (err) {
        ...
        try {
            await pool_1.default.query(`INSERT INTO notifications (user_id, title, message, type, course_id) 
           VALUES ($1, $2, $3, $4, $5)`, [
                studentId,
                'كورس جديد متاح',
...
exports.router.delete('/:courseId/student/:studentId', (0, authentication_1.authMiddleware)(['teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const courseId = Number(req.params.courseId);
    const studentId = Number(req.params.studentId);
    const teacherId = req.user.id;
    ...
    const enrollmentCheck = await pool_1.default.query('SELECT e.*, u.name as student_name FROM enrollments e JOIN users u ON e.user_id = u.id WHERE e.course_id = $1 AND e.user_id = $2', [courseId, studentId]);
    if (!enrollmentCheck.rowCount) {
        return res.status(404).json({
    ...
    try {
        ...
        await pool_1.default.query('DELETE FROM enrollments WHERE course_id = $1 AND user_id = $2', [
            courseId,
            studentId,
        ]);
...
exports.router.post('/:courseId/open-for-student/:studentId', (0, authentication_1.authMiddleware)(['teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const courseId = Number(req.params.courseId);
    const studentId = Number(req.params.studentId);
    const teacherId = req.user.id;
    if (isNaN(courseId) || isNaN(studentId)) {
...
exports.router.get('/:courseId/students', (0, authentication_1.authMiddleware)(['teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const courseId = Number(req.params.courseId);
    const teacherId = req.user.id;
    if (isNaN(courseId)) {
        return res.status(400).json({ message: 'Invalid course ID' });
    ...
    const studentsResult = await pool_1.default.query(`SELECT 
        u.id,
        u.name,
        u.email,
        u.phone,
...
exports.router.get('/my-exam-grades', (0, authentication_1.authMiddleware)(['student']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const studentId = req.user.id;
    // جلب كل الكورسات المشترك فيها الطالب
    const coursesRes = await pool_1.default.query(`SELECT c.id, c.title FROM enrollments e JOIN courses c ON e.course_id = c.id WHERE e.user_id = $1`, [studentId]);
    const courses = coursesRes.rows;
    const allExams = [];
    for (const course of courses) {
        // 1. امتحانات المحاضرات (Lecture Exams)
        const lecturesRes = await pool_1.default.query(`SELECT l.id FROM lectures l WHERE l.course_id = $1`, [
            course.id,
        ]);
        const lectureIds = lecturesRes.rows.map((l) => l.id);
        if (lectureIds.length) {
            const lectureExamsRes = await pool_1.default.query(`SELECT e.id, e.title, e.total_grade, e.lecture_id, e.created_at 
           FROM exams e 
           WHERE e.lecture_id = ANY($1::int[]) AND e.type = 'exam'`, [lectureIds]);
            for (const exam of lectureExamsRes.rows) {
                const subRes = await pool_1.default.query(`SELECT total_grade, passed, submitted_at FROM exam_submissions WHERE exam_id = $1 AND student_id = $2`, [exam.id, studentId]);
                allExams.push({
                    course_id: course.id,
        ...
        const comprehensiveExamsRes = await pool_1.default.query(`SELECT e.id, e.title, e.questions_count as total_grade, e.created_at, e.is_active, e.is_visible_to_students
         FROM course_level_exams e
         WHERE e.course_id = $1 AND e.is_active = true AND e.is_visible_to_students = true`, [course.id]);
        for (const exam of comprehensiveExamsRes.rows) {
            // التحقق من وجود محاولة ناجحة أو درجات
            const subRes = await pool_1.default.query(`SELECT total_grade, obtained_grade, submitted_at, status 
           FROM course_level_exam_attempts 
           WHERE exam_id = $1 AND student_id = $2 AND status = 'submitted'
           ORDER BY submitted_at DESC LIMIT 1`, [exam.id, studentId]);
        ...
        const courseExamsRes = await pool_1.default.query(`SELECT e.id, e.title, e.total_grade, e.created_at, e.is_visible
         FROM course_exams e
         WHERE e.course_id = $1 AND e.is_visible = true`, [course.id]);
        for (const exam of courseExamsRes.rows) {
            const subRes = await pool_1.default.query(`SELECT total_grade, passed, submitted_at, attempts_count 
...
exports.router.get('/:courseId/students-progress', (0, authentication_1.authMiddleware)(['teacher', 'admin']), (0, utils_1.asyncWrapper)(async (req, res) => {
    try {
        const courseId = Number(req.params.courseId);
        if (isNaN(courseId)) {
        ...
        if (req.user.role === 'teacher') {
            const courseCheck = await pool_1.default.query('SELECT id, teacher_id FROM courses WHERE id = $1', [
                courseId,
            ]);
            if (!courseCheck.rowCount) {
                return res.status(404).json({
        ...
        const comprehensiveDataRes = await pool_1.default.query(`
        WITH course_data AS (
      ...
      `, [courseId]);
        ...
        const [lecturesRes, videosRes, lectureExamsRes] = await Promise.all([
            pool_1.default.query('SELECT id, title FROM lectures WHERE course_id = $1 ORDER BY position', [
                courseId,
            ]),
        ...
        const studentsDetails = students.map((student) => {
            ...
            lectures.forEach((lecture) => {
                ...
                const watchPercentage = totalLectureVideos > 0 ? (watchedCount / totalLectureVideos) * 100 : 0;
...
exports.router.get('/lecture-exam/:examId/submissions', (0, authentication_1.authMiddleware)(['teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const examId = Number(req.params.examId);
    if (isNaN(examId)) {
        return res.status(400).json({ message: 'Invalid examId' });
    }
    // تحقق أن الامتحان يخص المدرس
    const examRes = await pool_1.default.query(`SELECT e.*, l.course_id FROM exams e JOIN lectures l ON e.lecture_id = l.id WHERE e.id = $1`, [examId]);
    if (!examRes.rowCount) {
        return res.status(404).json({ message: 'Lecture exam not found' });
    }
    const courseId = examRes.rows[0].course_id;
    const courseCheck = await pool_1.default.query('SELECT id FROM courses WHERE id = $1 AND teacher_id = $2', [courseId, req.user.id]);
    if (!courseCheck.rowCount) {
        return res.status(403).json({ message: 'Not allowed' });
    }
    // جلب تفاصيل الطلاب الذين حلوا الامتحان
    const subsRes = await pool_1.default.query(`SELECT s.id as submission_id, s.student_id, s.total_grade, s.submitted_at, s.passed, u.name, u.email, u.phone
       FROM exam_submissions s
       JOIN users u ON s.student_id = u.id
       WHERE s.exam_id = $1
       ORDER BY s.submitted_at DESC`, [examId]);
    res.json({ submissions: subsRes.rows });
}));
// تسليم امتحان الكورس — للطالب فقط (نفس منطق POST /api/exams/:examId/submit)
exports.router.post('/course-exam/:examId/submit', (0, authentication_1.authMiddleware)(['student']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const examId = Number(req.params.examId);
    if (Number.isNaN(examId)) {
        return res.status(400).json({ message: 'Invalid exam id' });
    }
    ...
    if (Array.isArray(req.body.answers) && req.body.answers.length > 0) {
        answers = req.body.answers.filter((a) => a != null);
    }
    else if (Array.isArray(req.body.questionIds) &&
        Array.isArray(req.body.selectedAnswers) &&
        req.body.questionIds.length === req.body.selectedAnswers.length) {
        answers = req.body.questionIds.map((qId, i) => ({
            questionId: qId,
    ...
    if (answers.length === 0) {
        return res.status(400).json({
            message: 'answers required: send answers as array of { questionId, selectedAnswer }, or questionIds + selectedAnswers arrays, or answers as { "questionId": "A", ... }',
        });
    }
    if (!attemptId) {
        const activeRes = await pool_1.default.query(`SELECT id FROM course_level_exam_attempts
         WHERE exam_id = $1 AND student_id = $2 AND status = 'in_progress'
         ORDER BY started_at DESC LIMIT 1`, [examId, req.user.id]);
    ...
    for (const answer of answers) {
        ...
        if (selectedAnswer === undefined || selectedAnswer === '' || !validAnswers.includes(selectedAnswer)) {
            return res.status(400).json({
                message: 'Each answer must include the selected option: use selectedAnswer (or selected_answer, answer, choice, option) with value A/B/C/D or a/b/c/d or 0/1/2/3. Received for questionId ' +
                    questionId +
                    ': ' +
                    JSON.stringify(answer),
            });
        }
        validatedAnswers.push({ questionId, selectedAnswer: selectedAnswer });
    }
    try {
...
exports.router.get('/course-exam/:examId/submissions', (0, authentication_1.authMiddleware)(['teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const examId = Number(req.params.examId);
    if (isNaN(examId)) {
        return res.status(400).json({ message: 'Invalid examId' });
    }
    ...
    // جلب تفاصيل الطلاب الذين حلوا الامتحان
    const subsRes = await pool_1.default.query(`SELECT 
...
exports.router.get('/course-exam/:examId/report', (0, authentication_1.authMiddleware)(['teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const examId = Number(req.params.examId);
    if (isNaN(examId)) {
        return res.status(400).json({ message: 'Invalid examId' });
    }
...
exports.router.get('/video/:videoId', (0, authentication_1.authMiddleware)(), (0, utils_1.asyncWrapper)(async (req, res) => {
    const videoId = Number(req.params.videoId);
    const userId = req.user.id;
    ...
    const videoResult = await pool_1.default.query(`SELECT 
        lv.id,
        lv.video_url,
        lv.title as video_title,
        lv.position,
    ...
    if (userRole === 'student') {
        const canAccessLecture = await lectureExam_1.LectureExamService.canStudentAccessLecture(videoInfo.lecture_id, userId);
        if (!canAccessLecture) {
            return res.status(403).json({
...
exports.router.post('/video/:videoId/track-view', (0, authentication_1.authMiddleware)(['student']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const videoId = Number(req.params.videoId);
    const userId = req.user.id;
    ...
    const enrollmentCheck = await pool_1.default.query('SELECT id FROM enrollments WHERE user_id = $1 AND course_id = $2', [userId, video.course_id]);
    if (!enrollmentCheck.rowCount) {
        return res.status(403).json({ message: 'ليس لديك صلاحية لمشاهدة هذا الفيديو' });
    }
    try {
...
exports.router.get('/:courseId/students-progress-summary', (0, authentication_1.authMiddleware)(['teacher', 'admin']), (0, utils_1.asyncWrapper)(async (req, res) => {
    try {
        const courseId = Number(req.params.courseId);
        if (isNaN(courseId)) {
            return res.status(400).json({ message: 'Invalid courseId' });
        ...
        const summaryRes = await pool_1.default.query(`
        SELECT 
          -- إحصائيات الكورس
          COUNT(DISTINCT l.id) as total_lectures,
          COUNT(DISTINCT lv.id) as total_videos,
      ...
      `, [courseId]);
        ...
        res.json({
            ...
            students_with_lecture_exam_submissions: parseInt(summary.students_with_lecture_exam_submissions) || 0,
...
exports.router.get('/:courseId/student/:studentId/detailed-report', (0, authentication_1.authMiddleware)(['teacher', 'admin']), (0, utils_1.asyncWrapper)(async (req, res) => {
    try {
        const courseId = Number(req.params.courseId);
        const studentId = Number(req.params.studentId);
        const userId = req.user.id;
        ...
        // تحقق من الصلاحيات
        if (userRole === 'teacher') {
            const courseCheck = await pool_1.default.query('SELECT id, title, teacher_id FROM courses WHERE id = $1', [courseId]);
        ...
        // جلب معلومات الكورس
        const courseRes = await pool_1.default.query('SELECT id, title, description, teacher_id FROM courses WHERE id = $1', [courseId]);
        if (!courseRes.rowCount) {
            return res.status(404).json({ message: 'الكورس غير موجود' });
        }
        const course = courseRes.rows[0];
        // تحقق أن الطالب مشترك في الكورس
        const enrollmentCheck = await pool_1.default.query('SELECT enrolled_at FROM enrollments WHERE user_id = $1 AND course_id = $2', [studentId, courseId]);
        if (!enrollmentCheck.rowCount) {
            return res.status(404).json({ message: 'الطالب غير مشترك في هذا الكورس' });
        }
        // جلب المحاضرات
        const lecturesRes = await pool_1.default.query('SELECT id, title, description, position FROM lectures WHERE course_id = $1 ORDER BY position', [courseId]);
        const lectures = lecturesRes.rows;
        // جلب الفيديوهات
        const videosRes = await pool_1.default.query(`SELECT lv.id, lv.title, lv.position, lv.video_url, l.id as lecture_id, l.title as lecture_title
         FROM lecture_videos lv
         JOIN lectures l ON lv.lecture_id = l.id
         WHERE l.course_id = $1
         ORDER BY l.position, lv.position`, [courseId]);
        const videos = videosRes.rows;
        // جلب مشاهدات المحاضرات
        const lectureViewsRes = await pool_1.default.query('SELECT lecture_id, viewed_at FROM lecture_views WHERE user_id = $1 AND lecture_id = ANY($2::int[])', [studentId, lectures.map((l) => l.id)]);
        ...
        FROM video_views vv
        JOIN lecture_videos lv ON vv.video_id = lv.id
        WHERE vv.user_id = $1 AND vv.course_id = $2`, [studentId, courseId]);
        // جلب امتحانات المحاضرات
        const lectureExamsRes = await pool_1.default.query("SELECT id, title, lecture_id FROM exams WHERE lecture_id = ANY($1::int[]) AND type = 'exam'", [lectures.map((l) => l.id)]);
        const lectureExams = lectureExamsRes.rows;
        // جلب نتائج امتحانات المحاضرات
        const lectureExamResultsRes = await pool_1.default.query('SELECT exam_id, total_grade, submitted_at FROM exam_submissions WHERE student_id = $1 AND exam_id = ANY($2::int[])', [studentId, lectureExams.map((e) => e.id)]);
        // التحقق من قفل كل محاضرة (امتحان بـ "قفل المحاضرات التالية" - لا تُفتح حتى النجاح)
        const lectureAccessChecks = await Promise.all(lectures.map((lecture) => lectureExam_1.LectureExamService.canStudentAccessLecture(lecture.id, studentId)));
        const lectureCanAccessMap = new Map(lectures.map((l, i) => [l.id, lectureAccessChecks[i]]));
        ...
        const lecturesData = lectures.map((lecture) => {
            ...
            const examResults = lectureExamResultsRes.rows.filter((r) => lectureExamsForLecture.some((e) => e.id === r.exam_id));
            return {
                id: lecture.id,
                title: lecture.title,
        ...
        res.json({
            ...
            progress_summary: {
                ...
                lectures_completion_percentage: lectures.length > 0
                ...
                videos_completion_percentage: videos.length > 0 ? (videoViewsRes.rows.length / videos.length) * 100 : 0,
                total_lecture_exams: lectureExams.length,
                solved_lecture_exams: lectureExamResultsRes.rows.length,
                lecture_exams_completion_percentage: lectureExams.length > 0
