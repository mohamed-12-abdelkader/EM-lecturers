...
exports.router.get('/student/subjects/:subjectId/books', (0, authentication_1.authMiddleware)(['student']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const studentId = req.user.id;
    ...
    const booksResult = await pool_1.default.query(`SELECT 
        sb.id,
        sb.name,
        sb.description,
        sb.image_url,
       ...
       ORDER BY sb.order_num ASC, sb.name ASC`, [subjectId]);
    res.json({
        success: true,
...
exports.router.get('/student/books/:bookId/chapters', (0, authentication_1.authMiddleware)(['student']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const studentId = req.user.id;
    const bookId = Number(req.params.bookId);
    if (isNaN(bookId)) {
        return res.status(400).json({ message: 'Invalid book ID' });
    }
    const bookCheckResult = await pool_1.default.query(`SELECT sb.id
       FROM subject_books sb
       JOIN subjects s ON s.id = sb.subject_id
       JOIN question_banks qb ON qb.id = s.question_bank_id
       JOIN user_grades ug ON ug.grade_id = qb.grade_id
       WHERE sb.id = $1 AND ug.user_id = $2 AND sb.is_active = TRUE`, [bookId, studentId]);
    ...
    const chaptersResult = await pool_1.default.query(`SELECT 
       ...
       ORDER BY c.order_num ASC, c.name ASC`, [bookId]);
    res.json({
        success: true,
