...
const centerGroups_1 = require("../services/centerGroups");
 ...
 */
router.post('/:groupId/students', (0, authentication_1.authMiddleware)(['admin', 'teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    ...
    // Verify group exists
    const groupRes = await pool_1.default.query(`SELECT id, teacher_id, name FROM study_groups WHERE id = $1`, [groupId]);
    ...
    const result = await centerGroups_1.CenterGroupsService.addStudentToGroup(groupId, name.trim(), phone?.trim() || null, parent_phone?.trim() || null);
    res.status(201).json({
 ...
 * Body: { names: string[] }
 */
router.post('/:groupId/students/bulk', (0, authentication_1.authMiddleware)(['admin', 'teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const groupId = Number(req.params.groupId);
    if (Number.isNaN(groupId)) {
        return res.status(400).json({ message: 'Invalid group id' });
    }
    ...
    const result = await centerGroups_1.CenterGroupsService.addStudentsByNamesToGroup(groupId, names);
 ...
 */
router.get('/:groupId/students', (0, authentication_1.authMiddleware)(['admin', 'teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const groupId = Number(req.params.groupId);
    if (Number.isNaN(groupId)) {
        return res.status(400).json({ message: 'Invalid group id' });
    }
    ...
    const students = await centerGroups_1.CenterGroupsService.getGroupStudents(groupId);
    const studentsWithQr = await Promise.all(students.map(async (s) => {
        const qrPayload = centerGroups_1.CenterGroupsService.generateAttendanceQrPayload(groupId, s.student_id);
        const qrCodeDataUrl = await centerGroups_1.CenterGroupsService.generateQrDataUrl(qrPayload);
 ...
 */
router.get('/:groupId/students/:studentId', (0, authentication_1.authMiddleware)(['admin', 'teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const groupId = Number(req.params.groupId);
    ...
    const membershipRes = await pool_1.default.query(`SELECT gs.id, gs.joined_at
       FROM group_students gs
       WHERE gs.group_id = $1 AND gs.student_id = $2`, [groupId, studentId]);
    if (!membershipRes.rowCount) {
        return res.status(404).json({ message: 'Student is not a member of this group' });
    }
    const membership = membershipRes.rows[0];
    // Get student details
    const studentRes = await pool_1.default.query(`SELECT id, name, phone, parent_phone, email, avatar, created_at
    ...
    if (include_attendance !== 'false') {
        attendance = await centerGroups_1.CenterGroupsService.getStudentAttendance(groupId, studentId, start_date, end_date);
    ...
    const qrPayload = centerGroups_1.CenterGroupsService.generateAttendanceQrPayload(groupId, studentId);
    const qrCodeDataUrl = await centerGroups_1.CenterGroupsService.generateQrDataUrl(qrPayload);
    res.json({
        group: {
            id: group.id,
 ...
 */
router.post('/:groupId/attendance/scan', (0, authentication_1.authMiddleware)(['admin', 'teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const groupId = Number(req.params.groupId);
    if (Number.isNaN(groupId)) {
        return res.status(400).json({ message: 'Invalid group id' });
    }
    ...
    let decoded;
    try {
        decoded = centerGroups_1.CenterGroupsService.verifyAttendanceQrPayload(qrPayload.trim());
    }
    catch {
    ...
    const attendance = await centerGroups_1.CenterGroupsService.recordAttendance(groupId, decoded.studentId, today, 'present');
    res.status(201).json({
 ...
 */
router.post('/:groupId/attendance', (0, authentication_1.authMiddleware)(['admin', 'teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const groupId = Number(req.params.groupId);
    if (Number.isNaN(groupId)) {
        return res.status(400).json({ message: 'Invalid group id' });
    }
    ...
    // Record attendance
    const attendance = await centerGroups_1.CenterGroupsService.recordAttendance(groupId, Number(student_id), date, status);
    res.status(201).json({
        message: 'تم تسجيل الحضور بنجاح',
        attendance,
    });
 ...
 */
router.post('/:groupId/attendance/bulk', (0, authentication_1.authMiddleware)(['admin', 'teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const groupId = Number(req.params.groupId);
    if (Number.isNaN(groupId)) {
        return res.status(400).json({ message: 'Invalid group id' });
    }
    ...
    const result = await centerGroups_1.CenterGroupsService.recordBulkAttendance(groupId, date, attendance);
    res.status(201).json({
 ...
 */
router.get('/:groupId/attendance', (0, authentication_1.authMiddleware)(['admin', 'teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const groupId = Number(req.params.groupId);
    if (Number.isNaN(groupId)) {
        return res.status(400).json({ message: 'Invalid group id' });
    }
    ...
    if (date) {
        ...
        const attendance = await centerGroups_1.CenterGroupsService.getAttendanceByDate(groupId, date);
        // Get all students in group to show who hasn't been marked
        const allStudents = await centerGroups_1.CenterGroupsService.getGroupStudents(groupId);
        ...
        const summary = await centerGroups_1.CenterGroupsService.getAttendanceSummary(groupId, start_date, end_date);
        res.json({
            group: {
                id: group.id,
                name: group.name,
 ...
 */
router.get('/:groupId/attendance/students/:studentId', (0, authentication_1.authMiddleware)(['admin', 'teacher']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const groupId = Number(req.params.groupId);
    const studentId = Number(req.params.studentId);
    if (Number.isNaN(groupId) || Number.isNaN(studentId)) {
        return res.status(400).json({ message: 'Invalid group id or student id' });
    ...
    const attendance = await centerGroups_1.CenterGroupsService.getStudentAttendance(groupId, studentId, start_date, end_date);
    // Calculate statistics
    const totalDays = attendance.length;
    const presentDays = attendance.filter((a) => a.status === 'present').length;
    const absentDays = attendance.filter((a) => a.status === 'absent').length;
    const attendanceRate = totalDays > 0 ? Math.round((presentDays / totalDays) * 100) : 0;
    // Get student info
    const studentRes = await pool_1.default.query(`SELECT id, name, phone, parent_phone FROM users WHERE id = $1`, [studentId]);
    res.json({
        group: {
            id: group.id,
            name: group.name,
