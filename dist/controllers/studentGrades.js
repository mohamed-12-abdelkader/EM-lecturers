...
exports.router.post('/', (0, authentication_1.authMiddleware)(['teacher', 'admin']), (0, utils_1.asyncWrapper)(async (req, res) => {
    ...
    const studentCheck = await pool_1.default.query('SELECT id, name, role FROM users WHERE id = $1', [
        student_id,
    ]);
    ...
    const examCheck = await pool_1.default.query(`SELECT ge.id, ge.name, ge.total_grade, ge.group_id, sg.name as group_name
       FROM group_exams ge
       JOIN study_groups sg ON ge.group_id = sg.id
       WHERE ge.name = $1 AND sg.teacher_id = $2`, [exam_name, teacherId]);
    ...
    const studentInGroup = await pool_1.default.query('SELECT 1 FROM group_students WHERE group_id = $1 AND student_id = $2', [exam.group_id, student_id]);
    if (studentInGroup.rows.length === 0) {
        // جلب قائمة الطلاب في المجموعة للمساعدة
        const groupStudents = await pool_1.default.query(`SELECT gs.student_id, u.name 
    ...
    const result = await pool_1.default.query(`INSERT INTO group_exam_grades 
       ...
       DO UPDATE SET 
         ...
         updated_at = NOW()
       RETURNING *`, [exam.id, student_id, grade, notes || null]);
...
exports.router.get('/group/:groupId/student/:studentId', (0, authentication_1.authMiddleware)(['teacher', 'admin']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const { groupId, studentId } = req.params;
    const teacherId = req.user.id;
    // التحقق من أن المجموعة موجودة وأن المدرس مالكها
    const groupCheck = await pool_1.default.query('SELECT id, name, teacher_id FROM study_groups WHERE id = $1', [groupId]);
    ...
    // التحقق من أن الطالب في المجموعة
    const studentInGroup = await pool_1.default.query('SELECT 1 FROM group_students WHERE group_id = $1 AND student_id = $2', [groupId, studentId]);
    if (studentInGroup.rows.length === 0) {
        return res.status(400).json({
            error: 'طالب غير موجود في المجموعة',
    ...
    const grades = await pool_1.default.query(`SELECT geg.id, geg.grade, geg.notes, geg.created_at, geg.updated_at,
       ...
       ORDER BY ge.created_at DESC`, [studentId, groupId]);
    res.json({
        group: {
        ...
        average_grade: grades.rows.length > 0
...
exports.router.get('/student/:studentId', (0, authentication_1.authMiddleware)(['teacher', 'admin']), (0, utils_1.asyncWrapper)(async (req, res) => {
    ...
    const grades = await pool_1.default.query(`SELECT geg.id, geg.grade, geg.notes, geg.created_at, geg.updated_at,
       ...
       ORDER BY ge.created_at DESC`, [studentId, teacherId]);
    res.json({
        student: {
            id: studentCheck.rows[0].id,
            name: studentCheck.rows[0].name,
...
exports.router.get('/exam/:examName', (0, authentication_1.authMiddleware)(['teacher', 'admin']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const { examName } = req.params;
    const teacherId = req.user.id;
    // البحث عن الامتحان
    const examCheck = await pool_1.default.query(`SELECT ge.id, ge.name, ge.total_grade, ge.exam_date, ge.group_id, sg.name as group_name
       FROM group_exams ge
       JOIN study_groups sg ON ge.group_id = sg.id
       WHERE ge.name = $1 AND sg.teacher_id = $2`, [examName, teacherId]);
    if (examCheck.rows.length === 0) {
        return res.status(404).json({
            error: 'امتحان غير موجود',
    ...
    const stats = await pool_1.default.query(`SELECT 
       ...
       WHERE gs.group_id = $2`, [exam.id, exam.group_id]);
    res.json({
        exam: {
            id: exam.id,
            name: exam.name,
...
// تحديث درجة طالب
exports.router.put('/:gradeId', (0, authentication_1.authMiddleware)(['teacher', 'admin']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const { gradeId } = req.params;
    const { grade, notes } = req.body;
    const teacherId = req.user.id;
    // التحقق من وجود الدرجة وأنها تخص المدرس
    const gradeCheck = await pool_1.default.query(`SELECT geg.*, ge.name as exam_name, ge.total_grade, sg.teacher_id
       FROM group_exam_grades geg
       JOIN group_exams ge ON geg.exam_id = ge.id
       JOIN study_groups sg ON ge.group_id = sg.id
       WHERE geg.id = $1`, [gradeId]);
...
// حذف درجة طالب
exports.router.delete('/:gradeId', (0, authentication_1.authMiddleware)(['teacher', 'admin']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const { gradeId } = req.params;
    const teacherId = req.user.id;
    // التحقق من وجود الدرجة وأنها تخص المدرس
    const gradeCheck = await pool_1.default.query(`SELECT geg.*, ge.name as exam_name, sg.teacher_id
       FROM group_exam_grades geg
       JOIN group_exams ge ON geg.exam_id = ge.id
       JOIN study_groups sg ON ge.group_id = sg.id
       WHERE geg.id = $1`, [gradeId]);
    ...
    // حذف الدرجة
    await pool_1.default.query('DELETE FROM group_exam_grades WHERE id = $1', [gradeId]);
    res.json({
        message: 'تم حذف الدرجة بنجاح',
    });
}));
// جلب قائمة امتحانات المدرس
exports.router.get('/exams/list', (0, authentication_1.authMiddleware)(['teacher', 'admin']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const teacherId = req.user.id;
    const exams = await pool_1.default.query(`SELECT ge.id, ge.name, ge.total_grade, ge.exam_date, ge.created_at,
              sg.name as group_name, sg.id as group_id,
              COUNT(geg.student_id) as graded_students,
       ...
       ORDER BY ge.created_at DESC`, [teacherId]);
    res.json({
        exams: exams.rows,
    });
}));
// إضافة درجة مباشرة للطالب في المجموعة (بدون امتحان فعلي)
exports.router.post('/direct', (0, authentication_1.authMiddleware)(['teacher', 'admin']), (0, utils_1.asyncWrapper)(async (req, res) => {
    ...
    // التحقق من أن المجموعة موجودة وأن المدرس مالكها
    const groupCheck = await pool_1.default.query('SELECT id, name, teacher_id FROM study_groups WHERE id = $1', [group_id]);
    if (groupCheck.rows.length === 0) {
        return res.status(404).json({
            error: 'مجموعة غير موجودة',
    ...
    // التحقق من أن الطالب في المجموعة
    const studentInGroup = await pool_1.default.query('SELECT 1 FROM group_students WHERE group_id = $1 AND student_id = $2', [group_id, student_id]);
    if (studentInGroup.rows.length === 0) {
        return res.status(400).json({
            error: 'طالب غير موجود في المجموعة',
    ...
    let examId;
    const existingExam = await pool_1.default.query('SELECT id FROM group_exams WHERE name = $1 AND group_id = $2', [exam_name, group_id]);
    if (existingExam.rows.length > 0) {
        examId = existingExam.rows[0].id;
    }
    else {
        // إنشاء امتحان وهمي
        const newExam = await pool_1.default.query(`INSERT INTO group_exams (name, group_id, total_grade, exam_date, created_at) 
         VALUES ($1, $2, $3, NOW(), NOW()) 
         RETURNING id`, [exam_name, group_id, examTotalGrade]);
    ...
    const result = await pool_1.default.query(`INSERT INTO group_exam_grades 
       ...
       DO UPDATE SET 
         ...
         notes = EXCLUDED.notes,
         updated_at = NOW()
       RETURNING *`, [examId, student_id, grade, notes || null]);
    const gradeRecord = result.rows[0];
    res.status(201).json({
        message: 'تم إضافة الدرجة بنجاح',
        grade: {
...
exports.router.post('/direct/bulk', (0, authentication_1.authMiddleware)(['teacher', 'admin']), (0, utils_1.asyncWrapper)(async (req, res) => {
...
exports.router.post('/bulk', (0, authentication_1.authMiddleware)(['teacher', 'admin']), (0, utils_1.asyncWrapper)(async (req, res) => {
...
exports.router.get('/group/:groupId/students', (0, authentication_1.authMiddleware)(['teacher', 'admin']), (0, utils_1.asyncWrapper)(async (req, res) => {
    ...
    const students = await pool_1.default.query(`SELECT gs.student_id, gs.joined_at,
...
exports.router.get('/group/:groupId/report', (0, authentication_1.authMiddleware)(['teacher', 'admin']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const { groupId } = req.params;
    const teacherId = req.user.id;
    // التحقق من ملكية المجموعة
    const groupCheck = await pool_1.default.query('SELECT id, name, teacher_id FROM study_groups WHERE id = $1', [groupId]);
    ...
    const groupStats = await pool_1.default.query(`SELECT 
       ...
       LEFT JOIN group_exam_grades geg ON ge.id = geg.exam_id AND gs.student_id = geg.student_id
       WHERE gs.group_id = $1`, [groupId]);
    // جلب درجات الطلاب في جميع الامتحانات
    const studentGrades = await pool_1.default.query(`SELECT 
         gs.student_id,
       ...
       ORDER BY average_grade DESC NULLS LAST`, [groupId]);
    // جلب تفاصيل الامتحانات
    const examDetails = await pool_1.default.query(`SELECT 
         ge.id,
         ge.name,
         ge.total_grade,
         ge.exam_date,
       ...
       ORDER BY ge.exam_date DESC`, [groupId]);
    res.json({
        group: {
            id: groupCheck.rows[0].id,
            name: groupCheck.rows[0].name,
...
exports.router.get('/exam/:examName/report', (0, authentication_1.authMiddleware)(['teacher', 'admin']), (0, utils_1.asyncWrapper)(async (req, res) => {
    const { examName } = req.params;
    const teacherId = req.user.id;
    // البحث عن الامتحان
    const examCheck = await pool_1.default.query(`SELECT ge.id, ge.name, ge.total_grade, ge.exam_date, ge.group_id, sg.name as group_name
    ...
    const exam = examCheck.rows[0];
    // جلب إحصائيات الامتحان
    const examStats = await pool_1.default.query(`SELECT 
         COUNT(gs.student_id) as total_students,
         COUNT(geg.student_id) as graded_students,
         AVG(geg.grade) as average_grade,
         MAX(geg.grade) as highest_grade,
    ...
    const studentGrades = await pool_1.default.query(`SELECT 
       ...
       JOIN group_exams ge ON ge.id = $1
       WHERE gs.group_id = $2
       ORDER BY geg.grade DESC NULLS LAST`, [exam.id, exam.group_id]);
    // جلب توزيع الدرجات
    const gradeDistribution = await pool_1.default.query(`SELECT 
         CASE 
